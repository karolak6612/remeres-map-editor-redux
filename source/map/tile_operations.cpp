...
namespace TileOperations {
	...
	namespace {
		...
		void UpdateItemFlags(const Item* i, uint16_t& statflags, uint8_t& minimapColor) {
			if (i->isSelected()) {
				statflags |= static_cast<uint16_t>(TileInternalState::SELECTED);
			}
			if (i->getUniqueID() != 0) {
				statflags |= static_cast<uint16_t>(TileInternalState::UNIQUE);
			}
			...
			if (it_type.unpassable) {
				statflags |= static_cast<uint16_t>(TileInternalState::BLOCKING);
			}
			if (it_type.isOptionalBorder) {
				statflags |= static_cast<uint16_t>(TileInternalState::OP_BORDER);
			}
			if (it_type.isTable) {
				statflags |= static_cast<uint16_t>(TileInternalState::HAS_TABLE);
			}
			if (it_type.isCarpet) {
				statflags |= static_cast<uint16_t>(TileInternalState::HAS_CARPET);
			}
			if (it_type.hookSouth) {
				statflags |= static_cast<uint16_t>(TileInternalState::HOOK_SOUTH);
			}
			if (it_type.hookEast) {
				statflags |= static_cast<uint16_t>(TileInternalState::HOOK_EAST);
			}
			if (i->hasLight()) {
				statflags |= static_cast<uint16_t>(TileInternalState::HAS_LIGHT);
			}
		}
	...
	void select(Tile* tile) {
		...
		tile->statflags |= static_cast<uint16_t>(TileInternalState::SELECTED);
	}
	...
	void update(Tile* tile) {
		tile->statflags &= static_cast<uint16_t>(TileInternalState::MODIFIED);
		...
		if (tile->ground) {
			...
			if (tile->ground->isBlocking()) {
				tile->statflags |= static_cast<uint16_t>(TileInternalState::BLOCKING);
			}
			if (tile->ground->getUniqueID() != 0) {
				tile->statflags |= static_cast<uint16_t>(TileInternalState::UNIQUE);
			}
			if (tile->ground->getMiniMapColor() != 0) {
				tile->minimapColor = tile->ground->getMiniMapColor();
			}
			if (tile->ground->hasLight()) {
				tile->statflags |= static_cast<uint16_t>(TileInternalState::HAS_LIGHT);
			}
		} else {
		...
		if ((tile->statflags & static_cast<uint16_t>(TileInternalState::BLOCKING)) == 0) {
