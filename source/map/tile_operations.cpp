...
namespace TileOperations {
	...
	namespace {
		...
		void UpdateItemFlags(const Item* i, uint16_t& statflags, uint8_t& minimapColor) {
			if (i->isSelected()) {
				statflags |= static_cast<uint16_t>(TileStateFlag::SELECTED);
			}
			if (i->getUniqueID() != 0) {
				statflags |= static_cast<uint16_t>(TileStateFlag::UNIQUE);
			}
			...
			if (it_type.unpassable) {
				statflags |= static_cast<uint16_t>(TileStateFlag::BLOCKING);
			}
			if (it_type.isOptionalBorder) {
				statflags |= static_cast<uint16_t>(TileStateFlag::OP_BORDER);
			}
			if (it_type.isTable) {
				statflags |= static_cast<uint16_t>(TileStateFlag::HAS_TABLE);
			}
			if (it_type.isCarpet) {
				statflags |= static_cast<uint16_t>(TileStateFlag::HAS_CARPET);
			}
			if (it_type.hookSouth) {
				statflags |= static_cast<uint16_t>(TileStateFlag::HOOK_SOUTH);
			}
			if (it_type.hookEast) {
				statflags |= static_cast<uint16_t>(TileStateFlag::HOOK_EAST);
			}
			if (i->hasLight()) {
				statflags |= static_cast<uint16_t>(TileStateFlag::HAS_LIGHT);
			}
		}
	...
	void select(Tile* tile) {
		...
		tile->statflags |= static_cast<uint16_t>(TileStateFlag::SELECTED);
	}
	...
	void update(Tile* tile) {
		tile->statflags &= static_cast<uint16_t>(TileStateFlag::MODIFIED);
		...
		if (tile->ground) {
			...
			if (tile->ground->isBlocking()) {
				tile->statflags |= static_cast<uint16_t>(TileStateFlag::BLOCKING);
			}
			if (tile->ground->getUniqueID() != 0) {
				tile->statflags |= static_cast<uint16_t>(TileStateFlag::UNIQUE);
			}
			if (tile->ground->getMiniMapColor() != 0) {
				tile->minimapColor = tile->ground->getMiniMapColor();
			}
			if (tile->ground->hasLight()) {
				tile->statflags |= static_cast<uint16_t>(TileStateFlag::HAS_LIGHT);
			}
		} else {
			tile->mapflags = static_cast<uint16_t>(TileMapFlag::NONE);
			tile->house_id = 0;
		}
		...
		if ((tile->statflags & static_cast<uint16_t>(TileStateFlag::BLOCKING)) == 0) {
