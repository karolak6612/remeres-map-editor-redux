...
protected:
    ...
    template <typename Func>
    void
    visitLeavesImpl(int start_nx, int start_ny, int end_nx, int end_ny, int start_cx, int start_cy, int end_cx, int end_cy, Func&& func)
    {
        if (cells_.empty()) {
            return;
        }
        ...
        for (int cy = start_cy; cy <= end_cy; ++cy) {
            ...
            while (it != cells_.end() && it->key <= row_end_key) {
                ...
                if (cx >= start_cx && cx <= end_cx) {
                    ...
                    row_cells.push_back(
                        {.cell = it->cell.get(),
                         .cell_start_nx = cell_start_nx,
                         .local_start_nx = std::max(start_nx, cell_start_nx) - cell_start_nx,
                         .local_end_nx = std::min(end_nx, cell_start_nx + NODES_PER_CELL - 1) - cell_start_nx}
                    );
                }
                ++it;
            }
