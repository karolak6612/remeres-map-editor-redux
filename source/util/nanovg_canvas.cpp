...
int NanoVGCanvas::CreateGameSpriteTexture(NVGcontext *vg, GameSprite *gs,
                                          uint64_t spriteId) {
        ...
        for (int sy = 0; sy < 32; ++sy) {
          for (int sx = 0; sx < 32; ++sx) {
            ...
            if (sa == 255) {
              ...
              float a = sa / 255.0f;
              float ia = 1.0f - a;
              composite[di + 0] = static_cast<uint8_t>(data[si + 0] * a +
                                                       composite[di + 0] * ia);
              composite[di + 1] = static_cast<uint8_t>(data[si + 1] * a +
                                                       composite[di + 1] * ia);
              composite[di + 2] = static_cast<uint8_t>(data[si + 2] * a +
                                                       composite[di + 2] * ia);
...
int NanoVGCanvas::CreateGenericSpriteTexture(NVGcontext *vg, Sprite *sprite,
                                             uint64_t spriteId) {
...
int NanoVGCanvas::GetOrCreateImage(uint64_t id, const uint8_t *data, int width,
                                   int height) {
...
int NanoVGCanvas::GetCachedImage(uint64_t id) const {
  // Const-cast to call MakeContextCurrent if needed, or just assume it's for
  // lookup Actually GetCachedImage doesn't call GL, it just looks in the map.
  // So it's fine.
  auto it = m_imageCache.find(id);
  if (it != m_imageCache.end()) {
    // Update LRU
