					...
					for (int sy = 0; sy < 32; ++sy) {
						for (int sx = 0; sx < 32; ++sx) {
							...
							if (sa == 255) {
								...
								float a = sa / 255.0f;
								float inv_a = 1.0f - a;
								composite.get()[dst_idx + 0] = (uint8_t)(spriteData.get()[src_idx + 0] * a + composite.get()[dst_idx + 0] * inv_a);
								composite.get()[dst_idx + 1] = (uint8_t)(spriteData.get()[src_idx + 1] * a + composite.get()[dst_idx + 1] * inv_a);
								composite.get()[dst_idx + 2] = (uint8_t)(spriteData.get()[src_idx + 2] * a + composite.get()[dst_idx + 2] * inv_a);
								composite.get()[dst_idx + 3] = std::max(composite.get()[dst_idx + 3], sa);
							}
						}
					}
				}
