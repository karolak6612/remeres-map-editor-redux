 ...
 *protected : *void OnNanoVGPaint(NVGcontext *vg, int width,
                                  int height) override {
     *nvgBeginPath(vg);
...
public:
    ...
    ~ScopedGLContext() = default; // Restoration of previous context is not reliably possible in
                                  // wx without a global tracker
...
public:
     ...
     * @param style Additional window styles (default includes vertical
     * scrollbar)
     */
