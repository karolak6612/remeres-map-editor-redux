...
/**
 * @class NanoVGCanvas
 * @brief Base class for wxGLCanvas controls that use NanoVG for
 * hardware-accelerated 2D rendering.
 *
 * This class provides a standardized pattern for creating high-performance,
 * custom-drawn UI controls using NanoVG. It handles OpenGL context management,
 * NanoVG initialization, texture caching, and virtual scrolling.
 *
 * ## Usage
...
public:
  ...
  ~ScopedGLContext() =
      default; // Restoration of previous context is not reliably possible in wx
               // without a global tracker
...
public:
   ...
   */
  NanoVGCanvas(wxWindow *parent, wxWindowID id = wxID_ANY,
               long style = wxVSCROLL | wxWANTS_CHARS);
   ...
   */
  [[nodiscard]] int GetScrollPosition() const { return m_scrollPos; }
   ...
   */
  [[nodiscard]] NVGcontext *GetNVGContext() const { return m_nvg.get(); }
  ...
  /**
   * @brief Gets or creates a cached NanoVG image for a Sprite (GameSprite or
   * generic).
   */
...
protected:
  ...
  int CreateGameSpriteTexture(NVGcontext *vg, GameSprite *gs,
                              uint64_t spriteId);
  int CreateGenericSpriteTexture(NVGcontext *vg, Sprite *sprite,
                                 uint64_t spriteId);
