...
inline std::unique_ptr<uint8_t[]> CreateCompositeRGBA(GameSprite &gs, int &outW,
                                                      int &outH) {
  outW = gs.width * 32;
  outH = gs.height * 32;
  ...
  for (int l = 0; l < gs.layers; ++l) {
    for (int w = 0; w < gs.width; ++w) {
      for (int h = 0; h < gs.height; ++h) {
        int spriteIdx =
            gs.getIndex(w, h, l, pattern_x, pattern_y, pattern_z, frame);
        ...
        // Right-to-left, bottom-to-top arrangement (standard RME rendering
        // order)
        ...
        for (int sy = 0; sy < 32; ++sy) {
          for (int sx = 0; sx < 32; ++sx) {
            ...
            if (sa == 255) {
              ...
              float inv_a = 1.0f - a;
              composite[dst_idx + 0] =
                  (uint8_t)(spriteData[src_idx + 0] * a +
                            composite[dst_idx + 0] * inv_a);
              composite[dst_idx + 1] =
                  (uint8_t)(spriteData[src_idx + 1] * a +
                            composite[dst_idx + 1] * inv_a);
              composite[dst_idx + 2] =
                  (uint8_t)(spriteData[src_idx + 2] * a +
                            composite[dst_idx + 2] * inv_a);
...
inline int CreateItemTexture(NVGcontext *vg, uint16_t id) {
  ...
  GameSprite *gs =
      dynamic_cast<GameSprite *>(g_gui.sprite_database.getSprite(it.clientID));
  if (!gs) {
    return 0;
  }
