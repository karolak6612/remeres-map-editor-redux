...
void LuaScriptsWindow::RefreshScriptList() {
	...
	for (size_t i = 0; i < scripts.size(); ++i) {
		...
		if (!script->isEnabled()) {
			script_list->SetItemTextColour(index, Theme::Get(Theme::Role::TextSubtle));
		} else {
			script_list->SetItemTextColour(index, Theme::Get(Theme::Role::Text));
		}
	}
...
void LuaScriptsWindow::LogMessage(const wxString& message, bool isError) {
	...
	wxTextAttr attr;
	if (isError) {
		attr.SetTextColour(Theme::Get(Theme::Role::Error)); // Red for errors
	} else {
		attr.SetTextColour(Theme::Get(Theme::Role::Text)); // Normal
	}
