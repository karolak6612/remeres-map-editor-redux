...
namespace LuaAPI {
	...
	static Creature* setTileCreature(Tile* tile, const std::string& creatureName, sol::optional<int> spawnTimeOpt, sol::optional<int> directionOpt) {
		...
		// Create new creature
		tile->creature.reset(newd Creature(creatureName));
	...
	static Spawn* setTileSpawn(Tile* tile, sol::optional<int> sizeOpt) {
		...
		tile->spawn.reset(newd Spawn(size));
		...
		lua.new_usertype<Tile>(
			...
			"moveItem", sol::overload(
							...
							[](Tile* tile, Item* item, int toIdx) {
								...
								auto it = std::find_if(tile->items.begin(), tile->items.end(), [item](const std::unique_ptr<Item>& uptr){
								...
								markTileForUndo(tile);
								std::unique_ptr<Item> uItem = std::move(*it);
								tile->items.erase(it);

								int insertPos = std::clamp(to, 0, (int)tile->items.size());
								tile->items.insert(tile->items.begin() + insertPos, std::move(uItem));
								tile->modify();
							},
							...
							[](Tile* sourceTile, Item* item, Tile* destTile, sol::optional<int> toIdx) {
								...
								auto it = std::find_if(sourceTile->items.begin(), sourceTile->items.end(), [item](const std::unique_ptr<Item>& uptr){
									return uptr.get() == item;
								});
								...
								destTile->items.insert(destTile->items.begin() + to, std::move(uItem));
