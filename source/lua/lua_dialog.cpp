...
public:
	MapPreviewCanvas(wxWindow* parent, Editor& editor) :
		MapCanvas(nullptr, editor, nullptr), // Pass null parent first
		parentWindow(parent) {
		// MapCanvas constructor expects MapWindow* as parent for internal use, but we pass nullptr.
        // We must reparent the wxGLCanvas to the actual parent dialog to ensure it is displayed correctly.
        Reparent(parent);
...
LuaDialog* LuaDialog::grid(sol::table options) {
	...
	int width = options.get_or(std::string("width"), 300);
	int height = options.get_or(std::string("height"), 200);
	...
	// Size options
	int iconSize = options.get_or(std::string("icon_size"), -1);
	int itemSize = options.get_or(std::string("item_size"), -1); // Alias
	int itemWidth = options.get_or(std::string("item_width"), -1);
	int itemHeight = options.get_or(std::string("item_height"), -1);
	int iconWidthOpt = options.get_or(std::string("icon_width"), -1);
	int iconHeightOpt = options.get_or(std::string("icon_height"), -1);
	int cellSize = options.get_or(std::string("cell_size"), -1);
	int cellWidthOpt = options.get_or(std::string("cell_width"), -1);
	int cellHeightOpt = options.get_or(std::string("cell_height"), -1);
	...
	// Calculate default cell size if not provided
	if (cellWidth <= 0) {
	...
	LuaGridCtrl* listctrl = new LuaGridCtrl(getParentForWidget(), wxID_ANY, wxDefaultPosition, wxSize(width, height), wxLC_ICON | wxLC_AUTOARRANGE | wxLC_SINGLE_SEL);

	// Setup internal state
	listctrl->iconWidth = iconWidth;
	listctrl->iconHeight = iconHeight;
	listctrl->cellWidth = cellWidth;
	listctrl->cellHeight = cellHeight;
...
#ifdef __WXMSW__
	// On Windows, we can control spacing directly
	ListView_SetIconSpacing(static_cast<HWND>(listctrl->GetHandle()), cellWidth, cellHeight);
#endif

	getSizerForWidget()->Add(listctrl, 1, getSizerFlags(options, wxEXPAND | wxALL), getSizerBorder(options));
	...
	if (options["items"].valid()) {
		...
		for (auto& pair : itemsTable) {
			if (pair.second.is<sol::table>()) {
				...
				long index = listctrl->InsertItem(listctrl->GetItemCount(), text, imageIndex);
				listctrl->SetItemData(index, index + 1); // 1-based index stored in data

				std::string tooltip = itemTable.get_or("tooltip", ""s);
				if (!tooltip.empty()) {
					listctrl->AddTooltip(index, tooltip);
				}
			}
		}
		listctrl->Thaw();
	...
	if (selection > 0 && selection <= listctrl->GetItemCount()) {
	...
	// Bind selection event
	listctrl->Bind(wxEVT_LIST_ITEM_SELECTED, [this, id](wxListEvent&) {
		onWidgetChange(id);
	});
	listctrl->Bind(wxEVT_LIST_ITEM_DESELECTED, [this, id](wxListEvent&) {
		onWidgetChange(id);
	});

	// Bind double click event
	listctrl->Bind(wxEVT_LIST_ITEM_ACTIVATED, [this, id](wxListEvent&) {
		onWidgetDoubleClick(id);
	});

	// Left click handling (trickier with ListCtrl)
	sol::function gridLeftClick = widget.onleftclick;
	if (gridLeftClick.valid()) {
		listctrl->Bind(wxEVT_LEFT_DOWN, [this, id, listctrl, gridLeftClick](wxMouseEvent& event) {
			if (!gridLeftClick.valid() || !lua.lua_state() || !gridLeftClick.lua_state()) {
				event.Skip();
				return;
			}
			int flags = 0;
			long index = listctrl->HitTest(event.GetPosition(), flags);
			if (index == wxNOT_FOUND) {
				event.Skip();
				return;
			}

			// Manually select since we intercept event
			...
			info["type"] = "grid";
			info["index"] = index + 1;
			info["text"] = listctrl->GetItemText(index).ToStdString();
			info["widget_id"] = id;
			// Tooltip not easily accessible from item directly without lookup
			std::string tip = static_cast<LuaGridCtrl*>(listctrl)->GetTooltip(index);
			if (!tip.empty()) {
				info["tooltip"] = tip;
			}
			...
			// Don't skip if we handled selection, or do skip to let default handler run too?
			// Generally safer to Skip() for native control behavior unless we want to block it
			event.Skip();
		});
	}
	...
	listctrl->Bind(wxEVT_RIGHT_DOWN, [this, id, listctrl, gridRightClick](wxMouseEvent& event) {
	...
	listctrl->Bind(wxEVT_CONTEXT_MENU, [this, id, listctrl, gridContextMenu](wxContextMenuEvent& event) {
		...
		wxPoint clientPos = listctrl->ScreenToClient(screenPos);
		int flags = 0;
		long index = listctrl->HitTest(clientPos, flags);
		...
		popupContextMenu(gridContextMenu, info, listctrl, screenPos);
	});

	applyCommonOptions(listctrl, options);
	return this;
}
