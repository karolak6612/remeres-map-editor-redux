...
namespace IngamePreview {
    ...
    IngamePreviewRenderer::IngamePreviewRenderer(TileRenderer* tile_renderer) : tile_renderer(tile_renderer)
    {
    ...
    void IngamePreviewRenderer::Render(
        NVGcontext* vg, const BaseMap& map, int viewport_x, int viewport_y, int viewport_width, int viewport_height,
        const Position& camera_pos, float zoom, bool lighting_enabled, uint8_t ambient_light, const Outfit& preview_outfit,
        Direction preview_direction, int animation_phase, int offset_x, int offset_y
    )
    {
        ...
        view.view_scroll_x
            = (camera_pos.x * TILE_SIZE) + (TILE_SIZE / 2) - offset + offset_x - static_cast<int>(viewport_width * zoom / 2.0f);
        view.view_scroll_y
            = (camera_pos.y * TILE_SIZE) + (TILE_SIZE / 2) - offset + offset_y - static_cast<int>(viewport_height * zoom / 2.0f);

        // Matching RME's projection (width * zoom x height * zoom)
        view.projectionMatrix
            = glm::ortho(0.0f, static_cast<float>(viewport_width) * zoom, static_cast<float>(viewport_height) * zoom, 0.0f, -1.0f, 1.0f);
        ...
        for (int z = last_visible; z >= 0; z--) {
            ...
            int floor_offset = (z <= GROUND_LAYER) ? (GROUND_LAYER - z) * TILE_SIZE : TILE_SIZE * (view.floor - z);
            int camera_offset = (camera_pos.z <= GROUND_LAYER) ? (GROUND_LAYER - camera_pos.z) * TILE_SIZE : 0;
            ...
            view.end_x = static_cast<int>(
                std::ceil((view.view_scroll_x + viewport_width * zoom + margin + max_floor_offset) / static_cast<float>(TILE_SIZE))
            );
            view.end_y = static_cast<int>(
                std::ceil((view.view_scroll_y + viewport_height * zoom + margin + max_floor_offset) / static_cast<float>(TILE_SIZE))
            );
            ...
            for (int x = view.start_x; x <= view.end_x; ++x) {
                for (int y = view.start_y; y <= view.end_y; ++y) {
                    ...
                    if (tile) {
                        ...
                        const DrawContext ctx {*sprite_batch, *primitive_renderer, view,          render_settings,
                                               frame_options, *light_buffer,       accumulators_, *atlas};
        ...
        // Assuming always drawn on top.
        {
            ...
            creature_drawer->BlitCreature(
                *sprite_batch, sprite_drawer.get(), draw_x, draw_y, preview_outfit, preview_direction,
