...
uint16_t Item::LiquidName2ID(std::string liquid) {
	static const std::map<std::string, uint16_t> name_to_id = [] {
		std::map<std::string, uint16_t> m;
		for (const auto& [id, name_str] : liquid_names) {
			std::string lower_name = name_str;
			to_lower_str(lower_name);
			m.try_emplace(lower_name, id);
		}
		return m;
	}();

	to_lower_str(liquid);
	const auto it = name_to_id.find(liquid);
