...
std::unique_ptr<Item> Item::Create(uint16_t _type,
                                   uint16_t _subtype /*= 0xFFFF*/) {
  if (_type == 0) {
    return nullptr;
  }
...
Item::Item(unsigned short _type, unsigned short _count)
    : id(_type), subtype(1), selected(false) {
  if (hasSubtype()) {
    subtype = _count;
  }
}
...
void Item::setID(uint16_t newid) { id = newid; }

void Item::setSubtype(uint16_t n) { subtype = n; }

bool Item::hasSubtype() const {
  const ItemType &it = g_items[id];
  return (it.isFluidContainer() || it.isSplash() || isCharged() ||
          it.stackable || it.charges != 0);
}
...
int Item::getHeight() const {
  ...
  const ItemType &it = g_items[id];
  // In Tibia, elevation is typically equal to a full tile step (sometimes
  // variable). RME's ItemType has 'hasElevation' bool. If hasElevation is true,
  // we assume standard elevation (8px usually in client draw logic terms, but
  // logical step is 1). For drawing: we want pixels. 8px is standard elevation
  // step.
  return it.hasElevation ? 8 : 0;
}

void Item::setUniqueID(unsigned short n) { setAttribute(ATTR_UID, n); }

void Item::setActionID(unsigned short n) { setAttribute(ATTR_AID, n); }

void Item::setText(const std::string &str) { setAttribute(ATTR_TEXT, str); }

void Item::setDescription(const std::string &str) {
  setAttribute(ATTR_DESC, str);
}

void Item::setTier(unsigned short n) { setAttribute(ATTR_TIER, n); }

double Item::getWeight() { return const_cast<const Item *>(this)->getWeight(); }

bool Item::canHoldText() const { return isReadable() || canWriteText(); }

bool Item::canHoldDescription() const { return g_items[id].allowDistRead; }
...
GroundBrush *Item::getGroundBrush() const {
  ...
  if (item_type.isGroundTile() && item_type.brush &&
      item_type.brush->is<GroundBrush>()) {
...
CarpetBrush *Item::getCarpetBrush() const {
  ...
  if (item_type.isCarpet && item_type.brush &&
      item_type.brush->is<CarpetBrush>()) {
...
std::unique_ptr<Item> Item::Create(pugi::xml_node xml) {
  ...
  int16_t count = 1;
  if ((attribute = xml.attribute("count")) ||
      (attribute = xml.attribute("subtype"))) {
