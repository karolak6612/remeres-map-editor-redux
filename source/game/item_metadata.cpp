...
bool ItemMetadataManager::load(const std::string& filename) {
    ...
    for (pugi::xml_node node = root.child("item"); node; node = node.next_sibling("item")) {
        ...
        if (typeStr == "invisible-stairs") meta.techType = TechItemType::INVISIBLE_STAIRS;
        else if (typeStr == "invisible-walkable") meta.techType = TechItemType::INVISIBLE_WALKABLE;
        else if (typeStr == "invisible-wall") meta.techType = TechItemType::INVISIBLE_WALL;
        else if (typeStr == "primal-light") meta.techType = TechItemType::PRIMAL_LIGHT;
        else if (typeStr == "client-id-zero") meta.techType = TechItemType::CLIENT_ID_ZERO;
        else meta.techType = TechItemType::NONE;
...
void ItemMetadataManager::applyLegacyDefaults() {
    ...
    for (size_t i = 100; i <= g_items.getMaxID(); ++i) {
        if (!g_items.typeExists(i)) continue;
