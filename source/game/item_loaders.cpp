...
void parseItemTypeAttribute(ItemType &it, std::string_view value) {
  static const std::unordered_map<std::string_view, void (*)(ItemType &)>
      parsers = {
...
void parseSlotTypeAttribute(ItemType &it, std::string_view value) {
  static const std::unordered_map<std::string_view, void (*)(ItemType &)>
      parsers = {
          ...
          {"two-handed",
           [](ItemType &i) { i.slot_position |= SLOTP_TWO_HAND; }},
...
void parseFloorChangeAttribute(ItemType &it, std::string_view value) {
  static const std::unordered_map<std::string_view, void (*)(ItemType &)>
      parsers = {
          {"down",
           [](ItemType &i) {
             i.floorChangeDown = true;
             i.floorChange = true;
           }},
          {"north",
           [](ItemType &i) {
             i.floorChangeNorth = true;
             i.floorChange = true;
           }},
          {"south",
           [](ItemType &i) {
             i.floorChangeSouth = true;
             i.floorChange = true;
           }},
          {"west",
           [](ItemType &i) {
             i.floorChangeWest = true;
             i.floorChange = true;
           }},
          {"east",
           [](ItemType &i) {
             i.floorChangeEast = true;
             i.floorChange = true;
           }},
...
bool OtbLoader::load(ItemDatabase &db, BinaryNode *itemNode,
                     OtbFileFormatVersion version, wxString &error,
  ...
  for (; itemNode != nullptr; itemNode = itemNode->advance()) {
    ...
    static const auto group_handlers = [] {
      std::array<void (*)(ItemType &, OtbFileFormatVersion),
                 ITEM_GROUP_LAST + 1>
          h{};
      h.fill([](ItemType &, OtbFileFormatVersion) {});
      h[ITEM_GROUP_DOOR] = [](ItemType &i,
                              [[maybe_unused]] OtbFileFormatVersion v) {
        i.type = ITEM_TYPE_DOOR;
      };
      h[ITEM_GROUP_CONTAINER] = [](ItemType &i,
                                   [[maybe_unused]] OtbFileFormatVersion v) {
        i.type = ITEM_TYPE_CONTAINER;
      };
      h[ITEM_GROUP_RUNE] = [](ItemType &i,
                              [[maybe_unused]] OtbFileFormatVersion v) {
        i.client_chargeable = true;
      };
      h[ITEM_GROUP_TELEPORT] = [](ItemType &i,
                                  [[maybe_unused]] OtbFileFormatVersion v) {
        i.type = ITEM_TYPE_TELEPORT;
      };
      h[ITEM_GROUP_MAGICFIELD] = [](ItemType &i,
                                    [[maybe_unused]] OtbFileFormatVersion v) {
        i.type = ITEM_TYPE_MAGICFIELD;
      };
    ...
    if (g_gui.sprite_database.dat_format == DAT_FORMAT_1010) {
    ...
    if (itemNode->getU32(flags)) {
      ...
      t.blockPathfinder =
          ((flags & FLAG_BLOCK_PATHFINDER) == FLAG_BLOCK_PATHFINDER);
      ...
      t.floorChangeDown =
          ((flags & FLAG_FLOORCHANGEDOWN) == FLAG_FLOORCHANGEDOWN);
      t.floorChangeNorth =
          ((flags & FLAG_FLOORCHANGENORTH) == FLAG_FLOORCHANGENORTH);
      t.floorChangeEast =
          ((flags & FLAG_FLOORCHANGEEAST) == FLAG_FLOORCHANGEEAST);
      t.floorChangeSouth =
          ((flags & FLAG_FLOORCHANGESOUTH) == FLAG_FLOORCHANGESOUTH);
      t.floorChangeWest =
          ((flags & FLAG_FLOORCHANGEWEST) == FLAG_FLOORCHANGEWEST);
      t.floorChange = t.floorChangeDown || t.floorChangeNorth ||
                      t.floorChangeEast || t.floorChangeSouth ||
                      t.floorChangeWest;

      // The OTB `FLAG_ALWAYSONTOP` is mapped to the editor's `alwaysOnBottom`
      // property.
    ...
    using AttributeHandler =
        bool (*)(ItemDatabase &, ItemType &, BinaryNode *, uint16_t, wxString &,
                 std::vector<std::string> &);
    static const auto handlers = [] {
      std::array<AttributeHandler, std::numeric_limits<uint8_t>::max() + 1> h{};
      h.fill([](ItemDatabase &, ItemType &, BinaryNode *node, uint16_t len,
                [[maybe_unused]] wxString &,
                [[maybe_unused]] std::vector<std::string> &) {
        node->skip(len);
        return true;
      });

      h[ITEM_ATTR_SERVERID] = [](ItemDatabase &db, ItemType &it,
                                 BinaryNode *node, uint16_t len, wxString &err,
                                 std::vector<std::string> &warnings) {
        if (len != sizeof(uint16_t)) {
          err = std::format("items.otb: Unexpected data length of server id "
                            "block (Should be {} bytes)",
                            sizeof(uint16_t));
          return false;
        }
      ...
      h[ITEM_ATTR_CLIENTID] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                                 uint16_t len, wxString &err,
                                 std::vector<std::string> &warnings) {
        if (len != sizeof(uint16_t)) {
          err = std::format("items.otb: Unexpected data length of client id "
                            "block (Should be {} bytes)",
                            sizeof(uint16_t));
          return false;
        }
        ...
        it.sprite = static_cast<GameSprite *>(
            g_gui.sprite_database.getSprite(it.clientID));
        return true;
      };

      h[ITEM_ATTR_SPEED] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                              uint16_t len, wxString &err,
                              std::vector<std::string> &warnings) {
        if (len != sizeof(uint16_t)) {
          err = std::format("items.otb: Unexpected data length of speed block "
                            "(Should be {} bytes)",
                            sizeof(uint16_t));
          return false;
        }
      ...
      h[ITEM_ATTR_LIGHT2] = [](ItemDatabase &, ItemType &, BinaryNode *node,
                               uint16_t len, [[maybe_unused]] wxString &,
                               std::vector<std::string> &warnings) {
        const size_t expected_len = 4; // sizeof(lightBlock2)
        if (len != expected_len) {
          warnings.push_back(
              std::format("items.otb: Unexpected data length of item light (2) "
                          "block (Should be {} bytes)",
                          expected_len));
          return true;
        }
      ...
      h[ITEM_ATTR_TOPORDER] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                                 uint16_t len, [[maybe_unused]] wxString &,
                                 std::vector<std::string> &warnings) {
        if (len != sizeof(uint8_t)) {
          warnings.push_back("items.otb: Unexpected data length of item "
                             "toporder block (Should be 1 byte)");
          return true;
        }
        uint8_t u8_top = 0;
      ...
      h[ITEM_ATTR_NAME] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                             uint16_t len, [[maybe_unused]] wxString &,
                             std::vector<std::string> &warnings) {
        if (len >= 128) {
          warnings.push_back("items.otb: Unexpected data length of item name "
                             "block (Should be < 128 bytes)");
          return true;
        }
        uint8_t name[128] = {0};
      ...
      h[ITEM_ATTR_DESCR] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                              uint16_t len, [[maybe_unused]] wxString &,
                              std::vector<std::string> &warnings) {
        if (len >= 128) {
          warnings.push_back("items.otb: Unexpected data length of item descr "
                             "block (Should be < 128 bytes)");
          return true;
        }
      ...
      h[ITEM_ATTR_MAXITEMS] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                                 uint16_t len, [[maybe_unused]] wxString &,
                                 std::vector<std::string> &warnings) {
        if (len != sizeof(uint16_t)) {
          warnings.push_back("items.otb: Unexpected data length of item volume "
                             "block (Should be 2 bytes)");
          return true;
        }
      ...
      h[ITEM_ATTR_WEIGHT] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                               uint16_t len, [[maybe_unused]] wxString &,
                               std::vector<std::string> &warnings) {
        if (len != sizeof(double)) {
          warnings.push_back("items.otb: Unexpected data length of item weight "
                             "block (Should be 8 bytes)");
          return true;
        }
        double weight;
        if (!node->getRAW(reinterpret_cast<uint8_t *>(&weight),
                          sizeof(double))) {
      ...
      h[ITEM_ATTR_ROTATETO] = [](ItemDatabase &, ItemType &it, BinaryNode *node,
                                 uint16_t len, [[maybe_unused]] wxString &,
                                 std::vector<std::string> &warnings) {
        if (len != sizeof(uint16_t)) {
          warnings.push_back("items.otb: Unexpected data length of item "
                             "rotateTo block (Should be 2 bytes)");
          return true;
        }
        uint16_t rotate = 0;
      ...
      h[ITEM_ATTR_WRITEABLE3] =
          [](ItemDatabase &, ItemType &it, BinaryNode *node, uint16_t len,
             [[maybe_unused]] wxString &, std::vector<std::string> &warnings) {
            const size_t expected_len = 4; // sizeof(writeableBlock3)
            if (len != expected_len) {
              warnings.push_back(
                  std::format("items.otb: Unexpected data length of item "
                              "writeable (3) block (Should be {} bytes)",
                              expected_len));
              return true;
            }
            ...
            if (!node->getU16(readOnlyID)) {
              warnings.push_back(
                  "Invalid item type property (writeable3_readOnlyID)");
            }
            if (!node->getU16(maxTextLen)) {
              warnings.push_back(
                  "Invalid item type property (writeable3_maxTextLen)");
            }
            it.maxTextLen = maxTextLen;
            return true;
          };

      h[ITEM_ATTR_CLASSIFICATION] =
          [](ItemDatabase &, ItemType &it, BinaryNode *node, uint16_t len,
             [[maybe_unused]] wxString &, std::vector<std::string> &warnings) {
            if (len != sizeof(uint8_t)) {
              warnings.push_back("items.otb: Unexpected data length of item "
                                 "classification block (Should be 1 byte)");
              return true;
            }
            uint8_t cls = 0;
            if (!node->getU8(cls)) {
...
void XmlLoader::loadAttributes(ItemType &it, pugi::xml_node node) {
  ...
  static const std::unordered_map<std::string_view, ParserFunc> parsers = {
      {"type",
       [](ItemType &it, pugi::xml_node node) {
      ...
      {"decayto",
       [](ItemType &it, [[maybe_unused]] pugi::xml_node node) {
         it.decays = true;
       }},
      {"maxtextlen",
       [](ItemType &it, pugi::xml_node node) {
