...
bool OtbLoader::load(
    ItemDatabase& db, BinaryNode* itemNode, OtbFileFormatVersion version, wxString& error, std::vector<std::string>& warnings
)
{
    uint8_t u8;
    ...
    for (; itemNode != nullptr; itemNode = itemNode->advance()) {
        ...
        static const auto handlers = [] {
            ...
            h.fill([](ItemDatabase&, ItemType&, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                      [[maybe_unused]] std::vector<std::string>&) {
                node->skip(len);
                return true;
            });

            h[ITEM_ATTR_SERVERID]
                = [](ItemDatabase& db, ItemType& it, BinaryNode* node, uint16_t len, wxString& err, std::vector<std::string>& warnings) {
            ...
            h[ITEM_ATTR_CLIENTID]
                = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, wxString& err, std::vector<std::string>& warnings) {
                      if (len != sizeof(uint16_t)) {
            ...
            h[ITEM_ATTR_LIGHT2] = [](ItemDatabase&, ItemType&, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
            ...
            h[ITEM_ATTR_TOPORDER] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                       std::vector<std::string>& warnings) {
            ...
            h[ITEM_ATTR_NAME] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                   std::vector<std::string>& warnings) {
                if (len >= 128) {
            ...
            h[ITEM_ATTR_DESCR] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                    std::vector<std::string>& warnings) {
                if (len >= 128) {
            ...
            h[ITEM_ATTR_MAXITEMS] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                       std::vector<std::string>& warnings) {
                if (len != sizeof(uint16_t)) {
            ...
            h[ITEM_ATTR_WEIGHT] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                     std::vector<std::string>& warnings) {
            ...
            h[ITEM_ATTR_ROTATETO] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                       std::vector<std::string>& warnings) {
                if (len != sizeof(uint16_t)) {
            ...
            h[ITEM_ATTR_WRITEABLE3] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                         std::vector<std::string>& warnings) {
                const size_t expected_len = 4; // sizeof(writeableBlock3)
                if (len != expected_len) {
                    warnings.push_back(
                        std::format("items.otb: Unexpected data length of item writeable (3) block (Should be {} bytes)", expected_len)
                    );
                    return true;
                }
            ...
            h[ITEM_ATTR_CLASSIFICATION] = [](ItemDatabase&, ItemType& it, BinaryNode* node, uint16_t len, [[maybe_unused]] wxString&,
                                             std::vector<std::string>& warnings) {
                if (len != sizeof(uint8_t)) {
...
void XmlLoader::loadAttributes(ItemType& it, pugi::xml_node node)
{
    ...
    static const std::unordered_map<std::string_view, ParserFunc> parsers
        = {{"type",
           ...
           {"decayto", [](ItemType& it, [[maybe_unused]] pugi::xml_node node) { it.decays = true; }},
           {"maxtextlen",
            [](ItemType& it, pugi::xml_node node) {
                if (auto attr = node.attribute("value")) {
