...
CreatureType::CreatureType()
    : isNpc(false), missing(false), in_other_tileset(false), standard(false),
      name(""), brush(nullptr) {
  ////
}

CreatureType::CreatureType(const CreatureType &ct)
    : isNpc(ct.isNpc), missing(ct.missing),
      in_other_tileset(ct.in_other_tileset), standard(ct.standard),
      name(ct.name), outfit(ct.outfit), brush(ct.brush) {
  ////
}
...
void CreatureType::preserve_assign_creature_fields(CreatureType *dest,
                                                   const CreatureType &src) {
...
CreatureType *CreatureType::loadFromXML(pugi::xml_node node,
  ...
  if (tmpType != "monster" && tmpType != "npc") {
    warnings.push_back((wxString("Invalid type tag of creature node \"") +
                        wxstr(tmpType) + "\"")
                           .ToStdString());
    return nullptr;
  }
  ...
  if ((attribute = node.attribute("looktype"))) {
    ...
    if (g_gui.sprite_database.getCreatureSprite(ct->outfit.lookType) ==
        nullptr) {
      warnings.push_back((wxString("Invalid creature \"") + wxstr(ct->name) +
                          "\" look type #" +
                          std::to_string(ct->outfit.lookType))
                             .ToStdString());
    }
  } else {
...
CreatureType *CreatureType::loadFromOTXML(const FileName &filename,
  ...
  for (pugi::xml_node optionNode = node.first_child(); optionNode;
       optionNode = optionNode.next_sibling()) {
    ...
    if ((attribute = optionNode.attribute("item")) ||
...
CreatureDatabase::~CreatureDatabase() { clear(); }
...
CreatureType *CreatureDatabase::addCreatureType(const std::string &name,
                                                bool isNpc,
                                                const Outfit &outfit) {
  assert((*this)[name] == nullptr);
...
bool CreatureDatabase::hasMissing() const {
  return std::ranges::any_of(
      creature_map, [](const auto &pair) { return pair.second->missing; });
}

bool CreatureDatabase::loadFromXML(const FileName &filename, bool standard,
                                   wxString &error,
                                   std::vector<std::string> &warnings) {
  pugi::xml_document doc;
  pugi::xml_parse_result result =
      doc.load_file(filename.GetFullPath().mb_str());
  if (!result) {
  ...
  for (pugi::xml_node creatureNode = node.first_child(); creatureNode;
    ...
    CreatureType *creatureType =
        CreatureType::loadFromXML(creatureNode, warnings);
    if (creatureType) {
      creatureType->standard = standard;
      if ((*this)[creatureType->name]) {
        warnings.push_back((wxString("Duplicate creature type name \"") +
                            wxstr(creatureType->name) + "\"! Discarding...")
                               .ToStdString());
        delete creatureType;
      } else {
...
bool CreatureDatabase::importXMLFromOT(const FileName &filename,
  ...
  if ((node = doc.child("monsters"))) {
    for (pugi::xml_node monsterNode = node.first_child(); monsterNode;
         monsterNode = monsterNode.next_sibling()) {
      ...
      pugi::xml_parse_result monsterResult =
          monsterDoc.load_file(monsterFile.GetFullPath().mb_str());
      if (!monsterResult) {
        continue;
      }

      CreatureType *creatureType =
          CreatureType::loadFromOTXML(monsterFile, monsterDoc, warnings);
      if (creatureType) {
    ...
    CreatureType *creatureType =
        CreatureType::loadFromOTXML(filename, doc, warnings);
    if (creatureType) {
      CreatureType *current = (*this)[creatureType->name];
...
bool CreatureDatabase::saveToXML(const FileName &filename) {
  ...
  for (const auto &creatureEntry : creature_map) {
    ...
    if (!creatureType->standard) {
      ...
      creatureNode.append_attribute("type") =
          creatureType->isNpc ? "npc" : "monster";
  ...
  return doc.save_file(filename.GetFullPath().mb_str(), "\t",
