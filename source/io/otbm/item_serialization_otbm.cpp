    ...
    switch (attr) {
        ...
        case OTBM_ATTR_PODIUMOUTFIT: {
            if (auto podium = item.asPodium()) {
                ...
                if (!stream->getU16(lookType) || !stream->getU8(lookHead) || !stream->getU8(lookBody) || !stream->getU8(lookLegs)
                    || !stream->getU8(lookFeet) || !stream->getU8(lookAddon) || !stream->getU16(lookMount) || !stream->getU8(lookMountHead)
                    || !stream->getU8(lookMountBody) || !stream->getU8(lookMountLegs) || !stream->getU8(lookMountFeet)) {
                    return false;
                }
    ...
    if (maphandle.version.otbm >= MAP_OTBM_4) {
        ...
        if (tier > 0) {
            if (tier <= 0xFF) {
                ...
                spdlog::warn(
                    "ItemSerializationOTBM: Item '{}' has tier {} which is too large for uint8_t, truncating to 255", item.getName(), tier
                );
                f.addU8(OTBM_ATTR_TIER);
                f.addU8(0xFF);
            }
