...
bool IOMapOTMM::saveMap(Map& map, NodeFileWriteHandle& f, const FileName& identifier, bool showdialog) {
	...
	f.addNode(0);
	{
		...
		f.addNode(OTMM_MAP_DATA);
		{
			...
			{
				for (const auto& tile_loc : map.tiles()) {
					// Update progressbar
					++tiles_saved;
					...
					// Get tile
					const Tile* save_tile = tile_loc.get();
					if (!save_tile) {
						continue;
					}
			...
			f.addNode(OTMM_SPAWN_DATA);
			{
				...
				for (const auto& spawn_pos : spawns) {
					const Tile* tile = map.getTile(spawn_pos);
					ASSERT(tile);
					...
					f.addNode(OTMM_SPAWN_AREA);
					{
						...
						for (int y = -tile->spawn->getSize(); y <= tile->spawn->getSize(); ++y) {
							for (int x = -tile->spawn->getSize(); x <= tile->spawn->getSize(); ++x) {
								Tile* creature_tile = map.getTile(spawn_pos + Position(x, y, 0));
								if (creature_tile) {
