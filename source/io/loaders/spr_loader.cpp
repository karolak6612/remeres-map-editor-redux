...
bool SprLoader::LoadData(SpriteDatabase *database, const wxFileName &datafile,
                         wxString &error, std::vector<std::string> &warnings) {
  FileReadHandle fh(nstr(datafile.GetFullPath()));

  if (!fh.isOk()) {
    error = wxstr(std::format("Failed to open file {} for reading",
                              datafile.GetFullPath().utf8_string()));
    return false;
  }
  ...
  if (total_pics > MAX_SPRITES) {
    error = wxstr(std::format("Sprite count {} exceeds limit (MAX_SPRITES={})",
                              total_pics, MAX_SPRITES));
    return false;
  }

  database->spritefile = nstr(datafile.GetFullPath());
  database->setUnloaded(false);
  ...
  // Resize image_space to match exact sprite count, removing potential stale
  // entries
  database->image_space.resize(total_pics + 1);

  std::vector<uint32_t> sprite_indexes =
      ReadSpriteIndexes(fh, total_pics, error);
  if (sprite_indexes.empty() && total_pics > 0) {
    return false;
  }

  return ReadSprites(database, fh, sprite_indexes, warnings, error);
}

std::vector<uint32_t> SprLoader::ReadSpriteIndexes(FileReadHandle &fh,
                                                   uint32_t total_pics,
                                                   wxString &error) {
  std::vector<uint32_t> sprite_indexes;
...
bool SprLoader::ReadSprites(SpriteDatabase *database, FileReadHandle &fh,
  ...
  for (uint32_t index : sprite_indexes) {
    ...
    if (!fh.seek(seek_pos)) {
      ...
      warnings.push_back(std::format(
          "SprLoader: Failed to seek to sprite data at offset {} for id {}",
          seek_pos, id));
      continue;
    }
    ...
    if (id < database->image_space.size() && database->image_space[id]) {
      NormalImage *spr =
          dynamic_cast<NormalImage *>(database->image_space[id].get());
      if (spr) {
        if (size > 0) {
          if (spr->size > 0) {
            // Duplicate GameSprite id
            warnings.push_back(
                std::format("items.spr: Duplicate GameSprite id {}", id));
        ...
        warnings.push_back(std::format(
            "SprLoader: Failed to cast sprite id {} to NormalImage", id));
        if (size > 0) {
          if (!fh.seekRelative(size)) {
            error = wxstr(fh.getErrorMessage());
            return false;
...
bool SprLoader::LoadDump(SpriteDatabase *database,
                         std::unique_ptr<uint8_t[]> &target, uint16_t &size,
                         int sprite_id) {
  if (!database) {
    return false;
  }
  return LoadDump(database->spritefile, database->is_extended, target, size,
                  sprite_id);
}

bool SprLoader::LoadDump(const std::string &filename, bool extended,
                         std::unique_ptr<uint8_t[]> &target, uint16_t &size,
                         int sprite_id) {
  if (sprite_id == 0) {
    // Empty GameSprite
  ...
  uint32_t address_size =
      extended ? SPRITE_ADDRESS_SIZE_EXTENDED : SPRITE_ADDRESS_SIZE_NORMAL;
