...
bool ReadFlagData(DatFormat format, FileReadHandle &file, GameSprite *sType,
                  uint8_t flag, uint8_t previous_flag,
  ...
  default: {
    warnings.push_back(
        std::format("Metadata: Unknown flag: {}. Previous flag: {}.",
                    static_cast<int>(flag), static_cast<int>(previous_flag)));
    break;
  }
  }
  return true;
...
bool LoadMetadataFlags(DatFormat format, FileReadHandle &file,
                       GameSprite *sType, uint32_t sprite_id,
                       std::vector<std::string> &warnings) {
  ...
  for (int count = 0; count < static_cast<int>(DatFlagLast); ++count) {
    ...
    if (!file.getU8(flag)) {
      warnings.push_back(std::format(
          "Metadata: error reading flag for sprite id {}", sprite_id));
      return false;
    }
    ...
    if (!ReadFlagData(format, file, sType, flag, previous_flag, warnings)) {
      warnings.push_back(std::format(
          "Metadata: error reading flag data for flag {} for sprite id {}",
          static_cast<int>(flag), sprite_id));
      return false;
    }
  }
  // Sanity check: If we exit the loop without hitting DatFlagLast, it's
  // potential corruption.
  warnings.push_back(
      std::format("Metadata: corruption warning - flag list exceeded limit "
                  "(255) without terminator for sprite id {}",
                  sprite_id));
  return true; // We continue even if there was no terminator, as it's just a
               // warning
}

} // namespace

bool DatLoader::LoadMetadata(SpriteDatabase *database,
                             const wxFileName &datafile, wxString &error,
                             std::vector<std::string> &warnings) {
  // items.otb has most of the info we need. This only loads the GameSprite
  // metadata
  FileReadHandle file(nstr(datafile.GetFullPath()));

  if (!file.isOk()) {
    // C++20 std::format
    error += wxstr(std::format(
        "Failed to open {} for reading\nThe error reported was: {}",
        datafile.GetFullPath().utf8_string(), file.getErrorMessage()));
    return false;
  }
  ...
  // get max id
  if (!file.getU16(database->item_count) ||
      !file.getU16(database->creature_count) || !file.getU16(effect_count) ||
      !file.getU16(distance_count)) {
    error = "Failed to read dat header counts";
    return false;
  }

  if (database->item_count == 0 || database->creature_count == 0) {
    error = "Invalid dat header counts (zero items or creatures)";
    return false;
  }

  constexpr uint32_t minID = 100; // items start with id 100
  const uint32_t max_id_needed =
      database->item_count + database->creature_count + 1;

  // Pre-size containers to avoid rehashing and frequent allocations
  database->sprite_space.resize(max_id_needed);
  // Resize image_space to MAX_SPRITES to ensure OOB access doesn't happen
  // during sprite id reading. SprLoader will later resize it to the exact
  // count, but we need it safe now.
  if (database->image_space.size() < MAX_SPRITES) {
    database->image_space.resize(MAX_SPRITES);
  }

  database->dat_format =
      database->client_version->getDatFormatForSignature(datSignature);

  if (database->dat_format == DAT_FORMAT_UNKNOWN) {
    error = wxstr(std::format("Unknown dat signature: {:#x}", datSignature));
    return false;
  }

  // Initialize flags from ClientVersion configuration
  database->is_extended = database->client_version->isExtended();
  database->has_frame_durations = database->client_version->hasFrameDurations();
  database->has_frame_groups = database->client_version->hasFrameGroups();
  database->has_transparency = database->client_version->isTransparent();

  uint32_t id = minID;
  const uint32_t maxID = database->item_count + database->creature_count;
  while (id <= maxID) {
    auto sTypeUnique = std::make_unique<GameSprite>();
    GameSprite *sType = sTypeUnique.get();
    database->sprite_space[id] = std::move(sTypeUnique);

    sType->id = id;

    // Load flags
    if (!LoadMetadataFlags(database->dat_format, file, sType, id, warnings)) {
    ...
    if (database->has_frame_groups && id > database->item_count) {
    ...
    for (uint32_t k = 0; k < static_cast<uint32_t>(group_count); ++k) {
      if (!ReadSpriteGroup(database, file, sType, k, warnings)) {
        error = wxstr(
            std::format("Failed to read sprite group {} for id {}", k, id));
        return false;
      }
    }
    ++id;
...
bool DatLoader::ReadSpriteGroup(SpriteDatabase *database, FileReadHandle &file,
                                GameSprite *sType, uint32_t group_index,
                                std::vector<std::string> &warnings) {

  // Skipping the group type
  if (database->has_frame_groups && sType->id > database->item_count) {
    if (!file.skip(1)) {
      return false;
    }
  }

  uint8_t width = 0, height = 0, layers = 0, pattern_x = 0, pattern_y = 0,
          pattern_z = 0, frames = 0;
  ...
  if (database->dat_format <= DAT_FORMAT_74) {
    pattern_z = 1;
  } else {
  ...
  if (frames > 1) {
    ...
    if (database->has_frame_durations) {
    ...
    if (group_index == 0) {
      sType->animator = std::make_unique<Animator>(frames, start_frame,
                                                   loop_count, async == 1);
    }
    ...
    if (database->has_frame_durations) {
      for (int i = 0; i < static_cast<int>(frames); ++i) {
        ...
        if (group_index == 0) {
          ...
          frame_duration->setValues(static_cast<int>(min),
                                    static_cast<int>(max));
        }
      }
      if (group_index == 0) {
  ...
  uint32_t numsprites =
      ...
      static_cast<uint32_t>(layers) * static_cast<uint32_t>(pattern_x) *
      static_cast<uint32_t>(pattern_y) * static_cast<uint32_t>(pattern_z) *
  ...
  for (uint32_t i = 0; i < numsprites; ++i) {
    ...
    if (group_index == 0) {
      // Validate sprite_id
      if (sprite_id == UINT32_MAX || sprite_id >= MAX_SPRITES ||
          sprite_id >= database->image_space.size()) {
        return false;
      }

      auto &imgPtr = database->image_space[sprite_id];
      if (!imgPtr) {
