...
bool IOMapOTBM::loadSpawns(Map& map, const FileName& dir) {
	std::string fn = dir.GetPath(wxPATH_GET_SEPARATOR | wxPATH_GET_VOLUME).mb_str(wxConvUTF8).data();
	fn += map.spawnfile;
	...
	std::string encoded_path = dir.GetPath(wxPATH_GET_SEPARATOR | wxPATH_GET_VOLUME).mb_str(wxConvWhateverWorks).data();
