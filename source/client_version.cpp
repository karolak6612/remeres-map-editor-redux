...
void ClientVersion::loadVersion(pugi::xml_node versionNode) {
	...
	auto version = std::make_unique<ClientVersion>(otb_versions[otbVersionName], versionName, wxstr(dataPath));
	...
	if (client_versions.find(version->getID()) != client_versions.end()) {
		wxLogError("Duplicate version id %i, discarding version '%s'.", version->getID(), version->name);
		return;
	}

	ClientVersion* rawVersion = version.get();
