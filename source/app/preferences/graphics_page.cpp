...
GraphicsPage::GraphicsPage(wxWindow *parent) : PreferencesPage(parent) {
  ...
  hide_items_when_zoomed_chkbox =
      newd wxCheckBox(this, wxID_ANY, "Hide items when zoomed out");
  hide_items_when_zoomed_chkbox->SetValue(
      g_settings.getBoolean(Config::HIDE_ITEMS_WHEN_ZOOMED));
  sizer->Add(hide_items_when_zoomed_chkbox,
             wxSizerFlags().Border(wxLEFT | wxTOP, FROM_DIP(this, 5)));
  SetWindowToolTip(hide_items_when_zoomed_chkbox,
                   "When this option is checked, \"loose\" items will be "
                   "hidden when you zoom very far out.");

  icon_selection_shadow_chkbox =
      newd wxCheckBox(this, wxID_ANY, "Use icon selection shadow");
  icon_selection_shadow_chkbox->SetValue(
      g_settings.getBoolean(Config::USE_GUI_SELECTION_SHADOW));
  sizer->Add(icon_selection_shadow_chkbox,
             wxSizerFlags().Border(wxLEFT | wxTOP, FROM_DIP(this, 5)));
  SetWindowToolTip(icon_selection_shadow_chkbox,
                   "When this option is checked, selected items in the palette "
                   "menu will be shaded.");

  use_memcached_chkbox =
      newd wxCheckBox(this, wxID_ANY, "Use memcached sprites");
  use_memcached_chkbox->SetValue(
      g_settings.getBoolean(Config::USE_MEMCACHED_SPRITES));
  sizer->Add(use_memcached_chkbox,
             wxSizerFlags().Border(wxLEFT | wxTOP, FROM_DIP(this, 5)));
  SetWindowToolTip(
      use_memcached_chkbox,
      "When this is checked, sprites will be loaded into memory at startup and "
      "unpacked at runtime. This is faster but consumes more memory.\nIf it is "
      "not checked, the editor will use less memory but there will be a "
      "performance decrease due to reading sprites from the disk.");

  anti_aliasing_chkbox =
      newd wxCheckBox(this, wxID_ANY, "Enable Anti-Aliasing");
  anti_aliasing_chkbox->SetValue(g_settings.getBoolean(Config::ANTI_ALIASING));
  sizer->Add(anti_aliasing_chkbox,
             wxSizerFlags().Border(wxLEFT | wxTOP, FROM_DIP(this, 5)));
  SetWindowToolTip(anti_aliasing_chkbox,
                   "Smoothens the map rendering using linear interpolation.");
  ...
  auto effect_names = PostProcessManager::GetAvailableEffectNames();
  ...
  SetWindowToolTip(screen_shader_choice, tmp,
                   "Apply a post-processing shader to the map view.");
  ...
  subsizer->Add(
      tmp = newd wxStaticText(this, wxID_ANY, "Icon background color: "),
      wxSizerFlags());
  subsizer->Add(icon_background_choice, wxSizerFlags());
  SetWindowToolTip(
      icon_background_choice, tmp,
      "This will change the background color on icons in all windows.");
  ...
  SetWindowToolTip(
      ...
      "The color of the main cursor on the map (while in drawing mode).");
  ...
  // Screenshot dir
  subsizer->Add(tmp =
                    newd wxStaticText(this, wxID_ANY, "Screenshot directory: "),
                wxSizerFlags());
  ...
  SetWindowToolTip(
      ...
      "Screenshot taken in the editor will be saved to this directory.");
  ...
  subsizer->Add(tmp = newd wxStaticText(this, wxID_ANY, "Screenshot format: "),
                wxSizerFlags());
  subsizer->Add(screenshot_format_choice, wxSizerFlags());
  SetWindowToolTip(screenshot_format_choice, tmp,
                   "This will affect the screenshot format used by the "
                   "editor.\nTo take a screenshot, press F11.");

  sizer->Add(subsizer,
             wxSizerFlags(1).Expand().Border(wxALL, FROM_DIP(this, 5)));
  ...
  fps_sizer->Add(
      tmp = newd wxStaticText(this, wxID_ANY, "FPS Limit (0 = unlimited): "),
      wxSizerFlags());
  fps_limit_spin = newd wxSpinCtrl(
      this, wxID_ANY, i2ws(g_settings.getInteger(Config::FRAME_RATE_LIMIT)),
      wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 0, 300);
  fps_sizer->Add(fps_limit_spin, wxSizerFlags());
  SetWindowToolTip(
      fps_limit_spin, tmp,
      "Limits the frame rate to save power. Set to 0 for unlimited.");
  ...
  sizer->Add(show_fps_chkbox,
             wxSizerFlags().Border(wxLEFT | wxTOP, FROM_DIP(this, 5)));
  SetWindowToolTip(show_fps_chkbox,
                   "Displays the current frame rate in the status bar.");
...
void GraphicsPage::Apply() {
  ...
  g_settings.setInteger(Config::USE_GUI_SELECTION_SHADOW,
                        icon_selection_shadow_chkbox->GetValue());
  if (g_settings.getBoolean(Config::USE_MEMCACHED_SPRITES) !=
      use_memcached_chkbox->GetValue()) {
    must_restart = true;
  }
  g_settings.setInteger(Config::USE_MEMCACHED_SPRITES_TO_SAVE,
                        use_memcached_chkbox->GetValue());

  g_settings.setInteger(Config::ANTI_ALIASING,
                        anti_aliasing_chkbox->GetValue());
  g_settings.setString(Config::SCREEN_SHADER,
                       nstr(screen_shader_choice->GetStringSelection()));
  ...
  // Screenshots
  g_settings.setString(Config::SCREENSHOT_DIRECTORY,
                       nstr(screenshot_directory_picker->GetPath()));
  ...
  g_settings.setInteger(Config::HIDE_ITEMS_WHEN_ZOOMED,
                        hide_items_when_zoomed_chkbox->GetValue());
  ...
  if (must_restart) {
    wxMessageBox(
        ...
        "Restart Required", wxOK | wxICON_INFORMATION);
  }
}
