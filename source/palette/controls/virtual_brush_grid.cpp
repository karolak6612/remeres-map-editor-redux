...
VirtualBrushGrid::VirtualBrushGrid(wxWindow *parent,
                                   const TilesetCategory *_tileset,
                                   RenderSize rsz)
    : NanoVGCanvas(parent, wxID_ANY, wxVSCROLL | wxWANTS_CHARS),
      BrushBoxInterface(_tileset), icon_size(rsz), selected_index(-1),
      hover_index(-1), columns(1), item_size(0), padding(4), m_animTimer(this) {
...
void VirtualBrushGrid::OnNanoVGPaint(NVGcontext *vg, int width, int height) {
  ...
  int rowHeight = (display_mode == DisplayMode::List) ? LIST_ROW_HEIGHT
                                                      : (item_size + padding);
...
void VirtualBrushGrid::DrawBrushItem(NVGcontext *vg, int i,
                                     const wxRect &rect) {
  ...
  if (i == selected_index) {
    ...
    NVGpaint shadowPaint =
        nvgBoxGradient(vg, x, y, w, h, 4.0f, 10.0f, nvgRGBA(100, 150, 255, 128),
                       nvgRGBA(0, 0, 0, 0));
    nvgBeginPath(vg);
    ...
    NVGpaint shadowPaint = nvgBoxGradient(
        vg, x, y + 2, w, h, 4.0f, shadowBlur,
        nvgRGBA(0, 0, 0, static_cast<int>(shadowAlpha)), nvgRGBA(0, 0, 0, 0));
    nvgBeginPath(vg);
    nvgRect(vg, x - 10, y - 10, w + 20, h + 20);
    nvgRoundedRect(vg, x, y, w, h, 4.0f);
    nvgPathWinding(vg, NVG_HOLE);
  ...
  // Draw brush sprite
  Brush *brush =
      (i < static_cast<int>(tileset->size())) ? tileset->brushlist[i] : nullptr;
  if (brush) {
    Sprite *spr = brush->getSprite();
    if (!spr) {
      spr = g_gui.sprite_database.getSprite(brush->getLookID());
    }
    ...
    if (tex > 0) {
      int iconSize = (display_mode == DisplayMode::List)
                         ? GRID_ITEM_SIZE_BASE
                         : (item_size - 2 * ICON_OFFSET);
      int iconX = rect.x + ICON_OFFSET;
      int iconY = rect.y + ICON_OFFSET;

      NVGpaint imgPaint = nvgImagePattern(
          vg, static_cast<float>(iconX), static_cast<float>(iconY),
          static_cast<float>(iconSize), static_cast<float>(iconSize), 0.0f, tex,
          1.0f);

      nvgBeginPath(vg);
      nvgRoundedRect(vg, static_cast<float>(iconX), static_cast<float>(iconY),
    ...
    if (display_mode == DisplayMode::List) {
      ...
      nvgText(vg, rect.x + 40, rect.y + rect.height / 2.0f, it->second.c_str(),
              nullptr);
    }
  }
}
...
wxRect VirtualBrushGrid::GetItemRect(int index) const {
  if (display_mode == DisplayMode::List) {
    ...
    return wxRect(padding, padding + index * LIST_ROW_HEIGHT, width,
                  LIST_ROW_HEIGHT);
  } else {
    int row = index / columns;
    int col = index % columns;

    return wxRect(padding + col * (item_size + padding),
                  padding + row * (item_size + padding), item_size, item_size);
  }
}
...
Brush *VirtualBrushGrid::GetSelectedBrush() const {
  if (selected_index >= 0 &&
      selected_index < static_cast<int>(tileset->size())) {
