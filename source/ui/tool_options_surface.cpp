...
void ToolOptionsSurface::DoSetSizeHints(int minW, int minH, int maxW, int maxH,
                                        int incW, int incH) {
...
void ToolOptionsSurface::RebuildLayout() {
  ...
  const int icon_sz =
      FromDIP(ICON_SIZE_LG); // Defaulting to large for "Pro" look
  const int gap = FromDIP(GRID_GAP);

  // 1. Tools
  bool has_tools =
      (current_type == TILESET_TERRAIN || current_type == TILESET_COLLECTION);
  ...
  // 3. Thickness Slider
  bool show_thickness =
      (current_type == TILESET_COLLECTION || current_type == TILESET_DOODAD);
  if (show_thickness) {
...
void ToolOptionsSurface::OnPaint(wxPaintEvent &evt) {
  ...
  // 2. Draw Sliders
  if (interactables.size_slider_rect.height > 0) {
    DrawSlider(dc, interactables.size_slider_rect, "Size", current_size,
               MIN_BRUSH_SIZE, MAX_BRUSH_SIZE, true);
  }
  if (interactables.thickness_slider_rect.height > 0) {
    // Doodad thickness logic usually 0-100% or similar? No, old code was just
    // "Thickness".
    DrawSlider(dc, interactables.thickness_slider_rect, "Thickness",
               current_thickness, MIN_BRUSH_THICKNESS, MAX_BRUSH_THICKNESS,
               true);
  }

  // 3. Checkboxes
  if (interactables.preview_check_rect.height > 0) {
    DrawCheckbox(dc, interactables.preview_check_rect, "Preview Border",
                 show_preview, interactables.hover_preview);
  }
  if (interactables.lock_check_rect.height > 0) {
    DrawCheckbox(dc, interactables.lock_check_rect, "Lock Doors (Shift)",
                 lock_doors, interactables.hover_lock);
  }

  // Debug Focus
  // if (HasFocus()) { wxPen p(*wxRED, 1); dc.SetPen(p);
  // dc.SetBrush(*wxTRANSPARENT_BRUSH); dc.DrawRectangle(GetClientSize()); }
}
...
void ToolOptionsSurface::DrawToolIcon(wxDC &dc, const ToolRect &tr) {
  ...
  if (tr.brush) {
    ...
    if (!s && tr.brush->getLookID() != 0) {
      s = g_gui.sprite_database.getSprite(tr.brush->getLookID());
    }

    if (s) {
      // Center the sprite in the rect
      // Assuming 32x32 icon size for now, which matches SPRITE_SIZE_32x32
      // (32x32)
...
void ToolOptionsSurface::DrawSlider(wxDC &dc, const wxRect &rect,
                                    const wxString &label, int value, int min,
                                    int max, bool active) {
  // Label
  dc.SetFont(GetFont());
  dc.SetTextForeground(Theme::Get(Theme::Role::Text));
  ...
  dc.DrawText(label, rect.GetLeft(),
              rect.GetTop() + (rect.height - extent.y) / 2);

  // Track
  int label_w = FromDIP(SLIDER_LABEL_WIDTH);
  int track_x = rect.GetLeft() + label_w;
  int track_w =
      rect.width - label_w - FromDIP(SLIDER_TEXT_MARGIN); // Room for value text
  ...
  if (value > min && max > min) {
    float pct = std::clamp(static_cast<float>(value - min) /
    ...
    dc.DrawCircle(track_x + fill_w, track_y + track_h / 2,
                  FromDIP(SLIDER_THUMB_RADIUS));
  }

  // Value Text
  wxString val_str = std::format("{}", value);
  dc.DrawText(val_str, track_x + track_w + FromDIP(SLIDER_VALUE_MARGIN),
              rect.GetTop() + (rect.height - extent.y) / 2);
}

void ToolOptionsSurface::DrawCheckbox(wxDC &dc, const wxRect &rect,
                                      const wxString &label, bool value,
                                      bool hover) {
  // Box
  ...
  dc.SetBrush(value ? wxBrush(Theme::Get(Theme::Role::Accent))
                    : wxBrush(Theme::Get(Theme::Role::Background)));
  dc.DrawRectangle(box);

  // Checkmark (simple)
  if (value) {
    dc.SetPen(*wxWHITE_PEN);
    dc.DrawLine(box.GetLeft() + 3, box.GetTop() + 7, box.GetLeft() + 6,
                box.GetTop() + 10);
    dc.DrawLine(box.GetLeft() + 6, box.GetTop() + 10, box.GetRight() - 3,
                box.GetTop() + 4);
  }
...
void ToolOptionsSurface::OnMouse(wxMouseEvent &evt) {
  ...
  if (evt.LeftDown()) {
    if (interactables.size_slider_rect.Contains(m_hoverPos)) {
      ...
      CaptureMouse();
      int val = CalculateSliderValue(interactables.size_slider_rect,
      ...
      CaptureMouse();
      int val = CalculateSliderValue(interactables.thickness_slider_rect,
      ...
      g_gui.SetStatusText(
          std::format("Selected Tool: {}", hover_brush->getName()));
    }
  ...
  if (interactables.size_slider_rect.Contains(m_hoverPos) ||
      interactables.thickness_slider_rect.Contains(m_hoverPos)) {
    hand_cursor = true;
  }
  ...
  if (prev_hover != hover_brush ||
      prev_preview != interactables.hover_preview ||
      prev_lock != interactables.hover_lock) {
    Refresh();
  }
}
...
int ToolOptionsSurface::CalculateSliderValue(const wxRect &sliderRect, int min,
                                             int max) const {
  int label_w = FromDIP(SLIDER_LABEL_WIDTH);
  ...
  float pct = std::clamp(
      static_cast<float>(rel_x) / static_cast<float>(track_w), 0.0f, 1.0f);
