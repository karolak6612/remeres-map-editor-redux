...
void ToolOptionsSurface::OnNanoVGPaint(NVGcontext* vg, int width, int height) {
	// Background (NanoVGCanvas might handle clear, but we want custom surface color)
...
void ToolOptionsSurface::DrawToolIcon(NVGcontext* vg, const ToolRect& tr) {
	...
	if (is_selected) {
		...
		nvgBeginPath(vg);
		nvgRoundedRect(vg, x, y, w, h, 4.0f);
		nvgFillColor(vg, nvgRGBA(60, 80, 100, 255));
		nvgFill(vg);
	} else if (is_hover) {
		nvgBeginPath(vg);
		nvgRoundedRect(vg, x, y, w, h, 4.0f);
	...
	if (tr.brush) {
		...
		if (s) {
			...
			if (tex > 0) {
				// Center sprite
				float spriteSize = 32.0f; // Standard
				float sx = x + (w - spriteSize) / 2.0f;
				float sy = y + (h - spriteSize) / 2.0f;

				NVGpaint imgPaint = nvgImagePattern(vg, sx, sy, spriteSize, spriteSize, 0.0f, tex, 1.0f);
				nvgBeginPath(vg);
				nvgRect(vg, sx, sy, spriteSize, spriteSize);
...
void ToolOptionsSurface::DrawSlider(NVGcontext* vg, const wxRect& rect, const wxString& label, int value, int min, int max, bool active) {
	...
	// Track Background
	nvgBeginPath(vg);
	nvgRoundedRect(vg, track_x, track_y, track_w, track_h, track_h / 2);
	...
	if (value > min && max > min) {
		...
		nvgBeginPath(vg);
		nvgRoundedRect(vg, track_x, track_y, fill_w, track_h, track_h / 2);
		nvgFillColor(vg, nvgRGBA(100, 180, 255, 255)); // Accent color
		nvgFill(vg);
		...
		NVGpaint glow = nvgRadialGradient(vg, thumbX, thumbY, 0, thumbR + 4, nvgRGBA(100, 180, 255, 128), nvgRGBA(0, 0, 0, 0));
		nvgBeginPath(vg);
		nvgCircle(vg, thumbX, thumbY, thumbR + 4);
		nvgFillPaint(vg, glow);
		nvgFill(vg);

		nvgBeginPath(vg);
		nvgCircle(vg, thumbX, thumbY, thumbR);
		nvgFillColor(vg, nvgRGBA(255, 255, 255, 255));
		nvgFill(vg);
	}
	...
	nvgFillColor(vg, nvgRGBA(255, 255, 255, 255));
	nvgText(vg, x + w, y + h / 2, val_str.ToStdString().c_str(), nullptr);
}
...
void ToolOptionsSurface::DrawCheckbox(NVGcontext* vg, const wxRect& rect, const wxString& label, bool value, bool hover) {
	...
	float box_y = rect.y + (rect.height - box_sz) / 2;
	float box_x = rect.x;
	...
	if (value) {
		nvgFillColor(vg, nvgRGBA(100, 180, 255, 255)); // Accent
	} else {
		nvgFillColor(vg, nvgRGBA(40, 40, 40, 255)); // Background
	}
	nvgFill(vg);
	...
	nvgFillColor(vg, nvgRGBA(220, 220, 220, 255));
	nvgTextAlign(vg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
	nvgText(vg, box_x + box_sz + 8, box_y + box_sz / 2, label.ToStdString().c_str(), nullptr);
}
