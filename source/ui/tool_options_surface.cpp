...
void ToolOptionsSurface::RebuildLayout() {
	...
	if (has_tools) { // Assume terrain
		...
		y += FromDIP(24); // Add height for last item
	}
...
void ToolOptionsSurface::OnNanoVGPaint(NVGcontext* vg, int width, int height) {
	// Background
	// NanoVGCanvas handles clearing, but we can draw a background if needed.
	// We'll trust the base class clear or draw a full rect if we want a specific color.
...
void ToolOptionsSurface::DrawToolIcon(NVGcontext* vg, const ToolRect& tr) {
	...
	if (tr.brush) {
		...
		if (s) {
			...
			if (tex > 0) {
				// Center sprite (assume 32x32 target)
			...
			nvgText(vg, x + w * 0.5f, y + h * 0.5f, tr.tooltip.Left(1).c_str().AsChar(), nullptr);
		}
	}
}
...
void ToolOptionsSurface::DrawSlider(NVGcontext* vg, const wxRect& rect, const wxString& label, int value, int min, int max, bool active) {
	...
	nvgFillColor(vg, NvgUtils::ToNvColor(Theme::Get(Theme::Role::Text)));
	nvgText(vg, x, y + h * 0.5f, label.c_str().AsChar(), nullptr);
	...
	if (value > min && max > min) {
		...
		// Thumb
		float thumb_r = FromDIP(SLIDER_THUMB_RADIUS);
		nvgBeginPath(vg);
		nvgCircle(vg, track_x + fill_w, track_y + track_h * 0.5f, thumb_r);
		nvgFillColor(vg, active ? nvgRGBA(255, 255, 255, 255) : NvgUtils::ToNvColor(Theme::Get(Theme::Role::Text)));
		nvgFill(vg);
	...
	nvgText(vg, x + w, y + h * 0.5f, val_str.c_str(), nullptr);
}
...
void ToolOptionsSurface::DrawCheckbox(NVGcontext* vg, const wxRect& rect, const wxString& label, bool value, bool hover) {
	...
	if (value) {
		nvgBeginPath(vg);
		nvgMoveTo(vg, x + box_sz * 0.2f, box_y + box_sz * 0.5f);
		nvgLineTo(vg, x + box_sz * 0.45f, box_y + box_sz * 0.75f);
		nvgLineTo(vg, x + box_sz * 0.8f, box_y + box_sz * 0.25f);
	...
	nvgTextAlign(vg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
	nvgFillColor(vg, NvgUtils::ToNvColor(Theme::Get(Theme::Role::Text)));
	nvgText(vg, x + box_sz + FromDIP(8), box_y + box_sz * 0.5f, label.c_str().AsChar(), nullptr);
}

void ToolOptionsSurface::OnMouse(wxMouseEvent& evt) {
	// Adjust mouse position by scroll position
	wxPoint pos = evt.GetPosition();
	pos.y += GetScrollPosition();
	m_hoverPos = pos;
