...
public:
   ...
   */
  void SavePerspective() { g_layout.SavePerspective(); }

  /**
   * Loads the stored perspective from the configuration file
   */
  void LoadPerspective() { g_layout.LoadPerspective(); }
  ...
  void FinishWelcomeDialog() { g_welcome.FinishWelcomeDialog(); }
  bool IsWelcomeDialogShown() { return g_welcome.IsWelcomeDialogShown(); }

  /**
   * Destroys (hides) the current loading bar.
   */

  void DestroyLoadBar() { g_loading.DestroyLoadBar(); }
...
public:
  void SetTitle(wxString newtitle) { g_status.SetTitle(newtitle); }
  void UpdateTitle() { g_status.UpdateTitle(); }
  void UpdateMenus();
  void ShowToolbar(ToolBarID id, bool show);
  void SetStatusText(wxString text) { g_status.SetStatusText(text); }
  ...
  // Search Results
  SearchResultWindow *ShowSearchWindow() { return g_search.ShowSearchWindow(); }
  void HideSearchWindow() { g_search.HideSearchWindow(); }

  // Minimap
  void CreateMinimap() { g_minimap.Create(); }
  void HideMinimap() { g_minimap.Hide(); }
  void DestroyMinimap() { g_minimap.Destroy(); }
  void UpdateMinimap(bool immediate = false) { g_minimap.Update(immediate); }
  bool IsMinimapVisible() const { return g_minimap.IsVisible(); }
  ...
  void SelectBrush();
  // Updates the palette AND selects the brush, second parameter is first
  // palette to look in Returns true if the brush was found and selected
...
public:
  ...
  ~ScopedLoadingBar() { g_loading.DestroyLoadBar(); }
