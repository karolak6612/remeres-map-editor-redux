...
void MapFlagsPanel::SetTile(Tile* tile, Map* map) {
	...
	if (tile) {
		...
		chk_nopvp->SetValue((tile->getMapFlags() & TileMapFlag::NOPVP) != TileMapFlag::NONE);
		chk_nologout->SetValue((tile->getMapFlags() & TileMapFlag::NOLOGOUT) != TileMapFlag::NONE);
		chk_pvpzone->SetValue((tile->getMapFlags() & TileMapFlag::PVPZONE) != TileMapFlag::NONE);
...
void MapFlagsPanel::OnToggleFlag(wxCommandEvent& event) {
	...
	if (chk_nopvp->GetValue()) {
		new_tile->setMapFlags(TileMapFlag::NOPVP);
	} else {
		new_tile->unsetMapFlags(TileMapFlag::NOPVP);
	}

	if (chk_nologout->GetValue()) {
		new_tile->setMapFlags(TileMapFlag::NOLOGOUT);
	} else {
		new_tile->unsetMapFlags(TileMapFlag::NOLOGOUT);
	}

	if (chk_pvpzone->GetValue()) {
		new_tile->setMapFlags(TileMapFlag::PVPZONE);
	} else {
		new_tile->unsetMapFlags(TileMapFlag::PVPZONE);
	}
