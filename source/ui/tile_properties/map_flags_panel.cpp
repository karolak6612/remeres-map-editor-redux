...
void MapFlagsPanel::SetTile(Tile* tile, Map* map) {
	...
	if (tile) {
		...
		chk_nopvp->SetValue(tile->getMapFlags() & static_cast<uint16_t>(TileMapFlag::NOPVP));
		chk_nologout->SetValue(tile->getMapFlags() & static_cast<uint16_t>(TileMapFlag::NOLOGOUT));
		chk_pvpzone->SetValue(tile->getMapFlags() & static_cast<uint16_t>(TileMapFlag::PVPZONE));
...
void MapFlagsPanel::OnToggleFlag(wxCommandEvent& event) {
	...
	if (chk_nopvp->GetValue()) {
		new_tile->setMapFlags(static_cast<uint16_t>(TileMapFlag::NOPVP));
	} else {
		new_tile->unsetMapFlags(static_cast<uint16_t>(TileMapFlag::NOPVP));
	}

	if (chk_nologout->GetValue()) {
		new_tile->setMapFlags(static_cast<uint16_t>(TileMapFlag::NOLOGOUT));
	} else {
		new_tile->unsetMapFlags(static_cast<uint16_t>(TileMapFlag::NOLOGOUT));
	}

	if (chk_pvpzone->GetValue()) {
		new_tile->setMapFlags(static_cast<uint16_t>(TileMapFlag::PVPZONE));
	} else {
		new_tile->unsetMapFlags(static_cast<uint16_t>(TileMapFlag::PVPZONE));
	}
