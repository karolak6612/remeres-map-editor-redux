...
void MapFlagsPanel::SetTile(Tile* tile, Map* map) {
	...
	if (tile) {
		...
		chk_nopvp->SetValue(static_cast<uint16_t>(tile->getMapFlags() & TileMapFlags::NOPVP));
		chk_nologout->SetValue(static_cast<uint16_t>(tile->getMapFlags() & TileMapFlags::NOLOGOUT));
		chk_pvpzone->SetValue(static_cast<uint16_t>(tile->getMapFlags() & TileMapFlags::PVPZONE));
