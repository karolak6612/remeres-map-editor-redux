...
SpawnCreaturePanel::SpawnCreaturePanel(wxWindow *parent)
  ...
  wxStaticBoxSizer *main_sizer =
      newd wxStaticBoxSizer(wxVERTICAL, this, "Spawn & Creature");

  // Spawn Node
  wxBoxSizer *spawn_sizer = newd wxBoxSizer(wxHORIZONTAL);
  spawn_bitmap = newd wxStaticBitmap(
      main_sizer->GetStaticBox(), wxID_ANY, wxNullBitmap, wxDefaultPosition,
      FROM_DIP(this, wxSize(32, 32)), wxBORDER_SIMPLE);
  spawn_bitmap->Hide();
  spawn_text =
      newd wxStaticText(main_sizer->GetStaticBox(), wxID_ANY, "No Spawn");
  ...
  creature_bitmap = newd wxStaticBitmap(
      main_sizer->GetStaticBox(), wxID_ANY, wxNullBitmap, wxDefaultPosition,
      FROM_DIP(this, wxSize(32, 32)), wxBORDER_SIMPLE);
  creature_bitmap->Hide();
  creature_text =
      newd wxStaticText(main_sizer->GetStaticBox(), wxID_ANY, "No Creature");
  creature_sizer->Add(creature_bitmap,
                      wxSizerFlags(0).Center().Border(wxALL, 2));
  ...
  creature_bitmap->Bind(wxEVT_LEFT_UP, &SpawnCreaturePanel::OnCreatureClick,
                        this);
  creature_text->Bind(wxEVT_LEFT_UP, &SpawnCreaturePanel::OnCreatureClick,
                      this);
}
...
void SpawnCreaturePanel::SetTile(Tile *tile) {
  if (tile && tile->spawn) {
    spawn_text->SetLabelText(
        wxString::Format("Spawn\nR=%d", tile->spawn->getSize()));
    spawn_bitmap->SetBitmap(
        IMAGE_MANAGER.GetBitmap(ICON_FIRE, FROM_DIP(this, wxSize(32, 32))));
    spawn_bitmap->Show();
  } else {
  ...
  if (tile && tile->creature) {
    ...
    GameSprite *spr = g_gui.sprite_database.getCreatureSprite(
        tile->creature->getLookType().lookType);
    if (spr) {
      wxBitmap bmp = SpriteIconGenerator::Generate(
          spr, SPRITE_SIZE_32x32, tile->creature->getLookType(), false, SOUTH);
      creature_bitmap->SetBitmap(bmp);
    } else {
      creature_bitmap->SetBitmap(
          IMAGE_MANAGER.GetBitmap(ICON_DRAGON, FROM_DIP(this, wxSize(32, 32))));
    }
