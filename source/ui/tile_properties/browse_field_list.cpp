...
void TilePropertiesListBox::UpdateItems() {
  ...
  if (current_tile) {
    ...
    for (auto it = current_tile->items.begin(); it != current_tile->items.end();
         ++it) {
...
void TilePropertiesListBox::OnDrawItem(NVGcontext *vg, const wxRect &rect,
                                       size_t n) {
  if (n >= items.size()) {
    return;
  }
...
int TilePropertiesListBox::OnMeasureItem(size_t n) const { return FromDIP(32); }
...
void BrowseFieldList::OnClickUp(wxCommandEvent &event) {
  ...
  if (editor && current_tile) {
    ...
    std::unique_ptr<Tile> new_tile =
        TileOperations::deepCopy(current_tile, editor->map);
    std::swap(new_tile->items[index_in_items],
              new_tile->items[index_in_items - 1]);
    new_tile->items[index_in_items - 1]->select();

    std::unique_ptr<Action> action =
        editor->actionQueue->createAction(ACTION_CHANGE_PROPERTIES);
...
void BrowseFieldList::OnClickDown(wxCommandEvent &event) {
  ...
  int index_in_items = selection - (current_tile->ground ? 1 : 0);
  if (index_in_items < 0 ||
      index_in_items >= (int)current_tile->items.size() - 1) {
    return;
  }
