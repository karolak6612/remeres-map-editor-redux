...
EditTownsDialog::EditTownsDialog(wxWindow* parent, Editor& editor) :
	...
	wxIconBundle icons;
	wxIcon icon16;
	icon16.CopyFromBitmap(IMAGE_MANAGER.GetBitmap(ICON_CITY, FromDIP(wxSize(16, 16))));
	icons.AddIcon(icon16);
	wxIcon icon32;
	icon32.CopyFromBitmap(IMAGE_MANAGER.GetBitmap(ICON_CITY, FromDIP(wxSize(32, 32))));
