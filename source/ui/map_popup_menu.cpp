	...
	if (anything_selected) {
		if (editor.selection.size() == 1) {
			...
			if (topSelectedItem || topCreature || topItem) {
				...
				AppendSeparator();
				Append(MAP_POPUP_MENU_PROPERTIES, "&Properties\tCTRL+P", "Properties for the current object")->SetBitmap(IMAGE_MANAGER.GetBitmapBundle(ICON_GEAR));
			} else {
			...
			wxMenuItem* browseTile = Append(MAP_POPUP_MENU_BROWSE_TILE, "Browse Field\tCTRL+B", "Navigate from tile items");
			browseTile->SetBitmap(IMAGE_MANAGER.GetBitmapBundle(ICON_SEARCH));
			browseTile->Enable(anything_selected);

			wxMenuItem* tileProps = Append(MAP_POPUP_MENU_TILE_PROPERTIES, "Tile Properties\tCTRL+I", "Show tile properties panel");
