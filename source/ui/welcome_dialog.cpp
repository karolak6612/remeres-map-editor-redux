...
public:
	WelcomePanel(WelcomeDialog* parent, const wxBitmap& logo, const std::vector<wxString>& recentFiles) :
		...
		sideSizer->Add(logoCtrl, wxSizerFlags(0).Center().Border(wxTOP | wxBOTTOM, FromDIP(20)));
		...
		sideSizer->Add(title, wxSizerFlags(0).Center().Border(wxTOP, FromDIP(15)));
		...
		sideSizer->Add(version, wxSizerFlags(0).Center().Border(wxBOTTOM, FromDIP(25)));
		...
		contentSizer->Add(footer, wxSizerFlags(0).Expand());

		content->SetSizer(contentSizer);

		// combine
		mainSizer->Add(sidebar, wxSizerFlags(0).Expand());
