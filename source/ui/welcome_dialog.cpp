...
void WelcomeDialog::BuildInterface(const wxString& title_text, const wxString& version_text, const wxBitmap& rme_logo) {
	...
	preferences_button->SetToolTip("Preferences (Esc)");
	...
	browse_map_button->SetToolTip("Open an existing map (Ctrl+O)");
	...
	exit_button->SetToolTip("Exit the editor (Alt+F4)");
	...
	m_force_load_checkbox->SetToolTip("Force load the map even if versions don't match");
