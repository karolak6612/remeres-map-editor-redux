...
SearchResultWindow::SearchResultWindow(wxWindow* parent) :
	...
	copyBtn->Bind(wxEVT_BUTTON, &SearchResultWindow::OnClickCopy, this);
...
void SearchResultWindow::OnClickCopy(wxCommandEvent& WXUNUSED(event)) {
	if (result_list->GetCount() == 0) {
		return;
	}

	wxString data;
	data << "Generated by Remere's Map Editor " << __RME_VERSION__ << "\n";
	data << "\n=============================================\n\n";
	for (size_t i = 0; i < result_list->GetCount(); ++i) {
		data << result_list->GetString(i) << "\n";
	}
