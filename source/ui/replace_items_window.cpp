...
ReplaceItemsButton::ReplaceItemsButton(wxWindow *parent)
    : DCButton(parent, wxID_ANY, wxDefaultPosition, DC_BTN_TOGGLE,
               RENDER_SIZE_32x32, 0),
      m_id(0) {
  ////
}
...
ReplaceItemsListBox::ReplaceItemsListBox(wxWindow *parent)
    : NanoVGListBox(parent, wxID_ANY, wxLB_SINGLE), m_arrow_image(0),
      m_flag_image(0) {}

bool ReplaceItemsListBox::AddItem(const ReplacingItem &item) {
  if (item.replaceId == 0 || item.withId == 0 ||
      item.replaceId == item.withId) {
    return false;
  }
...
void ReplaceItemsListBox::MarkAsComplete(const ReplacingItem &item,
                                         uint32_t total) {
...
void ReplaceItemsListBox::OnDrawItem(NVGcontext *vg, const wxRect &rect,
                                     size_t index) {
  ...
  if (m_flag_image == 0) {
    m_flag_image =
        IMAGE_MANAGER.GetNanoVGImage(vg, IMAGE_PROTECTION_ZONE_SMALL);
    if (m_flag_image == 0) {
      m_flag_image = -1;
    }
  }
  ...
  Sprite *sprite1 = g_gui.sprite_database.getSprite(type1.clientID);

  const ItemType &type2 = g_items.getItemType(item.withId);
  Sprite *sprite2 = g_gui.sprite_database.getSprite(type2.clientID);
  ...
  if (sprite1 && sprite2) {
    ...
    if (tex1 > 0) {
      NVGpaint imgPaint =
          nvgImagePattern(vg, x + 4, y + 4, 32, 32, 0, tex1, 1.0f);
      nvgBeginPath(vg);
    ...
    if (m_arrow_image > 0) {
      NVGpaint arrowPaint =
          nvgImagePattern(vg, x + 38, y + 10, 16, 16, 0, m_arrow_image, 1.0f);
      nvgBeginPath(vg);
    ...
    if (tex2 > 0) {
      NVGpaint imgPaint =
          nvgImagePattern(vg, x + 56, y + 4, 32, 32, 0, tex2, 1.0f);
      nvgBeginPath(vg);
    ...
    if (IsSelected(index)) {
      ...
      nvgFillColor(vg, nvgRGBA(textColour.Red(), textColour.Green(),
                               textColour.Blue(), 255));
    } else {
      wxColour textColour =
    ...
    nvgTextAlign(vg, NVG_ALIGN_LEFT |
                         NVG_ALIGN_MIDDLE); // Text alignment inside rect? No,
                                            // absolute coords.
    // nvgText expects x, y for baseline or according to align.
    // NVG_ALIGN_MIDDLE means y is vertical center.
    nvgText(vg, x + 104, y + 20,
            std::format("Replace: {} With: {}", item.replaceId, item.withId)
                .c_str(),
            nullptr);
    ...
    if (item.complete) {
      ...
      if (m_flag_image > 0) {
        NVGpaint flagPaint = nvgImagePattern(vg, rightX + 70, y + 10, 16, 16, 0,
                                             m_flag_image, 1.0f);
        nvgBeginPath(vg);
      ...
      nvgText(vg, rightX, y + 20, std::format("Total: {}", item.total).c_str(),
              nullptr);
    }
  }
}
...
ReplaceItemsDialog::ReplaceItemsDialog(wxWindow *parent, bool selectionOnly)
    : wxDialog(parent, wxID_ANY,
               (selectionOnly ? "Replace Items on Selection" : "Replace Items"),
  ...
  replace_button->Bind(wxEVT_LEFT_DOWN,
                       &ReplaceItemsDialog::OnReplaceItemClicked, this);
  with_button->Bind(wxEVT_LEFT_DOWN, &ReplaceItemsDialog::OnWithItemClicked,
                    this);
  add_button->Bind(wxEVT_BUTTON, &ReplaceItemsDialog::OnAddButtonClicked, this);
  remove_button->Bind(wxEVT_BUTTON, &ReplaceItemsDialog::OnRemoveButtonClicked,
                      this);
  execute_button->Bind(wxEVT_BUTTON,
                       &ReplaceItemsDialog::OnExecuteButtonClicked, this);
  close_button->Bind(wxEVT_BUTTON, &ReplaceItemsDialog::OnCancelButtonClicked,
                     this);
...
void ReplaceItemsDialog::UpdateWidgets() {
  ...
  remove_button->Enable(list->GetCount() != 0 &&
                        list->GetSelection() != wxNOT_FOUND);
...
void ReplaceItemsDialog::OnRemoveButtonClicked(
    wxCommandEvent &WXUNUSED(event)) {
  list->RemoveSelected();
  UpdateWidgets();
}

void ReplaceItemsDialog::OnExecuteButtonClicked(
    wxCommandEvent &WXUNUSED(event)) {
  ...
  for (const ReplacingItem &info : items) {
    ItemFinder finder(info.replaceId,
    ...
    if (!result.empty()) {
      std::unique_ptr<Action> action =
          editor->actionQueue->createAction(ACTION_REPLACE_ITEMS);
    ...
    const int value =
        static_cast<int>((static_cast<double>(done) / items.size()) * 100);
...
void ReplaceItemsDialog::OnCancelButtonClicked(
    wxCommandEvent &WXUNUSED(event)) {
  Close();
}
