...
VisualSimilarityService::~VisualSimilarityService() { m_timer.Stop(); }
...
static bool IsFullyOpaqueRGBA(std::span<const uint8_t> rgba,
...
static uint64_t CalculateAHashRGBA(std::span<const uint8_t> rgba, int w,
                                   int h) {
  ...
  for (int y = 0; y < 8; ++y) {
    for (int x = 0; x < 8; ++x) {
      ...
      uint8_t avg =
          (uint8_t)((pixel_count > 0) ? (block_sum / pixel_count) : 0);
      gray_8x8[y * 8 + x] = avg;
...
static std::vector<bool> ExtractBinaryMaskRGBA(std::span<const uint8_t> rgba,
                                               int &outTruePixels) {
...
static std::vector<float> CalculateHistogramRGBA(std::span<const uint8_t> rgba,
                                                 uint8_t threshold = 10) {
  const int BINS = 8;
...
static float CompareHistograms(std::span<const float> h1,
                               std::span<const float> h2) {
...
static std::vector<bool> ResizeMask(const std::vector<bool> &src, int srcW,
...
VisualSimilarityService::VisualItemData
VisualSimilarityService::CalculateData(uint16_t itemId) {
  VisualItemData data;
  data.id = itemId;
  data.isOpaque = false;
  data.aHash = 0;
  ...
  Sprite *sprite = g_gui.sprite_database.getSprite(it.clientID);
  if (!sprite) {
    return data;
  }
  ...
  data.height = h;
  data.isOpaque =
      IsFullyOpaqueRGBA({composite.get(), static_cast<size_t>(w * h * 4)});

  // Store both for robustness
  data.aHash = CalculateAHashRGBA(
      {composite.get(), static_cast<size_t>(w * h * 4)}, w, h);
  data.binaryMask = ExtractBinaryMaskRGBA(
      {composite.get(), static_cast<size_t>(w * h * 4)}, data.truePixels);
  data.histogram =
      CalculateHistogramRGBA({composite.get(), static_cast<size_t>(w * h * 4)});
...
std::vector<uint16_t> VisualSimilarityService::FindSimilar(uint16_t itemId,
                                                           size_t count) {
...

  {
    ...
    for (const auto &pair : itemDataCache) {
      ...
      double score = 0.0;
      float histScore =
          CompareHistograms(sourceData.histogram, target.histogram);

      if (sourceData.isOpaque) {
        // Strategy A: aHash for opaque sprites
        // Mappingtool: We only compare against other opaque sprites to avoid
        // matching walls with items
        if (target.isOpaque) {
        ...
        if (target.isOpaque) {
          continue; // Mappingtool: Transparent target does not match opaque
                    // candidate
        }
        ...
        if (sourceData.width == target.width &&
            sourceData.height == target.height) {
          ...
          auto m1 = ResizeMask(sourceData.binaryMask, sourceData.width,
                               sourceData.height, w, h);
          auto m2 =
              ResizeMask(target.binaryMask, target.width, target.height, w, h);
          sourceOnes = 0;
          targetOnes = 0;
