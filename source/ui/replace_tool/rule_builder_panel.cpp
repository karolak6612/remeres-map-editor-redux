...
bool RuleBuilderPanel::ItemDropTarget::OnDropText(wxCoord x, wxCoord y,
                                                  const wxString &data) {
  ...
  if (hit.type == RuleBuilderPanel::HitResult::Source) {
    if (hit.ruleIndex >= 0 && hit.ruleIndex < (int)m_panel->m_rules.size()) {
      ...
      return true;
             ...
             hit.type == RuleBuilderPanel::HitResult::DeleteTarget) {
    if (hit.ruleIndex >= 0 && hit.ruleIndex < (int)m_panel->m_rules.size()) {
      // Replace existing target
      if (hit.targetIndex >= 0 &&
          hit.targetIndex < m_panel->m_rules[hit.ruleIndex].targets.size()) {
        ...
        return true;
             ...
             RuleBuilderPanel::HitResult::AddTarget) { // Drag to [+] slot
    if (hit.ruleIndex >= 0 && hit.ruleIndex < (int)m_panel->m_rules.size()) {
      // TRASH LOGIC: Reject if has trash
      bool hasTrash = std::ranges::any_of(
          m_panel->m_rules[hit.ruleIndex].targets,
          [](const auto &t) { return t.id == TRASH_ITEM_ID; });
      if (hasTrash) {
        return false;
      }
    ...
    // Initialize with empty targets (User needs to add them) or maybe auto-add
    // the same item? User said "Drop New Source Here", so empty targets is
    // fine.
...
wxDragResult RuleBuilderPanel::ItemDropTarget::OnDragOver(wxCoord x, wxCoord y,
                                                          wxDragResult def) {
...
int RuleBuilderPanel::GetRuleHeight(int index, int width) const {
  ...
  float availableWidth =
      width - CARD_MARGIN_X * 2 - TARGET_START_X - CARD_PADDING;
  ...
  bool hasTrash =
      std::ranges::any_of(m_rules[index].targets,
                          [](const auto &t) { return t.id == TRASH_ITEM_ID; });
  if (!hasTrash) {
    targetCount++; // [+] slot
  }
...
void RuleBuilderPanel::LayoutRules() {
  ...
  m_ruleYCache.clear();
  int currentY =
      RuleCardRenderer::CARD_MARGIN_Y + RuleCardRenderer::HEADER_HEIGHT;
  ...
  int footerHeight =
      RuleCardRenderer::ITEM_H + (RuleCardRenderer::CARD_MARGIN_Y * 2);
...
void RuleBuilderPanel::OnMouse(wxMouseEvent &event) {
  if (event.LeftDown()) {
    ...
    if (hit.type == HitResult::DeleteRule && hit.ruleIndex != -1) {
      ...
      LayoutRules();
      Refresh();
    } else if (hit.type == HitResult::DeleteTarget && hit.ruleIndex != -1 &&
               hit.targetIndex != -1) {
      m_rules[hit.ruleIndex].targets.erase(
          m_rules[hit.ruleIndex].targets.begin() + hit.targetIndex);
      ...
      wxMessageDialog dlg(this,
                          "Are you sure you want to clear all rules? This "
  ...
  if (event.Moving()) {
    HitResult hit = HitTest(event.GetX(), event.GetY());
    if (hit.type != m_dragHover.type ||
        hit.ruleIndex != m_dragHover.ruleIndex ||
        hit.targetIndex != m_dragHover.targetIndex) {
      m_dragHover = hit;
      Refresh();
    }
  }
...
RuleBuilderPanel::HitResult RuleBuilderPanel::HitTest(int x, int y) const {
  ...
  if (y < HEADER_HEIGHT) {
    ...
    // Otherwise, we are clicking the header background/labels - consume the hit
    // so we don't click rules underneath
  ...
  for (size_t i = 0; i < m_rules.size(); ++i) {
    ...
    if (card.Contains(x, absY)) {
      ...
      if (localX >= startX && localX <= startX + CARD_W && localY >= sourceY &&
          localY <= sourceY + ITEM_HEIGHT) {
      ...
      int columns =
          std::max(1, (int)(availableWidth / (CARD_W + ITEM_SPACING)));

      for (size_t t = 0; t < m_rules[i].targets.size(); ++t) {
        int row = t / columns;
        int col = t % columns;
        float tx = targetStartX + col * (RuleCardRenderer::CARD_W +
                                         RuleCardRenderer::ITEM_SPACING);
        float ty = RuleCardRenderer::CARD_PADDING +
                   row * (ITEM_HEIGHT + RuleCardRenderer::ITEM_SPACING);

        if (localX >= tx && localX <= tx + RuleCardRenderer::CARD_W &&
            localY >= ty && localY <= ty + ITEM_HEIGHT) {
      ...
      // Add Target Slot
      bool hasTrash =
          std::ranges::any_of(m_rules[i].targets, [](const auto &t) {
  ...
  int newRuleY =
      GetRuleY(m_rules.size(), width) + RuleCardRenderer::CARD_MARGIN_Y;
...
void RuleBuilderPanel::OnNanoVGPaint(NVGcontext *vg, int width, int height) {
  ...
  // Note: NanoVGCanvas already applies a convert translation of nvgTranslate(0,
  // -scrollPos). To draw fixed elements (Background, Header), we must undo this
  // translation.
  ...
  RuleCardRenderer::DrawClearButton(vg, width,
                                    m_dragHover.type == HitResult::ClearRules);
  RuleCardRenderer::DrawSaveButton(vg, width,
                                   m_dragHover.type == HitResult::SaveRule);
  nvgRestore(vg);

  // 2. Draw Content (Implicitly scrolled by base class)
  for (size_t i = 0; i < m_rules.size(); ++i) {
    int ruleY = GetRuleY(i, width);
    bool hoverDel = (m_dragHover.type == HitResult::DeleteRule &&
                     m_dragHover.ruleIndex == (int)i);
    int dragType =
        (m_dragHover.ruleIndex == (int)i) ? (int)m_dragHover.type : 0;
    int dragIdx =
        (m_dragHover.ruleIndex == (int)i) ? m_dragHover.targetIndex : -1;

    RuleCardRenderer::DrawRuleCard(this, vg, (int)i, ruleY, width, hoverDel,
                                   dragIdx, dragType, m_isExternalDrag);
  }

  int newRuleY =
      GetRuleY(m_rules.size(), width) + RuleCardRenderer::CARD_MARGIN_Y;
  RuleCardRenderer::DrawNewRuleArea(vg, width, newRuleY,
                                    m_dragHover.type == HitResult::NewRule);
}
