...
ItemGridPanel::ItemGridPanel(wxWindow *parent, Listener *listener)
    : VirtualItemGrid(parent, wxID_ANY), listener(listener), m_draggable(false),
      m_showDetails(true) {
...
void ItemGridPanel::SetFilter(const wxString &filter) {
  ...
  for (uint16_t id : allItems) {
    ...
    if (lowerFilter.IsEmpty()) {
      ...
      if (name.Lower().Contains(lowerFilter) ||
          wxString(std::format("{}", id)).Contains(lowerFilter) ||
          wxString(std::format("{}", it.clientID)).Contains(lowerFilter)) {
        filteredItems.push_back(id);
      }
    }
  }
...
size_t ItemGridPanel::GetItemCount() const { return filteredItems.size(); }
...
void ItemGridPanel::SetOverrideNames(
    const std::map<uint16_t, wxString> &names) {
...
void ItemGridPanel::SetDraggable(bool check) { m_draggable = check; }
