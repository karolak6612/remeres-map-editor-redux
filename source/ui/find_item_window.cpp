...
FindItemDialog::FindItemDialog(wxWindow* parent, const wxString& title, bool onlyPickupables /* = false*/) :
	wxDialog(parent, wxID_ANY, title, wxDefaultPosition, FROM_DIP(parent, wxSize(800, 600)), wxDEFAULT_DIALOG_STYLE),
	input_timer(this),
	...
	items_list->SetMinSize(FROM_DIP(this, wxSize(230, 512)));
	...
	wxIconBundle icons;
	wxIcon icon16;
	icon16.CopyFromBitmap(IMAGE_MANAGER.GetBitmap(ICON_SEARCH, FromDIP(wxSize(16, 16))));
	icons.AddIcon(icon16);
	wxIcon icon32;
	icon32.CopyFromBitmap(IMAGE_MANAGER.GetBitmap(ICON_SEARCH, FromDIP(wxSize(32, 32))));
