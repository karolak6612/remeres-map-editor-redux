...
BrowseTileListBox::BrowseTileListBox(wxWindow *parent, wxWindowID id,
                                     Tile *tile)
...
void BrowseTileListBox::OnDrawItem(NVGcontext *vg, const wxRect &rect,
                                   size_t n) {
  Item *item = items[n];
...
int BrowseTileListBox::OnMeasureItem(size_t n) const { return FromDIP(32); }
...
void BrowseTileListBox::UpdateItems() {
  ...
  for (auto it = edit_tile->items.rbegin(); it != edit_tile->items.rend();
       ++it) {
...
BrowseTileWindow::BrowseTileWindow(wxWindow *parent, Tile *tile,
                                   wxPoint position /* = wxDefaultPosition */)
    : wxDialog(parent, wxID_ANY, "Browse Field", position,
               FROM_DIP(parent, wxSize(600, 400)),
  ...
  infoSizer->Add(newd wxStaticText(this, wxID_ANY, "Position:  " + pos),
                 wxSizerFlags(0).Left());
  infoSizer->Add(
      item_count_txt = newd wxStaticText(
          this, wxID_ANY, "Item count:  " + i2ws(item_list->GetItemCount())),
      wxSizerFlags(0).Left());
  infoSizer->Add(newd wxStaticText(this, wxID_ANY,
                                   "Protection zone:  " + b2yn(tile->isPZ())),
                 wxSizerFlags(0).Left());
  infoSizer->Add(newd wxStaticText(
                     this, wxID_ANY,
                     "No PvP:  " + b2yn(tile->getMapFlags() & TILESTATE_NOPVP)),
                 wxSizerFlags(0).Left());
  infoSizer->Add(newd wxStaticText(this, wxID_ANY,
                                   "No logout:  " + b2yn(tile->getMapFlags() &
                                                         TILESTATE_NOLOGOUT)),
                 wxSizerFlags(0).Left());
  infoSizer->Add(newd wxStaticText(this, wxID_ANY,
                                   "PvP zone:  " + b2yn(tile->getMapFlags() &
                                                        TILESTATE_PVPZONE)),
                 wxSizerFlags(0).Left());
  infoSizer->Add(
      newd wxStaticText(this, wxID_ANY, "House:  " + b2yn(tile->isHouseTile())),
      wxSizerFlags(0).Left());
  ...
  select_raw_button->Bind(wxEVT_BUTTON, &BrowseTileWindow::OnClickSelectRaw,
                          this);
...
void BrowseTileWindow::OnClickDelete(wxCommandEvent &WXUNUSED(event)) {
  ...
  item_count_txt->SetLabelText("Item count:  " +
                               i2ws(item_list->GetItemCount()));
}
