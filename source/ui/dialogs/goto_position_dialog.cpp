...
GotoPositionDialog::GotoPositionDialog(wxWindow* parent, Editor& editor) :
	...
	sizer->Add(posctrl, wxSizerFlags(0).Border(wxTOP | wxLEFT | wxRIGHT, 20));
	...
	sizer->Add(tmpsizer, wxSizerFlags(0).Center().Border(wxALL, 20)); // Border to top too
	...
	wxIconBundle icons;
	icons.AddIcon(wxIcon(IMAGE_MANAGER.GetBitmap(ICON_LOCATION, wxSize(16, 16))));
	icons.AddIcon(wxIcon(IMAGE_MANAGER.GetBitmap(ICON_LOCATION, wxSize(32, 32))));
	SetIcons(icons);
}
