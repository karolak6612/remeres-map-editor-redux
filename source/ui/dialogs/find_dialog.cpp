...
static bool caseInsensitiveContains(std::string_view haystack,
                                    std::string_view needle_lower) {
  auto it =
      std::search(haystack.begin(), haystack.end(), needle_lower.begin(),
                  needle_lower.end(), [](char ch1, char ch2) {
...
FindDialog::FindDialog(wxWindow *parent, wxString title)
    : wxDialog(g_gui.root, wxID_ANY, title, wxDefaultPosition, wxDefaultSize,
               wxRESIZE_BORDER | wxCAPTION | wxCLOSE_BOX),
      idle_input_timer(this), result_brush(nullptr), result_id(0) {
  wxSizer *sizer = newd wxBoxSizer(wxVERTICAL);

  search_field =
      newd KeyForwardingTextCtrl(this, JUMP_DIALOG_TEXT, "", wxDefaultPosition,
                                 wxDefaultSize, wxTE_PROCESS_ENTER);
  search_field->SetHint("Type to search...");
  search_field->SetToolTip(
      "Type at least 2 characters to search for brushes or items.");
  ...
  // We can't call it here since it calls an abstract function, call in child
  // constructors instead. RefreshContents();
...
void FindDialog::OnKeyDown(wxKeyEvent &event) {
  ...
  case WXK_DOWN: {
    if (item_list->GetItemCount() > 0) {
      ...
      if (n == wxNOT_FOUND) {
        n = 0;
      } else if (static_cast<uint32_t>(n) < itemcount - amount &&
...
void FindDialog::OnClickCancel(wxCommandEvent &WXUNUSED(event)) { EndModal(0); }
...
void FindBrushDialog::OnClickOKInternal() {
  // This is kind of stupid as it would fail unless the "Please enter a search
  // string" wasn't there
  if (item_list->GetItemCount() > 0) {
...
FindDialogListBox::FindDialogListBox(wxWindow *parent, wxWindowID id)
    : NanoVGListBox(parent, id, wxLB_SINGLE), cleared(false),
      no_matches(false) {
  Clear();
}
...
void FindDialogListBox::OnDrawItem(NVGcontext *vg, const wxRect &rect,
                                   size_t n) {
  ...
  if (no_matches) {
    ...
    nvgText(vg, rect.x + 40, rect.y + rect.height / 2.0f,
            "No matches for your search.", nullptr);
  } else if (cleared) {
    nvgFontSize(vg, 12.0f);
    nvgFontFace(vg, "sans");
    nvgFillColor(vg, nvgRGBA(textColour.Red(), textColour.Green(),
    ...
    nvgTextAlign(vg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
    nvgText(vg, rect.x + 40, rect.y + rect.height / 2.0f,
            "Please enter your search string.", nullptr);
  } else {
    ASSERT(n < brushlist.size());
    Sprite *spr = g_gui.sprite_database.getSprite(brushlist[n]->getLookID());
    if (spr) {
      int tex = GetOrCreateSpriteTexture(vg, spr);
      if (tex > 0) {
        int icon_size = rect.height;
        NVGpaint imgPaint = nvgImagePattern(vg, rect.x, rect.y, icon_size,
                                            icon_size, 0, tex, 1.0f);
        nvgBeginPath(vg);
    ...
    nvgText(vg, rect.x + rect.height + 8, rect.y + rect.height / 2.0f,
            name.ToUTF8().data(), nullptr);
  }
}

int FindDialogListBox::OnMeasureItem(size_t n) const { return FromDIP(32); }
