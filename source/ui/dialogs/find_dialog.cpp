...
{
    auto it = std::search(haystack.begin(), haystack.end(), needle_lower.begin(), needle_lower.end(), [](char ch1, char ch2) {
...
{
    if (event.GetKeyCode() == WXK_UP || event.GetKeyCode() == WXK_DOWN || event.GetKeyCode() == WXK_PAGEDOWN
...
FindDialogListBox::FindDialogListBox(wxWindow* parent, wxWindowID id) :
    NanoVGListBox(parent, id, wxLB_SINGLE), cleared(false), no_matches(false)
{
    Clear();
}
