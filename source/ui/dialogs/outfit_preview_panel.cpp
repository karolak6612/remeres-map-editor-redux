...
OutfitPreviewPanel::OutfitPreviewPanel(wxWindow *parent, const Outfit &outfit)
    : wxPanel(parent, wxID_ANY, wxDefaultPosition,
              wxSize(PREVIEW_SIZE, PREVIEW_SIZE), wxBORDER_NONE),
      preview_outfit(outfit), preview_direction(0) {
...
void OutfitPreviewPanel::OnPaint(wxPaintEvent &event) {
  ...
  GameSprite *spr =
      g_gui.sprite_database.getCreatureSprite(preview_outfit.lookType);
  if (spr) {
    ...
    if (bmp.IsOk()) {
      ...
      if (gc) {
        ...
        double scale = std::min<double>((double)PREVIEW_SIZE / sw,
                                        (double)PREVIEW_SIZE / sh);
