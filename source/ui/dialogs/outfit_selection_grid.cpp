...
OutfitSelectionGrid::OutfitSelectionGrid(wxWindow *parent,
                                         OutfitChooserDialog *owner,
                                         bool is_favorites)
    : NanoVGCanvas(parent, wxID_ANY, wxVSCROLL | wxWANTS_CHARS),
      is_favorites(is_favorites), selected_index(-1), owner(owner), columns(1),
      item_width(OUTFIT_TILE_WIDTH), item_height(OUTFIT_TILE_HEIGHT),
      padding(4), hover_index(-1) {
  ...
  if (is_favorites) {
    Bind(wxEVT_MENU, &OutfitChooserDialog::OnFavoriteRename, owner,
         OutfitChooserDialog::ID_FAVORITE_RENAME);
    Bind(wxEVT_MENU, &OutfitChooserDialog::OnFavoriteEdit, owner,
         OutfitChooserDialog::ID_FAVORITE_EDIT);
    Bind(wxEVT_MENU, &OutfitChooserDialog::OnFavoriteDelete, owner,
         OutfitChooserDialog::ID_FAVORITE_DELETE);
  }
}
...
void OutfitSelectionGrid::UpdateFilter(const wxString &filter,
  ...
  std::ranges::copy_if(
      all_outfits, std::back_inserter(filtered_outfits), [&](const auto &item) {
        bool match = filter.IsEmpty() ||
                     item.name.Lower().Contains(filter.Lower()) ||
                     wxString::Format("%d", item.lookType).Contains(filter);
  ...
  if (it != filtered_outfits.end()) {
    selected_index =
        static_cast<int>(std::distance(filtered_outfits.begin(), it));
  } else {
    selected_index = -1;
  }
...
int OutfitSelectionGrid::GetOrCreateOutfitImage(NVGcontext *vg, int lookType,
                                                const Outfit &outfit) {
  uint32_t cache_key = (static_cast<uint32_t>(lookType) << 16) |
                       (outfit.getColorHash() & 0xFFFF);
  ...
  uint64_t elapsedTime = g_gui.texture_gc.getElapsedTime();
  GameSprite *spr = g_gui.sprite_database.getCreatureSprite(lookType);
  if (!spr) {
    return 0;
  }
...
void OutfitSelectionGrid::OnNanoVGPaint(NVGcontext *vg, int width, int height) {
  ...
  // So to fill the visible background, we need to draw at (0, scrollPos, width,
  // height). NanoVGCanvas implementation: nvgTranslate(m_nvg, 0, -m_scrollPos);
  ...
  int end_row = (scrollPos + height + (item_height + padding) - 1) /
                (item_height + padding);
  ...
  for (int i = start_idx; i < end_idx; ++i) {
    ...
    int lookType = is_favorites ? favorite_items[i].outfit.lookType
                                : filtered_outfits[i].lookType;
    wxString name =
        is_favorites ? favorite_items[i].label : filtered_outfits[i].name;
    ...
    if (i == selected_index) {
      ...
      nvgFillColor(vg,
                   NvgUtils::ToNvColor(Theme::Get(Theme::Role::CardBaseHover)));
    } else {
    ...
    // Border
    nvgBeginPath(vg);
    nvgRoundedRect(vg, rect.x + 0.5f, rect.y + 0.5f, rect.width - 1.0f,
                   rect.height - 1.0f, 4.0f);
    if (i == selected_index) {
      nvgStrokeColor(vg, NvgUtils::ToNvColor(Theme::Get(Theme::Role::Accent)));
      nvgStrokeWidth(vg, 2.0f);
    } else {
      nvgStrokeColor(vg,
                     NvgUtils::ToNvColor(Theme::Get(Theme::Role::CardBorder)));
    ...
    nvgText(vg, rect.x + rect.width / 2, rect.y + 75, name.ToUTF8().data(),
            nullptr);
...
wxRect OutfitSelectionGrid::GetItemRect(int index) const {
  ...
  return wxRect(padding + col * (item_width + padding),
                padding + row * (item_height + padding), item_width,
                item_height);
}
...
void OutfitSelectionGrid::OnContextMenu(wxContextMenuEvent &event) {
  ...
  menu.Append(OutfitChooserDialog::ID_FAVORITE_RENAME, "Rename...")
      ->SetBitmap(IMAGE_MANAGER.GetBitmap(ICON_PEN_TO_SQUARE, wxSize(16, 16)));
  menu.Append(OutfitChooserDialog::ID_FAVORITE_EDIT, "Update with Current")
      ->SetBitmap(IMAGE_MANAGER.GetBitmap(ICON_SYNC, wxSize(16, 16)));
  menu.Append(OutfitChooserDialog::ID_FAVORITE_DELETE, "Delete")
      ->SetBitmap(IMAGE_MANAGER.GetBitmap(ICON_TRASH_CAN, wxSize(16, 16)));
