...
void OutfitSelectionGrid::UpdateFilter(const wxString& filter, bool colorable_only)
{
    ...
    std::ranges::copy_if(all_outfits, std::back_inserter(filtered_outfits), [&](const auto& item) {
        bool match
            = filter.IsEmpty() || item.name.Lower().Contains(filter.Lower()) || wxString::Format("%d", item.lookType).Contains(filter);
    ...
    auto it = std::ranges::find_if(filtered_outfits, [&](const auto& item) { return item.lookType == owner->GetOutfit().lookType; });
...
void OutfitSelectionGrid::UpdateVirtualSize()
{
    ...
    int count = static_cast<int>(is_favorites ? favorite_items.size() : filtered_outfits.size());
...
int OutfitSelectionGrid::GetOrCreateOutfitImage(NVGcontext* vg, int lookType, const Outfit& outfit)
{
    ...
    wxBitmap bmp = SpriteIconGenerator::Generate(g_gui.sprites, g_gui.loader, clientID, SPRITE_SIZE_64x64, outfit);
...
void OutfitSelectionGrid::OnNanoVGPaint(NVGcontext* vg, int width, int height)
{
    ...
    nvgBeginPath(vg);
    nvgRect(vg, 0.0f, static_cast<float>(scrollPos), static_cast<float>(width), static_cast<float>(height));
    ...
    for (int i = start_idx; i < end_idx; ++i) {
        ...
        nvgBeginPath(vg);
        nvgRoundedRect(
            vg, static_cast<float>(rect.x), static_cast<float>(rect.y), static_cast<float>(rect.width), static_cast<float>(rect.height),
            4.0f
        );
        ...
        nvgBeginPath(vg);
        nvgRoundedRect(
            vg, static_cast<float>(rect.x) + 0.5f, static_cast<float>(rect.y) + 0.5f, static_cast<float>(rect.width) - 1.0f,
            static_cast<float>(rect.height) - 1.0f, 4.0f
        );
        if (i == selected_index) {
        ...
        if (imgId > 0) {
            float ix = static_cast<float>(rect.x) + ((static_cast<float>(rect.width) - 64.0f) / 2.0f);
            float iy = static_cast<float>(rect.y) + 8.0f;

            NVGpaint imgPaint = nvgImagePattern(vg, ix, iy, 64.0f, 64.0f, 0.0f, imgId, 1.0f);
            nvgBeginPath(vg);
            nvgRect(vg, ix, iy, 64.0f, 64.0f);
        ...
        nvgText(
            vg, static_cast<float>(rect.x) + (static_cast<float>(rect.width) / 2.0f), static_cast<float>(rect.y) + 75.0f,
            name.ToUTF8().data(), nullptr
        );
        ...
        nvgText(
            vg, static_cast<float>(rect.x) + (static_cast<float>(rect.width) / 2.0f), static_cast<float>(rect.y) + 92.0f, idStr.c_str(),
            nullptr
        );
    }
}
