...
OutfitChooserDialog::OutfitChooserDialog(wxWindow *parent,
                                         const Outfit &current_outfit)
    : wxDialog(parent, wxID_ANY, "Customise Character", wxDefaultPosition,
               wxSize(1200, 850), wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER),
      current_outfit(current_outfit), current_speed(220), current_name("You"),
  ...
  int maxLookType = g_gui.sprite_database.getCreatureSpriteMaxID();
  for (int i = 1; i <= maxLookType; ++i) {
    ...
    GameSprite *spr = g_gui.sprite_database.getCreatureSprite(item.lookType);
  ...
  auto CreateHeader = [this](const wxString &title) -> wxWindow * {
    ...
    header->SetForegroundColour(
        Theme::Get(Theme::Role::TextSubtle)); // Subtle grey for headers
    return header;
  };
  ...
  col1_sizer->Add(new wxStaticLine(this, wxID_ANY), 0,
  ...
  head_btn = new wxButton(this, ID_COLOR_HEAD, "Head", wxDefaultPosition,
                          wxSize(50, -1));
  head_btn->SetBitmap(IMAGE_MANAGER.GetBitmap(ICON_USER_SOLID, wxSize(16, 16)));
  body_btn = new wxButton(this, ID_COLOR_BODY, "Primary", wxDefaultPosition,
                          wxSize(50, -1));
  body_btn->SetBitmap(IMAGE_MANAGER.GetBitmap(ICON_SHIRT, wxSize(16, 16)));
  legs_btn = new wxButton(this, ID_COLOR_LEGS, "Secondary", wxDefaultPosition,
                          wxSize(50, -1));
  legs_btn->SetBitmap(IMAGE_MANAGER.GetBitmap(ICON_SOCKS, wxSize(16, 16)));
  feet_btn = new wxButton(this, ID_COLOR_FEET, "Detail", wxDefaultPosition,
                          wxSize(50, -1));
  feet_btn->SetBitmap(
      IMAGE_MANAGER.GetBitmap(ICON_SHOE_PRINTS, wxSize(16, 16)));
  ...
  m_palette->Bind(wxEVT_BUTTON,
                  [this](wxCommandEvent &evt) { SelectColor(evt.GetInt()); });
  ...
  text_fields->Add(new wxStaticText(this, wxID_ANY, "Speed:"), 0,
                   wxALIGN_CENTER_VERTICAL);
  speed_ctrl =
      new wxSpinCtrl(this, ID_SPEED, wxEmptyString, wxDefaultPosition,
                     wxDefaultSize, wxSP_ARROW_KEYS, 50, 3000, current_speed);
  text_fields->Add(speed_ctrl, 1, wxEXPAND);

  text_fields->Add(new wxStaticText(this, wxID_ANY, "Name:"), 0,
                   wxALIGN_CENTER_VERTICAL);
  ...
  outfits_header_row->Add(CreateHeader("Available Outfits"), 1,
                          wxALIGN_BOTTOM | wxLEFT, 4);

  outfit_search = new wxSearchCtrl(this, ID_SEARCH, wxEmptyString,
  ...
  outfits_header_row->Add(colorable_only_check, 0,
                          wxALIGN_BOTTOM | wxRIGHT | wxBOTTOM, 4);
  ...
  favorites_panel->SetMinSize(
      wxSize(430, -1)); // Force width for 4 columns of 100px
  favs_sizer->Add(favorites_panel, 1, wxEXPAND);

  wxButton *fav_btn =
      new wxButton(this, ID_ADD_FAVORITE, "Save Current Outfit as Favorite",
  ...
  wxButton *ok_btn =
      new wxButton(this, wxID_OK, "OK", wxDefaultPosition, wxSize(90, 30));
  ...
  colorable_only_check->Bind(wxEVT_CHECKBOX,
                             &OutfitChooserDialog::OnSearchOutfit, this);
...
void OutfitChooserDialog::OnRandomize(wxCommandEvent &event) {
  ...
  std::uniform_int_distribution<int> distrib(0,
                                             TemplateOutfitLookupTableSize - 1);
...
void OutfitChooserDialog::OnAddFavorite(wxCommandEvent &event) {
  wxString label = wxGetTextFromUser(
      "Enter a name for this favorite:", "Add to Favorites", "My Outfit");
  if (label.IsEmpty()) {
    return;
  }
...
void OutfitChooserDialog::OnFavoriteRename(wxCommandEvent &event) {
  ...
  wxString newLabel = wxGetTextFromUser(
      "Enter a new name for this favorite:", "Rename Favorite", oldLabel);
...
void OutfitChooserDialog::OnFavoriteEdit(wxCommandEvent &event) {
  ...
  if (wxMessageBox("Update this favorite with current preview settings?",
                   "Edit Favorite", wxYES_NO | wxICON_QUESTION) == wxYES) {
...
void OutfitChooserDialog::OnFavoriteDelete(wxCommandEvent &event) {
  ...
  if (wxMessageBox("Are you sure you want to delete this favorite?",
                   "Delete Favorite", wxYES_NO | wxICON_WARNING) == wxYES) {
...
Outfit OutfitChooserDialog::GetOutfit() const { return current_outfit; }
