...
DCButton::DCButton()
    : NanoVGCanvas(nullptr, wxID_ANY, 0), type(DC_BTN_NORMAL), state(false),
      size(RENDER_SIZE_16x16), sprite(nullptr), overlay(nullptr) {
...
DCButton::DCButton(wxWindow *parent, wxWindowID id, wxPoint pos, int type,
                   RenderSize sz, int sprite_id)
    : NanoVGCanvas(parent, id, 0), type(type), state(false), size(sz),
      sprite(nullptr), overlay(nullptr) {
...
void DCButton::SetSprite(int _sprid) {
  if (_sprid != 0) {
    sprite = g_gui.sprite_database.getSprite(_sprid);
  } else {
    sprite = nullptr;
  }
  Refresh();
...
void DCButton::OnNanoVGPaint(NVGcontext *vg, int width, int height) {
  ...
  if (type == DC_BTN_TOGGLE && GetValue()) {
    DrawSunkenBorder(vg, static_cast<float>(size_x),
                     static_cast<float>(size_y));
  } else {
    DrawRaisedBorder(vg, static_cast<float>(size_x),
                     static_cast<float>(size_y));
  }
  ...
  if (sprite) {
    ...
    if (tex > 0) {
      ...
      NVGpaint imgPaint =
          nvgImagePattern(vg, 2, 2, imgSize, imgSize, 0, tex, 1.0f);
      nvgBeginPath(vg);
      ...
      if (overlay && type == DC_BTN_TOGGLE && GetValue()) {
        ...
        if (overlayTex > 0) {
          NVGpaint ovPaint =
              nvgImagePattern(vg, 2, 2, imgSize, imgSize, 0, overlayTex, 1.0f);
          nvgBeginPath(vg);
          nvgRect(vg, 2, 2, imgSize, imgSize);
...
void DCButton::OnClick(wxMouseEvent &WXUNUSED(evt)) {
  wxCommandEvent event(type == DC_BTN_TOGGLE
