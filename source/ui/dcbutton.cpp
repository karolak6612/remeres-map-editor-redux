...
DCButton::DCButton(wxWindow* parent, wxWindowID id, wxPoint pos, int type, RenderSize sz, int sprite_id) :
    NanoVGCanvas(parent, id, 0), type(type), state(false), size(sz), sprite_id(0), overlay_id(0)
{
...
void DCButton::SetOverlay(int espr)
{
    overlay_id = espr;
    Refresh();
}
...
void DCButton::SetValue(bool val)
{
    ...
    if (state != oldval) {
        if (GetValue() && g_settings.getInteger(Config::USE_GUI_SELECTION_SHADOW)) {
            SetOverlay(EDITOR_SPRITE_SELECTION_MARKER);
        } else {
            SetOverlay(0);
        }
    }
...
void DCButton::OnNanoVGPaint(NVGcontext* vg, int width, int height)
{
    ...
    if (sprite_id != 0) {
        int tex = 0;
        if (sprite_id < 0) {
            Sprite* spr = g_gui.sprites.getEditorSprite(sprite_id);
            if (spr) {
        ...
        if (tex > 0) {
            ...
            if (overlay_id != 0 && type == DC_BTN_TOGGLE && GetValue()) {
                int overlayTex = 0;
                if (overlay_id < 0) {
                    Sprite* spr = g_gui.sprites.getEditorSprite(overlay_id);
                    if (spr) {
