...
DatDebugViewListBox::DatDebugViewListBox(wxWindow *parent, wxWindowID id)
  ...
  sprites.reserve(g_gui.sprite_database.getItemSpriteMaxID());
  for (int id = 0; id < g_gui.sprite_database.getItemSpriteMaxID(); ++id) {
    Sprite *spr = g_gui.sprite_database.getSprite(id);
    if (spr) {
...
void DatDebugViewListBox::OnDrawItem(wxDC &dc, const wxRect &rect,
                                     size_t n) const {
  if (n < sprites.size()) {
    sprites[n]->DrawTo(&dc, SPRITE_SIZE_32x32, rect.GetX(), rect.GetY(),
...
DatDebugView::DatDebugView(wxWindow *parent) : wxPanel(parent) {
  ...
  search_field =
      newd wxTextCtrl(this, wxID_ANY, "", wxDefaultPosition, wxDefaultSize);
