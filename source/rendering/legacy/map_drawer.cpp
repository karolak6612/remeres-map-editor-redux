...
MapDrawer::MapDrawer(MapCanvas* canvas) :
	canvas(canvas), editor(canvas->editor_) {
	light_drawer = std::make_shared<LightDrawer>();
	brush_renderer = std::make_unique<rme::render::BrushRenderer>();
	selection_renderer = std::make_unique<rme::render::SelectionRenderer>();
	ui_renderer = std::make_unique<rme::render::UIRenderer>();
	grid_renderer = std::make_unique<rme::render::GridRenderer>();
	tooltip_renderer = std::make_unique<rme::render::TooltipRenderer>();

	brush_renderer->initialize();
	selection_renderer->initialize();
	ui_renderer->initialize();
	grid_renderer->initialize();
	tooltip_renderer->initialize();
}
...
void MapDrawer::SetupVars() {
	...
	dragging = canvas->dragging_;
	dragging_draw = canvas->dragging_draw_;
	...
	// Fill render context for specialized renderers
	render_context.viewportWidth = screensize_x;
	render_context.viewportHeight = screensize_y;
	render_context.zoom = zoom;
	render_context.scrollX = view_scroll_x;
	render_context.scrollY = view_scroll_y;
	render_context.currentFloor = floor;
	render_context.startX = start_x;
	render_context.startY = start_y;
	render_context.endX = end_x;
	render_context.endY = end_y;
	render_context.tileSize = tile_size;
	render_context.mouseMapX = mouse_map_x;
	render_context.mouseMapY = mouse_map_y;
}
...
void MapDrawer::Draw() {
	...
	if (options.dragging && selection_renderer) {
		selection_renderer->renderSelectionBox(render_context.mouseMapX, render_context.mouseMapY, render_context.mouseMapX, render_context.mouseMapY);
	}
	...
	if (options.show_grid && grid_renderer) {
		grid_renderer->renderGrid(render_context, options.show_grid);
	}

	if (options.show_ingame_box && ui_renderer) {
		ui_renderer->renderIngameBox(screensize_x / 2, screensize_y / 2, 15 * tile_size, 11 * tile_size);
	}

	if (options.show_tooltips && tooltip_renderer) {
		tooltip_renderer->renderTooltips();
	}
}
...
void MapDrawer::DrawDraggingShadow() {
	...
	if (!editor.selection.isBusy() && dragging && !options.ingame) {
		for (TileSet::iterator tit = editor.selection.begin(); tit != editor.selection.end(); tit++) {
			...
			move_x = canvas->drag_start_x_ - mouse_map_x;
			move_y = canvas->drag_start_y_ - mouse_map_y;
			move_z = canvas->drag_start_z_ - floor;
...
void MapDrawer::DrawSelectionBox() {
	...
	int last_click_rx = canvas->last_click_abs_x_ - view_scroll_x;
	int last_click_ry = canvas->last_click_abs_y_ - view_scroll_y;
	double cursor_rx = canvas->cursor_x_ * zoom;
	double cursor_ry = canvas->cursor_y_ * zoom;
...
void MapDrawer::DrawBrush() {
	...
	if (dragging_draw) {
		...
		if (brush->isWall()) {
			int last_click_start_map_x = std::min(canvas->last_click_map_x_, mouse_map_x);
			int last_click_start_map_y = std::min(canvas->last_click_map_y_, mouse_map_y);
			int last_click_end_map_x = std::max(canvas->last_click_map_x_, mouse_map_x) + 1;
			int last_click_end_map_y = std::max(canvas->last_click_map_y_, mouse_map_y) + 1;
			...
			if (g_gui.GetBrushShape() == BRUSHSHAPE_SQUARE || brush->isSpawn() /* Spawn brush is always square */) {
				if (brush->isRaw() || brush->isOptionalBorder()) {
					...
					if (mouse_map_x < canvas->last_click_map_x_) {
						start_x = mouse_map_x;
						end_x = canvas->last_click_map_x_;
					} else {
						start_x = canvas->last_click_map_x_;
						end_x = mouse_map_x;
					}
					if (mouse_map_y < canvas->last_click_map_y_) {
						start_y = mouse_map_y;
						end_y = canvas->last_click_map_y_;
					} else {
						start_y = canvas->last_click_map_y_;
						end_y = mouse_map_y;
					}
				...
				int start_y, end_y;
				int width = std::max(
					std::abs(std::max(mouse_map_y, canvas->last_click_map_y_) - std::min(mouse_map_y, canvas->last_click_map_y_)),
					std::abs(std::max(mouse_map_x, canvas->last_click_map_x_) - std::min(mouse_map_x, canvas->last_click_map_x_))
				);

				if (mouse_map_x < canvas->last_click_map_x_) {
					start_x = canvas->last_click_map_x_ - width;
					end_x = canvas->last_click_map_x_;
				} else {
					start_x = canvas->last_click_map_x_;
					end_x = canvas->last_click_map_x_ + width;
				}

				if (mouse_map_y < canvas->last_click_map_y_) {
					start_y = canvas->last_click_map_y_ - width;
					end_y = canvas->last_click_map_y_;
				} else {
					start_y = canvas->last_click_map_y_;
					end_y = canvas->last_click_map_y_ + width;
				}
...
void MapDrawer::DrawTile(TileLocation* location) {
	...
	Waypoint* waypoint = canvas->editor_.map.waypoints.getWaypoint(location);
