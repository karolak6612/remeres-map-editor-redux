...
void LiveCursorDrawer::draw(const DrawContext& ctx, IMapAccess& map_access)
{
    auto& sprite_batch = ctx.sprite_batch;
    const auto& view = ctx.view;
    const auto& settings = ctx.settings;

    if (settings.ingame || !map_access.isLive()) {
        return;
    }

    if (auto* live = map_access.getLiveSocket(); live) {
        for (LiveCursor& cursor : live->getCursorList()) {
            ...
            if (cursor.pos.z < view.floor) {
                cursor.color = wxColor(
                    cursor.color.Red(), cursor.color.Green(), cursor.color.Blue(), std::max<uint8_t>(cursor.color.Alpha() / 2, 64)
                );
            }
