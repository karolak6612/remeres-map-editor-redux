...
void BrushCursorDrawer::draw(
    SpriteBatch& sprite_batch, PrimitiveRenderer& primitive_renderer, const AtlasManager& atlas, int x, int y, Brush* brush, uint8_t r,
    uint8_t g, uint8_t b
)
{
    x += (TILE_SIZE / 2);
    ...
    for (int i = 0; i < segments; i++) {
        ...
        primitive_renderer.drawTriangle(
            center, glm::vec2(cos(angle1) * radius + x, sin(angle1) * radius + y),
            glm::vec2(cos(angle2) * radius + x, sin(angle2) * radius + y), circleColor
        );
    }
    ...
    // Tip Triangle: (-5, -5), (0,0), (5, -5) relative to x,y
    primitive_renderer.drawTriangle(glm::vec2(x - 5, y - 5), glm::vec2(x, y), glm::vec2(x + 5, y - 5), bgColor);
    ...
    for (int i = 0; i < 8; ++i) {
        primitive_renderer.drawLine(
            glm::vec2(vertexes[i][0] + x, vertexes[i][1] + y), glm::vec2(vertexes[i + 1][0] + x, vertexes[i + 1][1] + y), borderColor
        );
    }
}
