...
void BrushCursorDrawer::draw(const DrawContext &ctx, int x, int y, Brush *brush,
  ...
  if (g_gui.atlas_lifecycle.ensureAtlasManager()) {
    ...
    ctx.sprite_batch.drawRect((float)(x - 15), (float)(y - 20), (float)30,
                              (float)15, bgColor,
  ...
  // Tip Triangle: (-5, -5), (0,0), (5, -5) relative to x,y
  ctx.primitive_renderer.drawTriangle(glm::vec2(x - 5, y - 5), glm::vec2(x, y),
                                      glm::vec2(x + 5, y - 5), bgColor);
  ...
  for (int i = 0; i < 8; ++i) {
    ctx.primitive_renderer.drawLine(
        ...
        glm::vec2(vertexes[i + 1][0] + x, vertexes[i + 1][1] + y), borderColor);
  }
}
