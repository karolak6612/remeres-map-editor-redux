...
)
{
    ...
    if (!only_colors) {
        if (ctx.view.zoom < 10.0 || !ctx.options.hide_items_when_zoomed) {
            ...
            bool should_pulse = calculate_house_color && (static_cast<int>(tile->getHouseID()) == current_house_id)
            ...
            if (tile->creature && ctx.options.show_creatures) {
                creature_drawer->BlitCreature(
                    ...
                    CreatureDrawOptions {.map_pos = position, .transient_selection_bounds = ctx.options.transient_selection_bounds}
                );
    ...
    if (has_metadata) {
        ...
        if (!atlas.isSimpleAndLoaded(metadata)) {
            ...
            if (cached_patterns) {
                ...
                patterns = PatternCalculator::Calculate(&metadata, it, item, tile, tile->getPosition());
            }
            rme::collectTileSprites(clientID, patterns.x, patterns.y, patterns.z, patterns.frame);
        }
    }
