...
)
{
    ...
    if (only_colors) {
        ...
        PlanGroundItem(ctx, tile, position, r, g, b, plan);
    ...
    // Ground tooltip
    if (settings.show_tooltips && map_z == view.floor && tile->ground) {
        ItemDefinitionView ground_it = tile->ground->getDefinition();
        if (ground_it) {
    ...
    if (!only_colors) {
        if (view.zoom < 10.0 || !settings.hide_items_when_zoomed) {
            PlanStackedItems(ctx, tile, position, light_buffer, current_house_id, is_house_tile, r, g, b, plan);
        }
...
)
{
    ...
    GameSprite* ground_sprite = tile->ground->getSprite();
    if (!ground_sprite) {
        return;
    }
