...
TileRenderer::TileRenderer(ItemDrawer *id, SpriteDrawer *sd, CreatureDrawer *cd,
                           CreatureNameDrawer *cnd, FloorDrawer *fd,
                           MarkerDrawer *md, TooltipDrawer *td, Editor *ed)
    : item_drawer(id), sprite_drawer(sd), creature_drawer(cd), floor_drawer(fd),
      marker_drawer(md), tooltip_drawer(td), creature_name_drawer(cnd),
      editor(ed) {}

// Helper function to populate tooltip data from an item (in-place)
static bool FillItemTooltipData(TooltipData &data, Item *item,
                                const ItemType &it, const Position &pos,
                                bool isHouseTile, float zoom) {
  if (!item) {
    return false;
  }
    ...
    if (ctx.view.zoom < 10.0 || !ctx.options.hide_items_when_zoomed) {
      ...
      for (const auto &item : tile->items) {
        ...
        if (process_tooltips) {
          ...
          if (FillItemTooltipData(itemData, item.get(), it, position,
