...
void TileColorCalculator::Calculate(const Tile *tile,
                                    const DrawingOptions &options,
                                    uint32_t current_house_id, int spawn_count,
                                    uint8_t &r, uint8_t &g, uint8_t &b) {
  bool showspecial =
      options.settings.show_only_colors || options.settings.show_special_tiles;

  if (options.settings.show_blocking && tile->isBlocking() &&
      tile->size() > 0) {
  ...
  if (options.settings.highlight_items && item_count > 0 &&
  ...
  if (options.settings.show_spawns && spawn_count > 0) {
    // Precomputed 0.7^n * 256 for n=1..9
    static constexpr std::array<int, 9> spawn_factor = {179, 125, 88, 61, 43,
                                                        30,  21,  15, 10};
  ...
  if (options.settings.show_houses && tile->isHouseTile()) {
    ...
    if (static_cast<int>(house_id) == current_house_id) {
      ...
      if (options.frame.highlight_pulse > 0.0f) {
        float boost =
            options.frame.highlight_pulse * 0.6f; // Max 60% boost towards white

        r = static_cast<uint8_t>(
            std::min(255, static_cast<int>(r + (255 - r) * boost)));
        g = static_cast<uint8_t>(
            std::min(255, static_cast<int>(g + (255 - g) * boost)));
        b = static_cast<uint8_t>(
            std::min(255, static_cast<int>(b + (255 - b) * boost)));
      }
    }
...
void TileColorCalculator::GetHouseColor(uint32_t house_id, uint8_t &r,
                                        uint8_t &g, uint8_t &b) {
...
void TileColorCalculator::GetMinimapColor(const Tile *tile, uint8_t &r,
                                          uint8_t &g, uint8_t &b) {
