...
void FloorDrawer::draw(RenderList& list, ItemDrawer* item_drawer, SpriteDrawer* sprite_drawer, CreatureDrawer* creature_drawer, const RenderView& view, const DrawingOptions& options, Editor& editor) {
	if (view.floor != 8 && view.floor != 0 && options.transparent_floors) {
		int map_z = view.floor - 1;
		for (int map_x = view.start_x; map_x <= view.end_x; map_x++) {
			for (int map_y = view.start_y; map_y <= view.end_y; map_y++) {
