	...
	if (g_gui.secondary_map != nullptr && !options.ingame) {
		...
		for (; it != end; ++it) {
			...
			if (view.zoom <= 10.0 || !options.hide_items_when_zoomed) {
				ItemVector::iterator item_it;
				for (item_it = tile->items.begin(); item_it != tile->items.end(); item_it++) {
					if ((*item_it)->isBorder()) {
						item_drawer->BlitItem(sprite_batch, primitive_renderer, sprite_drawer, creature_drawer, draw_x, draw_y, tile, *item_it, options, true, 160, r, g, b);
					} else {
						item_drawer->BlitItem(sprite_batch, primitive_renderer, sprite_drawer, creature_drawer, draw_x, draw_y, tile, *item_it, options, true, 160, 160, 160, 160);
					}
				}
