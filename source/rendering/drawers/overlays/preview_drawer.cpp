			...
			for (int map_y = view.start_y; map_y <= view.end_y; map_y++) {
				...
				if (tile) {
					...
					if (tile->ground) {
						...
						if (options.show_special_tiles && testFlags(tile->getMapFlags(), TileMapFlag::PvpZone)) {
							r = r / 3 * 2;
							b = r / 3 * 2;
						}
						if (options.show_special_tiles && testFlags(tile->getMapFlags(), TileMapFlag::NoLogout)) {
							b /= 2;
						}
						if (options.show_special_tiles && testFlags(tile->getMapFlags(), TileMapFlag::NoPvp)) {
							g /= 2;
						}
						if (tile->ground) {
