...
void BrushOverlayDrawer::draw(const DrawContext &ctx, MapDrawer *drawer,
  ...
  if (drawer->canvas->drawing_controller->IsDraggingDraw()) {
    ...
    if (brush->is<WallBrush>()) {
      int last_click_start_map_x =
          std::min(drawer->canvas->last_click_map_x, ctx.view.mouse_map_x);
      int last_click_start_map_y =
          std::min(drawer->canvas->last_click_map_y, ctx.view.mouse_map_y);
      int last_click_end_map_x =
          std::max(drawer->canvas->last_click_map_x, ctx.view.mouse_map_x) + 1;
      int last_click_end_map_y =
          std::max(drawer->canvas->last_click_map_y, ctx.view.mouse_map_y) + 1;
      ...
      if (g_gui.atlas_lifecycle.ensureAtlasManager()) {
        const AtlasManager &atlas = *g_gui.atlas_lifecycle.getAtlasManager();
        // Top
        ...
        if (delta_x > TILE_SIZE && delta_y > TILE_SIZE) {
          float h = (last_click_end_sy - TILE_SIZE) -
                    (last_click_start_sy + TILE_SIZE);
          ctx.sprite_batch.drawRect(
      ...
      if (g_gui.GetBrushShape() == BRUSHSHAPE_SQUARE ||
        ...
        if (brush->is<RAWBrush>() || brush->is<OptionalBorderBrush>()) {
              ...
              if (brush->is<OptionalBorderBrush>()) {
                if (g_gui.atlas_lifecycle.ensureAtlasManager()) {
                  const AtlasManager &atlas =
                      *g_gui.atlas_lifecycle.getAtlasManager();
                  ctx.sprite_batch.drawRect(
          ...
          int last_click_start_map_y =
              std::min(drawer->canvas->last_click_map_y, ctx.view.mouse_map_y);
          int last_click_end_map_x =
              std::max(drawer->canvas->last_click_map_x, ctx.view.mouse_map_x) +
              1;
          int last_click_end_map_y =
              std::max(drawer->canvas->last_click_map_y, ctx.view.mouse_map_y) +
              1;
          ...
          if (g_gui.atlas_lifecycle.ensureAtlasManager()) {
            ...
            if (autoborder_active) {
              ...
              ctx.sprite_batch.drawRect(
                  static_cast<float>(last_click_start_sx),
                  static_cast<float>(last_click_start_sy + thickness),
                  thickness, h - 2 * thickness, brushColor, atlas);
              // Right
              ctx.sprite_batch.drawRect(
              ...
              ctx.sprite_batch.drawRect(
                  static_cast<float>(last_click_start_sx),
                  static_cast<float>(last_click_start_sy), w, h, brushColor,
                  *g_gui.atlas_lifecycle.getAtlasManager());
            }
          }
        }
              ...
              if (brush->is<RAWBrush>()) {
                ...
                if (brush->is<WaypointBrush>()) {
                  ...
                  get_color(brush, editor,
                            ...
                            r, g, b);
                  drawer->brush_cursor_drawer->draw(ctx, cx, cy, brush, r, g,
                                                    b);
                } else {
