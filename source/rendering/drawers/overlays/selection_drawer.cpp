...
void SelectionDrawer::draw(const DrawContext &ctx, const MapCanvas *canvas) {
  ...
  const float last_click_rx =
      static_cast<float>(canvas->last_click_abs_x - ctx.view.view_scroll_x);
  const float last_click_ry =
      static_cast<float>(canvas->last_click_abs_y - ctx.view.view_scroll_y);
