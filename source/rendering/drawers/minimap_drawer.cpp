...
void MinimapDrawer::Draw(wxDC &pdc, const wxSize &size, Editor &editor,
                         MapCanvas *canvas) {
  ...
  // If MinimapWindow is a wxWindow (not GLCanvas), we cannot just issue GL
  // commands!
  ...
  // Note: Resizing texture every frame if map resizes is bad, but map resize is
  // rare.
  ...
  if (g_gui.IsRenderingEnabled()) {
    ...
    // For now, updating the visible window 60 times a second via PBO is WAY
    // faster than DrawPoint.
    renderer->updateRegion(editor.map, floor, start_x, start_y, map_draw_w,
                           map_draw_h);

    // Render
    renderer->render(projection, 0, 0, window_width, window_height,
                     (float)start_x, (float)start_y, (float)map_draw_w,
                     (float)map_draw_h);
    ...
    if (g_settings.getInteger(Config::MINIMAP_VIEW_BOX)) {
      ...
      // Bottom
      primitive_renderer->drawLine(glm::vec2(x, y + h), glm::vec2(x + w, y + h),
                                   color);
      // Left
      primitive_renderer->drawLine(glm::vec2(x, y), glm::vec2(x, y + h), color);
      // Right
      primitive_renderer->drawLine(glm::vec2(x + w, y), glm::vec2(x + w, y + h),
                                   color);
...
void MinimapDrawer::ScreenToMap(int screen_x, int screen_y, int &map_x,
                                int &map_y) {
