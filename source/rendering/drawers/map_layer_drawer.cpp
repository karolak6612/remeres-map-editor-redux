...
void MapLayerDrawer::DrawSingleNode(MapNode* nd, int nd_map_x, int nd_map_y, int map_z, bool live, SpriteBatch& sprite_batch, const RenderView& view, const DrawingOptions& options, LightBuffer& light_buffer, int base_screen_x, int base_screen_y, bool draw_lights) {
...
void MapLayerDrawer::DrawNodes(SpriteBatch& sprite_batch, int map_z, bool live_client, const RenderView& view, const DrawingOptions& options, LightBuffer& light_buffer, const std::vector<VisibleNode>& nodes) {
	...
	for (const auto& node : nodes) {
		DrawSingleNode(node.node, node.x, node.y, map_z, live_client, sprite_batch, view, options, light_buffer, base_screen_x, base_screen_y, draw_lights);
	}
}
	...
	if (live_client) {
		for (int nd_map_x = nd_start_x; nd_map_x <= nd_end_x; nd_map_x += 4) {
			for (int nd_map_y = nd_start_y; nd_map_y <= nd_end_y; nd_map_y += 4) {
				...
				DrawSingleNode(nd, nd_map_x, nd_map_y, map_z, true, sprite_batch, view, options, light_buffer, base_screen_x, base_screen_y, draw_lights);
			}
		}
	} else {
		...
		editor->map.visitLeaves(safe_start_x, safe_start_y, safe_end_x, safe_end_y, [&](MapNode* nd, int nd_map_x, int nd_map_y) {
			DrawSingleNode(nd, nd_map_x, nd_map_y, map_z, false, sprite_batch, view, options, light_buffer, base_screen_x, base_screen_y, draw_lights);
		});
	}
}
