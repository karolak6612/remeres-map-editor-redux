...
void MapLayerDrawer::Draw(const DrawContext &ctx,
  ...
  if (live_client) {
    ...
    // Use SpatialHashGrid::visitLeaves which handles O(1) viewport query
    // internally Expand the query range slightly to handle the 4-tile alignment
    // and safety margin
    int safe_start_x =
        nd_start_x - PAINTERS_ALGORITHM_SAFETY_MARGIN_PIXELS / TILE_SIZE;
    int safe_start_y =
        nd_start_y - PAINTERS_ALGORITHM_SAFETY_MARGIN_PIXELS / TILE_SIZE;
    int safe_end_x =
        nd_end_x + PAINTERS_ALGORITHM_SAFETY_MARGIN_PIXELS / TILE_SIZE;
    int safe_end_y =
