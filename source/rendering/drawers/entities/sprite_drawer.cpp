...
void SpriteDrawer::glBlitAtlasQuad(RenderList& list, int sx, int sy, const AtlasRegion* region, DrawColor color) {
	if (region) {
		float nr = color.r / 255.0f;
		float ng = color.g / 255.0f;
		float nb = color.b / 255.0f;
		float na = color.a / 255.0f;
		list.addSprite(static_cast<float>(sx), static_cast<float>(sy), static_cast<float>(TILE_SIZE), static_cast<float>(TILE_SIZE), *region, nr, ng, nb, na);
	}
}

void SpriteDrawer::glBlitSquare(RenderList& list, int sx, int sy, DrawColor color, int size) {
	if (size == 0) {
		size = TILE_SIZE;
	}
	float nr = color.r / 255.0f;
	...
	if (g_gui.gfx.hasAtlasManager()) {
		list.addRect(static_cast<float>(sx), static_cast<float>(sy), static_cast<float>(size), static_cast<float>(size), nr, ng, nb, na, g_gui.gfx.getAtlasManager()->getWhitePixel());
	}
}

void SpriteDrawer::glDrawBox(RenderList& list, int sx, int sy, int width, int height, DrawColor color) {
	float nr = color.r / 255.0f;
	float ng = color.g / 255.0f;
	float nb = color.b / 255.0f;
	float na = color.a / 255.0f;
	...
	if (g_gui.gfx.hasAtlasManager()) {
		list.addRectLines(static_cast<float>(sx), static_cast<float>(sy), static_cast<float>(width), static_cast<float>(height), nr, ng, nb, na, g_gui.gfx.getAtlasManager()->getWhitePixel());
	}
}

void SpriteDrawer::BlitSprite(RenderList& list, int screenx, int screeny, uint32_t spriteid, DrawColor color) {
	GameSprite* spr = g_items[spriteid].sprite;
	if (spr == nullptr) {
		return;
	}
	BlitSprite(list, screenx, screeny, spr, color);
}

void SpriteDrawer::BlitSprite(RenderList& list, int screenx, int screeny, GameSprite* spr, DrawColor color) {
	if (spr == nullptr) {
		return;
	}
	screenx -= spr->getDrawOffset().first;
	...
	for (int cx = 0; cx != spr->width; ++cx) {
		for (int cy = 0; cy != spr->height; ++cy) {
			for (int cf = 0; cf != spr->layers; ++cf) {
				const AtlasRegion* region = spr->getAtlasRegion(cx, cy, cf, -1, 0, 0, 0, tme);
				if (region) {
					glBlitAtlasQuad(list, screenx - cx * TILE_SIZE, screeny - cy * TILE_SIZE, region, color);
				}
			}
		}
	}
