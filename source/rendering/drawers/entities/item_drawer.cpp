...
void ItemDrawer::BlitItem(
    const DrawContext& ctx, SpriteDrawer* sprite_drawer, CreatureDrawer* creature_drawer, int& draw_x, int& draw_y,
...
)
{
    ...
    bool is_transient_selected
        = !ephemeral && options.transient_selection_bounds && options.transient_selection_bounds->contains(pos.x, pos.y);
    ...
    if (it.isMetaItem() || metadata == nullptr || (!ephemeral && it.pickupable && !options.show_items)) {
        return;
    }
    ...
    if (!ephemeral && options.transparent_items && (!it.isGroundTile() || metadata->width > 1 || metadata->height > 1) && !it.isSplash()
    ...
    if (metadata->width == 1 && metadata->height == 1 && metadata->layers == 1) {
        ...
        if (subtype == -1 && pattern_x == 0 && pattern_y == 0 && pattern_z == 0 && frame == 0) {
            region = atlas.getAtlasRegion(clientID, *metadata, 0, 0, 0, -1, 0, 0, 0, 0);
        } else {
            region = atlas.getAtlasRegion(clientID, *metadata, 0, 0, 0, subtype, pattern_x, pattern_y, pattern_z, frame);
        }
        ...
        if (region) {
            ...
            if (item->getID() == 369) {
                ...
                uint32_t precise_expected_id = atlas.getSpriteId(*metadata, frame, 0, 0);
        ...
        for (int cx = 0; cx != metadata->width; cx++) {
            for (int cy = 0; cy != metadata->height; cy++) {
                for (int cf = 0; cf != metadata->layers; cf++) {
                    const AtlasRegion* region
                        = atlas.getAtlasRegion(clientID, *metadata, cx, cy, cf, subtype, pattern_x, pattern_y, pattern_z, frame);
                    if (region) {
                        sprite_drawer->glBlitAtlasQuad(
                            ctx, screenx - cx * TILE_SIZE, screeny - cy * TILE_SIZE, region, DrawColor(red, green, blue, alpha)
                        );
                    }
                }
            }
    ...
    if (it.isPodium()) {
        ...
        creature_drawer->BlitCreature(
            ctx, sprite_drawer, draw_x, draw_y, outfit, static_cast<Direction>(podium->getDirection()),
    ...
    if (!options.ingame && options.show_light_str) {
        ...
        if (light.intensity > 0) {
            ...
            sprite_drawer->glBlitSquare(
                ctx, draw_x + startOffset - 1, draw_y + startOffset - 1, DrawColor(byteR, byteG, byteB, byteA), sqSize
            );
        }
    }
}
...
void ItemDrawer::DrawRawBrush(
    const DrawContext& ctx, SpriteDrawer* sprite_drawer, int screenx, int screeny, ItemType* itemType, uint8_t r, uint8_t g, uint8_t b,
    uint8_t alpha
)
{
    ...
    sprite_drawer->BlitSprite(ctx, screenx, screeny, clientID, DrawColor(r, g, b, alpha));
}
