...
void ItemDrawer::BlitItem(const DrawContext &ctx, SpriteDrawer *sprite_drawer,
  ...
  bool is_transient_selected =
      !ephemeral && options.transient_selection_bounds &&
      options.transient_selection_bounds->contains(pos.x, pos.y);
  ...
  if (spr->width == 1 && spr->height == 1 && spr->layers == 1) {
    ...
    if (subtype == -1 && pattern_x == 0 && pattern_y == 0 && pattern_z == 0 &&
      ...
      region = spr->getAtlasRegion(0, 0, 0, subtype, pattern_x, pattern_y,
                                   pattern_z, frame);
    }
    ...
    for (int cx = 0; cx != spr->width; cx++) {
      for (int cy = 0; cy != spr->height; cy++) {
        for (int cf = 0; cf != spr->layers; cf++) {
          const AtlasRegion *region = spr->getAtlasRegion(
              cx, cy, cf, subtype, pattern_x, pattern_y, pattern_z, frame);
          if (region) {
...
void ItemDrawer::DrawRawBrush(const DrawContext &ctx,
  ...
  sprite_drawer->BlitSprite(ctx, screenx, screeny, spr,
