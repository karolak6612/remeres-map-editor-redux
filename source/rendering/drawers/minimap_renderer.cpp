...
bool MinimapRenderer::initialize() {
  ...
  if (!shared_geometry_.initialize()) {
  ...
  glNamedBufferStorage(instance_vbo_->GetID(),
                       instance_vbo_capacity_ * sizeof(InstanceData), nullptr,
  ...
  glVertexArrayVertexBuffer(vao, 0, shared_geometry_.getQuadVBO(), 0,
                            4 * sizeof(float));
  glVertexArrayElementBuffer(vao, shared_geometry_.getQuadEBO());
  ...
  glVertexArrayAttribFormat(vao, 2, 4, GL_FLOAT, GL_FALSE,
                            offsetof(InstanceData, x));
  glVertexArrayAttribBinding(vao, 2, 1);

  // Attribute 3: Layer
  glEnableVertexArrayAttrib(vao, 3);
  glVertexArrayAttribFormat(vao, 3, 1, GL_FLOAT, GL_FALSE,
                            offsetof(InstanceData, layer));
...
void MinimapRenderer::createPaletteTexture() {
  ...
  glTextureSubImage1D(palette_texture_id_->GetID(), 0, 0, 256, GL_RGBA,
                      GL_UNSIGNED_BYTE, palette.data());

  glTextureParameteri(palette_texture_id_->GetID(), GL_TEXTURE_MIN_FILTER,
                      GL_NEAREST);
  glTextureParameteri(palette_texture_id_->GetID(), GL_TEXTURE_MAG_FILTER,
                      GL_NEAREST);
  glTextureParameteri(palette_texture_id_->GetID(), GL_TEXTURE_WRAP_S,
                      GL_CLAMP_TO_EDGE);
}
...
void MinimapRenderer::resize(int width, int height) {
  ...
  if (error != GL_NO_ERROR) {
    spdlog::error(
        "MinimapRenderer: FAILED to create texture array {}x{}x{}! Error: {}",
        TILE_SIZE, TILE_SIZE, num_layers, error);
  }
  ...
  glTextureParameteri(texture_id_->GetID(), GL_TEXTURE_WRAP_S,
                      GL_CLAMP_TO_EDGE);
  glTextureParameteri(texture_id_->GetID(), GL_TEXTURE_WRAP_T,
                      GL_CLAMP_TO_EDGE);
  spdlog::info("MinimapRenderer: Resized tiled texture to {}x{} ({}x{} tiles, "
               "{} layers)",
               width_, height_, cols_, rows_, num_layers);
}

void MinimapRenderer::updateRegion(const Map &map, int floor, int x, int y,
                                   int w, int h) {
  if (!texture_id_ || !pbo_) {
    return;
  }
  ...
  pending_updates.reserve((end_row - start_row + 1) *
                          (end_col - start_col + 1));
  ...
  for (int r = start_row; r <= end_row; ++r) {
    for (int c = start_col; c <= end_col; ++c) {
      ...
      pending_updates.push_back({.layer = layer,
                                 .dest_x_in_tile = offset_x,
                                 .dest_y_in_tile = offset_y,
                                 .map_start_x = int_x,
                                 .map_start_y = int_y,
                                 .w = update_w,
                                 .h = update_h,
                                 .pbo_offset = total_buffer_needed});
  ...
  if (pbo_->getSize() < total_buffer_needed) {
    pbo_->cleanup();
    pbo_->initialize(
        std::max(total_buffer_needed, pbo_->getSize() * 2)); // Growth strategy
  }
  ...
  for (const auto &update : pending_updates) {
    ...
    for (int dy = 0; dy < update.h; ++dy) {
      for (int dx = 0; dx < update.w; ++dx) {
        const Tile *tile = map.getTile(update.map_start_x + dx,
                                       update.map_start_y + dy, floor);
        if (tile) {
  ...
  for (const auto &update : pending_updates) {
    glTextureSubImage3D(texture_id_->GetID(), 0, update.dest_x_in_tile,
                        update.dest_y_in_tile, update.layer, update.w, update.h,
                        1, GL_RED_INTEGER, GL_UNSIGNED_BYTE,
                        reinterpret_cast<void *>(update.pbo_offset));
  }
...
void MinimapRenderer::render(const glm::mat4 &projection, int x, int y, int w,
                             int h, float map_x, float map_y, float map_w,
                             float map_h) {
  if (!texture_id_) {
    return;
  }
  ...
  for (int r = start_row; r <= end_row; ++r) {
    for (int c = start_col; c <= end_col; ++c) {
      ...
      instance_data_.push_back({.x = screen_tile_x,
                                .y = screen_tile_y,
                                .w = screen_tile_w,
                                .h = screen_tile_h,
  ...
  if (instance_data_.size() > instance_vbo_capacity_) {
    ...
    // Update VAO binding
    glVertexArrayVertexBuffer(vao_->GetID(), 1, instance_vbo_->GetID(), 0,
                              sizeof(InstanceData));
  }

  // Upload data
  glNamedBufferSubData(instance_vbo_->GetID(), 0,
  ...
  // Render
  {
    ...
    glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0,
                            static_cast<GLsizei>(instance_data_.size()));
    glBindVertexArray(0);
  }
}
