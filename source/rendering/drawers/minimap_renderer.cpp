...
bool MinimapRenderer::initialize() {
	...
	// Instance Attributes (2, 3)
	// Bind instance buffer to binding point 1 with divisor 1
	glVertexArrayBindingDivisor(vao_, 1, 1);

	// Loc 2: Rect (vec4)
	glEnableVertexArrayAttrib(vao_, 2);
	glVertexArrayAttribFormat(vao_, 2, 4, GL_FLOAT, GL_FALSE, 0);
	glVertexArrayAttribBinding(vao_, 2, 1);

	// Loc 3: Layer (float)
	glEnableVertexArrayAttrib(vao_, 3);
	glVertexArrayAttribFormat(vao_, 3, 1, GL_FLOAT, GL_FALSE, 4 * sizeof(float));
...
void MinimapRenderer::render(const glm::mat4& projection, int x, int y, int w, int h, float map_x, float map_y, float map_w, float map_h) {
	...
	for (int r = start_row; r <= end_row; ++r) {
		for (int c = start_col; c <= end_col; ++c) {
			...
			// Push Rect (x,y,w,h) and Layer
			instance_data_.push_back(screen_tile_x);
			instance_data_.push_back(screen_tile_y);
			instance_data_.push_back(screen_tile_w);
			instance_data_.push_back(screen_tile_h);
			instance_data_.push_back((float)layer);
		}
	}
	...
	// Note: Re-allocating dynamic buffer every frame for simplicity.
	// For production, a ring buffer or persistent mapping would be better,
	// but this is already vastly superior to loop-draw.
	glNamedBufferData(instance_vbo_, instance_data_.size() * sizeof(float), instance_data_.data(), GL_DYNAMIC_DRAW);

	// Bind buffer to VAO binding point 1 (if not done in init, but we did)
	// However, we need to bind it again if we deleted/recreated it? No, name is constant.
	// But we DO need to ensure the binding point is linked to the buffer.
	glVertexArrayVertexBuffer(vao_, 1, instance_vbo_, 0, 5 * sizeof(float));

	glBindVertexArray(vao_);
	glDrawArraysInstanced(GL_TRIANGLE_FAN, 0, 4, (GLsizei)(instance_data_.size() / 5));
	glBindVertexArray(0);
}
