...
wxBitmap SpriteIconGenerator::Generate(GameSprite *sprite, SpriteSize size,
  ...
  int image_size =
      std::max<uint8_t>(sprite->width, sprite->height) * SPRITE_PIXELS;
    ...
    for (uint8_t w = 0; w < sprite->width; w++) {
      for (uint8_t h = 0; h < sprite->height; h++) {
        ...
        if (data) {
          ...
          image.Paste(img, (sprite->width - w - 1) * SPRITE_PIXELS,
                      (sprite->height - h - 1) * SPRITE_PIXELS);
        }
      }
    }
  }
  ...
  if (rescale &&
      (size == SPRITE_SIZE_16x16 || size == SPRITE_SIZE_64x64 ||
       image.GetWidth() > SPRITE_PIXELS || image.GetHeight() > SPRITE_PIXELS)) {
    int new_size = 32;
...
wxBitmap SpriteIconGenerator::Generate(GameSprite *sprite, SpriteSize size,
                                       const Outfit &outfit, bool rescale,
                                       Direction direction) {
  ASSERT(sprite->width >= 1 && sprite->height >= 1);
        ...
        for (uint8_t w = 0; w < mountSpr->width; w++) {
          for (uint8_t h = 0; h < mountSpr->height; h++) {
            ...
            if (mountSpr->layers == 2) {
              ...
              data = mountSpr
                         ->getTemplateImage(
                             mountSpr->getIndex(w, h, 0, mount_frame_index, 0,
                                                0, 0),
                             mountOutfit)
                         ->getRGBData();
            } else {
              // Standard mount
              data = mountSpr
                         ->spriteList[mountSpr->getIndex(
                             w, h, l, mount_frame_index, 0, 0, 0)]
                         ->getRGBData();
            }
            ...
            if (data) {
              ...
              img.SetMaskColour(0xFF, 0x00, 0xFF);
              // Mount offset
              int mount_x = (sprite->width - w - 1) * SPRITE_PIXELS -
                            mountSpr->getDrawOffset().first;
              int mount_y = (sprite->height - h - 1) * SPRITE_PIXELS -
                            mountSpr->getDrawOffset().second;
      ...
      for (uint8_t w = 0; w < sprite->width; w++) {
        for (uint8_t h = 0; h < sprite->height; h++) {
          ...
          if (sprite->layers == 2) {
            ...
            data = sprite
                       ->getTemplateImage(sprite->getIndex(w, h, 0, frame_index,
                                                           pattern_y, pattern_z,
                                                           0),
                                          outfit)
                       ->getRGBData();
            ...
            if (l == 0) {
              data = sprite
                         ->getTemplateImage(
                             sprite->getIndex(w, h, 0, frame_index, pattern_y,
                                              pattern_z, 0),
                             outfit)
                         ->getRGBData();
            }
            if (l == 2) {
              data = sprite
                         ->getTemplateImage(
                             sprite->getIndex(w, h, 2, frame_index, pattern_y,
                                              pattern_z, 0),
                             outfit)
                         ->getRGBData();
            }
            ...
            data = sprite
                       ->spriteList[sprite->getIndex(w, h, l, frame_index,
                                                     pattern_y, pattern_z, 0)]
                       ->getRGBData();
          }
