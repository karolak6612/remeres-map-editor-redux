...
void LightDrawer::draw(
    const ViewState& view, bool fog, const LightBuffer& light_buffer, const DrawColor& global_color, float light_intensity,
...
)
{
    ...
    for (size_t i = 0; i < light_buffer.size(); ++i) {
        ...
        // Frustum culling
        if (screen_x + screen_radius < 0 || screen_x - screen_radius > buffer_w || screen_y + screen_radius < 0
            || screen_y - screen_radius > buffer_h) {
            continue;
        }
        ...
        gpu_lights_.push_back(
            {.position = {screen_x, screen_y},
             .intensity = static_cast<float>(light_buffer.intensity[i]),
             .padding = 0.0f,
             .color
             = {(c.Red() / 255.0f) * light_intensity, (c.Green() / 255.0f) * light_intensity, (c.Blue() / 255.0f) * light_intensity, 1.0f}}
        );
    }
