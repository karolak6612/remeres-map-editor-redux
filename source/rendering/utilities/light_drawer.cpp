...
void LightDrawer::draw(const DrawContext &ctx) {
  ...
  if (gpu_lights_.empty()) {
    ...
    glNamedBufferSubData(light_ssbo->GetID(), 0, needed_size,
                         gpu_lights_.data());
  }
  ...
  // 3. Render to FBO
  {
    ...
    if (!gpu_lights_.empty()) {
      ...
      // Enable MAX blending
      {
        ...
        ScopedGLBlend blendState(GL_ONE, GL_ONE,
                                 GL_MAX); // Factors don't matter much for MAX,
                                          // but usually 1,1 is safe
  ...
  // Actually MapDrawer doesn't seem to set viewport every time, but
  // `view.projectionMatrix` assumes 0..screensize.

  // Use PrimitiveRenderer or just a simple quad?
  // We can use a simple Blit shader or re-use `sprite_drawer->glBlitSquare` if
  // it supports textures. Or just do a quick manual draw using fixed pipeline
  // or a simple shader. Let's use `glBlitNamedFramebuffer`? No, we need
  // blending (Multiply). So we draw a textured quad.

  // We can reuse the `shader` but we need a "PASS THROUGH" mode or a separate
  // shader. Easier to just use `glEnable(GL_TEXTURE_2D)` and fixed function if
  // compatible? No, we are in Core Profile likely. Let's assume we need a
  // simple texture shader. BUT wait, `LightDrawer::draw` previously drew a quad
  // with the computed light. We can just use a simple "Texture Shader" here.
  ...
  // Let's use the local `shader` with a "Mode" switch? Or just a second tiny
  // shader. Adding a mode switch to `shader` is easiest. "uMode": 0 = Light
  // Render, 1 = Composite.
  ...
  shader->SetMat4("uProjection", view.projectionMatrix * view.viewMatrix *
