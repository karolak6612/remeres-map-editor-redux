...
void LightDrawer::draw(const RenderView& view, bool fog, const LightBuffer& light_buffer, const wxColor& global_color, float light_intensity, float ambient_light_level) {
	...
	// 2. Prepare Lights
	// Filter and convert lights to GPU format directly into mapped buffer
	size_t worst_case_size = light_buffer.lights.size() * sizeof(GPULight);

	if (worst_case_size > light_ssbo_capacity_) {
		light_ssbo_capacity_ = std::max(worst_case_size, static_cast<size_t>(light_ssbo_capacity_ * 1.5));
		if (light_ssbo_capacity_ < 1024) {
			light_ssbo_capacity_ = 1024;
		}
		// Destroy and recreate buffer for Immutable Storage with GL_MAP_WRITE_BIT
	...
	if (worst_case_size > 0) {
		GPULight* mapped_lights = static_cast<GPULight*>(glMapNamedBufferRange(
			light_ssbo->GetID(),
			0,
			worst_case_size,
	...
	// 3. Render to FBO
	{
		...
		if (active_lights > 0) {
			...
			// Enable MAX blending
			{
				...
				if (active_lights > static_cast<size_t>(std::numeric_limits<GLsizei>::max())) {
					spdlog::error("Too many lights for glDrawArraysInstanced");
					return;
				}
				glDrawArraysInstanced(GL_TRIANGLE_FAN, 0, 4, static_cast<GLsizei>(active_lights));
			}
