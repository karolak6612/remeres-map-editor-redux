...
void LightDrawer::computeBrightness(const RenderView& view, const LightBuffer& light_buffer, const DrawingOptions& options) {
	...
	tile_brightness_.assign(tile_count * 4, 0);
	...
	for (size_t light_index = 0; light_index < light_buffer.lights.size(); ++light_index) {
		...
		for (int ty = min_ty; ty <= max_ty; ++ty) {
			for (int tx = min_tx; tx <= max_tx; ++tx) {
				...
				const uint8_t light_r = static_cast<uint8_t>(std::clamp(std::lround(light_color.r * factor * 255.0f), 0l, 255l));
				const uint8_t light_g = static_cast<uint8_t>(std::clamp(std::lround(light_color.g * factor * 255.0f), 0l, 255l));
				const uint8_t light_b = static_cast<uint8_t>(std::clamp(std::lround(light_color.b * factor * 255.0f), 0l, 255l));
