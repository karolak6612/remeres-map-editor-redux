...
std::vector<std::string> PostProcessManager::GetAvailableEffectNames() {
  return {ShaderNames::NONE, ShaderNames::SCANLINE, ShaderNames::XBRZ};
}

void PostProcessManager::Register(const std::string &name,
  ...
  effects.push_back(std::make_shared<PostProcessEffect>(name, fragment_source,
                                                        vertex_source));
}
...
void PostProcessManager::Initialize(const std::string &default_vertex_source) {
  ...
  for (auto &effect : effects) {
    ...
    if (!effect->shader->Load(v_source, effect->fragment_source)) {
      spdlog::error("PostProcessManager: Failed to load shader '{}'",
                    effect->name);
  ...
  effects.erase(
      std::remove_if(effects.begin(), effects.end(),
                     [](const std::shared_ptr<PostProcessEffect> &effect) {
