...
DrawingController::DrawingController(MapCanvas* canvas, Editor& editor) :
    canvas(canvas), editor(editor), drawing(false), dragging_draw(false), replace_dragging(false), last_draw_pos(Position(-1, -1, -1))
{
}
            ...
            if (brush->is<SpawnBrush>()) {
                ...
                if (brush->is<WallBrush>()) {
                    ...
                    if (g_gui.GetBrushShape() == BRUSHSHAPE_SQUARE) {
                        ...
                        int width = std::max(
                            std::abs(
                                std::max(mouse_map_pos.y, canvas->GetLastClickMapY())
                                - std::min(mouse_map_pos.y, canvas->GetLastClickMapY())
                            ),
                            std::abs(
                                std::max(mouse_map_pos.x, canvas->GetLastClickMapX())
                                - std::min(mouse_map_pos.x, canvas->GetLastClickMapX())
                            )
                        );
