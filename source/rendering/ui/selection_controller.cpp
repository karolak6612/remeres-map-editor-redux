        ...
        if (dragging && (move_x != 0 || move_y != 0 || move_z != 0)) {
            ...
            if (boundbox_selection) {
                if (mouse_map_pos.x == canvas->GetLastClickMapX() && mouse_map_pos.y == canvas->GetLastClickMapY() && ctrl_down) {
                    ...
                    ExecuteBoundboxSelection(
                        Position(canvas->GetLastClickMapX(), canvas->GetLastClickMapY(), canvas->GetLastClickMapZ()), mouse_map_pos,
                        mouse_map_pos.z
                    );
                }
            } else if (ctrl_down) {
...
void SelectionController::HandlePropertiesRelease(const Position& mouse_map_pos, bool shift_down, bool ctrl_down, bool alt_down)
{
    ...
    if (boundbox_selection) {
        if (mouse_map_pos.x == canvas->GetLastClickMapX() && mouse_map_pos.y == canvas->GetLastClickMapY() && ctrl_down) {
            ...
            ExecuteBoundboxSelection(
                Position(canvas->GetLastClickMapX(), canvas->GetLastClickMapY(), canvas->GetLastClickMapZ()), mouse_map_pos, mouse_map_pos.z
            );
        }
    } else if (ctrl_down) {
        // Nothing
...
void SelectionController::ExecuteBoundboxSelection(const Position& start_pos, const Position& end_pos, int floor)
{
    ...
    if (width == 0) {
        ...
        for (int i = 0; i < threadcount; ++i) {
            ...
            threads.push_back(
                std::make_unique<SelectionThread>(editor, Position(s_x + cleared, s_y, s_z), Position(s_x + cleared + chunksize, e_y, e_z))
            );
            cleared += chunksize;
            remainder -= chunksize;
        }
