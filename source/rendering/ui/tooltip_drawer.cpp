...
void TooltipDrawer::prepareFields(const TooltipData& tooltip) {
	...
	scratch_fields.reserve(32);

	auto add_numeric_field = [&](std::string_view label, auto num_value, uint8_t r, uint8_t g, uint8_t b) {
		if (active_field_count >= scratch_fields.size()) {
			scratch_fields.emplace_back();
		}
		FieldLine& f = scratch_fields[active_field_count++];
		f.label = label;
		f.storage.clear();
		fmt::format_to(std::back_inserter(f.storage), "{}", num_value);
		f.external_value = {}; // Clear external view since we use storage
		f.r = r;
		f.g = g;
		f.b = b;
		f.wrappedLines.clear();
	};

	auto add_text_field = [&](std::string_view label, std::string_view val, uint8_t r, uint8_t g, uint8_t b) {
		if (active_field_count >= scratch_fields.size()) {
			scratch_fields.emplace_back();
		}
		FieldLine& f = scratch_fields[active_field_count++];
		f.label = label;
		f.external_value = val;
		f.storage.clear(); // Ensure storage is empty so getValue() returns external_value
		f.r = r;
		f.g = g;
		f.b = b;
		f.wrappedLines.clear();
	...
	if (tooltip.category == TooltipCategory::WAYPOINT) {
		add_text_field("Waypoint", tooltip.waypointName, WAYPOINT_HEADER_R, WAYPOINT_HEADER_G, WAYPOINT_HEADER_B);
	} else {
		if (tooltip.actionId > 0) {
			add_numeric_field("Action ID", tooltip.actionId, ACTION_ID_R, ACTION_ID_G, ACTION_ID_B);
		}
		if (tooltip.uniqueId > 0) {
			add_numeric_field("Unique ID", tooltip.uniqueId, UNIQUE_ID_R, UNIQUE_ID_G, UNIQUE_ID_B);
		}
		if (tooltip.doorId > 0) {
			add_numeric_field("Door ID", tooltip.doorId, DOOR_ID_R, DOOR_ID_G, DOOR_ID_B);
		}
		if (tooltip.destination.x > 0) {
			...
			f.label = "Destination";
			f.storage.clear();
			fmt::format_to(std::back_inserter(f.storage), "{}, {}, {}", tooltip.destination.x, tooltip.destination.y, tooltip.destination.z);
			f.external_value = {};
			f.r = TELEPORT_DEST_R;
			f.g = TELEPORT_DEST_G;
			f.b = TELEPORT_DEST_B;
		...
		if (!tooltip.description.empty()) {
			add_text_field("Description", tooltip.description, BODY_TEXT_R, BODY_TEXT_G, BODY_TEXT_B);
		}
		if (!tooltip.text.empty()) {
			// We need quotes for text, so we use storage
			if (active_field_count >= scratch_fields.size()) {
				scratch_fields.emplace_back();
			}
			FieldLine& f = scratch_fields[active_field_count++];
...
TooltipDrawer::LayoutMetrics TooltipDrawer::calculateLayout(NVGcontext* vg, const TooltipData& tooltip, float maxWidth, float minWidth, float padding, float fontSize) {
	...
	for (size_t i = 0; i < active_field_count; ++i) {
	...
	for (size_t i = 0; i < active_field_count; ++i) {
		auto& field = scratch_fields[i];
		std::string_view val = field.getValue();
		const char* start = val.data();
		const char* end = start + val.length();
		...
		if (valueWidth <= maxValueWidth) {
			// Single line
			field.wrappedLines.push_back(val);
			totalLines++;
...
void TooltipDrawer::draw(NVGcontext* vg, const RenderView& view) {
	...
	for (size_t i = 0; i < active_count; ++i) {
		...
		if (active_field_count == 0 && tooltip.containerItems.empty()) {
			continue;
		}
