...
bool TooltipDrawer::FillItemTooltipData(TooltipData& data, const Item* item, const ItemType& it, const Position& pos, bool isHouseTile, float zoom) {
	if (!item) {
		return false;
	}
	...
	if (is_teleport) {
		const Teleport* tp = static_cast<const Teleport*>(item);
