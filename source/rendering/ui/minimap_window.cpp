...
MinimapWindow::MinimapWindow(wxWindow *parent)
    : wxGLCanvas(parent, GetCoreProfileAttributes(), wxID_ANY,
                 wxDefaultPosition, wxSize(205, 130)),
      update_timer(this) {
  spdlog::info(
      "MinimapWindow::MinimapWindow - Creating own context shared with global");
...
MinimapWindow::~MinimapWindow() {
  ...
  if (context_ok) {
    ...
    spdlog::warn("MinimapWindow: Destroying without a current OpenGL context. "
...
void MinimapWindow::OnPaint(wxPaintEvent &event) {
  ...
  if (!gladInitialized) {
    ...
    if (!gladLoadGL()) {
      ...
      spdlog::info("MinimapWindow::OnPaint - GLAD loaded. GL Version: {}",
...
void MinimapWindow::OnMouseClick(wxMouseEvent &event) {
  ...
  g_gui.SetScreenCenterPosition(
      Position(new_map_x, new_map_y, g_gui.GetCurrentFloor()));
  Refresh();
  g_gui.RefreshView();
}
