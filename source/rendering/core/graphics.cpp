...
std::unique_ptr<uint8_t[]> GameSprite::NormalImage::getRGBData() {
	if (id == 0) {
		...
		return std::make_unique<uint8_t[]>(pixels_data_size); // Zero initialized (Black/Empty)
	}
...
std::unique_ptr<uint8_t[]> GameSprite::NormalImage::getRGBAData() {
	...
	if (id == 0) {
		...
		return std::make_unique<uint8_t[]>(pixels_data_size); // Zero initialized (Transparent)
	}
