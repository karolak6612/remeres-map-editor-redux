...
void SpriteDatabase::insertEditorSprite(int id, std::unique_ptr<Sprite> sprite)
{
    if (id < 0) {
...
void SpriteDatabase::DrawItemSprite(int clientID, wxDC* dc, SpriteSize sz, int start_x, int start_y, int width, int height)
{
    if (clientID < 0) {
        ...
        icon_space[clientID].DrawTo(clientID, metadata_space[clientID], dc, sz, start_x, start_y, width, height);
    }
}

void SpriteDatabase::DrawCreatureSprite(
    int lookType, const Outfit& outfit, wxDC* dc, SpriteSize sz, int start_x, int start_y, int width, int height
)
{
    size_t clientID = static_cast<size_t>(lookType) + item_count;
    if (clientID < metadata_space.size()) {
        icon_space[clientID].DrawTo(clientID, metadata_space[clientID], dc, sz, outfit, start_x, start_y, width, height);
    }
}

// --- CreatureSprite implementation ---

CreatureSprite::CreatureSprite(uint32_t clientID, const Outfit& outfit) : clientID(clientID), outfit(outfit) { }
...
{
    g_gui.sprites.DrawCreatureSprite(clientID, outfit, dc, sz, start_x, start_y, width, height);
}
