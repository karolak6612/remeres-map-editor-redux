...
#include "rendering/io/sprite_loader.h" // For SpriteLoader
...
void Image::visit(TextureGC& gc) const {
	lastaccess.store(static_cast<int64_t>(gc.getCachedTime()), std::memory_order_relaxed);
}

void Image::clean(time_t time, int longevity, SpriteDatabase& sprites, TextureGC& gc) {
	// Base implementation does nothing
}

const AtlasRegion* Image::EnsureAtlasSprite(SpriteDatabase* sprites, AtlasManager& atlas_mgr, TextureGC& gc, SpriteLoader& loader, bool use_memcached, uint32_t sprite_id, std::unique_ptr<uint8_t[]> preloaded_data) {
	...
	if (preloaded_data) {
		...
		rgba = getRGBAData(sprites, loader, use_memcached); // TemplateImage passes its own, but we can't easily pass it here without adding SpriteDatabase to EnsureAtlasSprite.
	}
	...
	if (region) {
		if (!isGLLoaded) {
			isGLLoaded = true;
			gc.addResidentImage(handle); // Add to resident set
		}
		return region;
	} else {
