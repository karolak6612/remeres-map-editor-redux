...
#include "app/main.h"
#include "rendering/core/texture_gc.h" // Added this include
#include "ui/gui.h"
...
int Animator::getFrame() {
  long time = g_gui.texture_gc.getElapsedTime();
...
void Animator::setFrame(int frame) {
  ASSERT(frame == -1 || frame == 255 || frame == 254 ||
         (frame >= 0 && frame < frame_count));
  ...
  if (async) {
    ...
    is_complete = false;
    last_time = g_gui.texture_gc.getElapsedTime();
...
int Animator::getPingPongFrame() {
  ...
  if (next_frame < 0 || next_frame >= frame_count) {
    direction =
        direction == ANIMATION_FORWARD ? ANIMATION_BACKWARD : ANIMATION_FORWARD;
    count *= -1;
  }
