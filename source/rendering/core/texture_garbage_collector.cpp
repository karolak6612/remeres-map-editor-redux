...
void TextureGarbageCollector::NotifyTextureLoaded() { loaded_textures++; }

void TextureGarbageCollector::NotifyTextureUnloaded() { loaded_textures--; }
...
void TextureGarbageCollector::AddSpriteToCleanup(GameSprite *spr) {
  ...
  // Clean if needed
  const size_t clean_threshold = static_cast<size_t>(
      std::max(MIN_CLEAN_THRESHOLD,
               g_settings.getInteger(Config::SOFTWARE_CLEAN_THRESHOLD)));
  if (cleanup_list.size() > clean_threshold) {
    const auto software_clean_size =
        std::max(0, g_settings.getInteger(Config::SOFTWARE_CLEAN_SIZE));
    const auto cleanup_count =
        std::min(cleanup_list.size(), static_cast<size_t>(software_clean_size));
...
void TextureGarbageCollector::GarbageCollect(
    ...
    std::vector<void *> &resident_images, time_t current_time) {
  if (g_settings.getInteger(Config::TEXTURE_MANAGEMENT)) {
    if (loaded_textures >
            g_settings.getInteger(Config::TEXTURE_CLEAN_THRESHOLD) &&
        current_time - lastclean >
            g_settings.getInteger(Config::TEXTURE_CLEAN_PULSE)) {
      ...
      for (size_t i = resident_game_sprites.size(); i > 0; --i) {
        ...
        // For now, GameSprites stay resident if they have software DC/animator
        // state This part of the refactor is more subtle; we'll keep it simple
        // first.
      }
...
void TextureGarbageCollector::CleanSoftwareSprites(
