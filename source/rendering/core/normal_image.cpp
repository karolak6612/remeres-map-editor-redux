...
NormalImage::NormalImage()
    : id(0), atlas_region(nullptr), size(0), dump(nullptr) {}

NormalImage::~NormalImage() {
  // dump auto-deleted
  if (isGLLoaded) {
    if (g_gui.atlas_lifecycle.hasAtlasManager()) {
      g_gui.atlas_lifecycle.getAtlasManager()->removeSprite(id);
    }
  }
}
...
void NormalImage::clean(time_t time, int longevity) {
  ...
  if (time - static_cast<time_t>(lastaccess.load(std::memory_order_relaxed)) >
          5 &&
      !g_settings.getInteger(
          Config::USE_MEMCACHED_SPRITES)) { // We keep dumps around for 5
                                            // seconds.
    dump.reset();
  }
}
std::unique_ptr<uint8_t[]> NormalImage::getRGBData() {
  if (id == 0) {
    ...
    return std::make_unique<uint8_t[]>(
        pixels_data_size); // Value-initialized (zeroed)
  }
  ...
  if (!dump) {
    ...
    if (!g_gui.sprite_database.loadSpriteDump(dump, size, id)) {
      return nullptr;
    }
  }
  ...
  uint8_t bpp = g_gui.sprite_database.hasTransparency() ? 4 : RGB_COMPONENTS;
  size_t write = 0;
  size_t read = 0;
  ...
  while (read < size && write < static_cast<size_t>(pixels_data_size)) {
    if (read + 1 >= size) {
      spdlog::warn("NormalImage::getRGBData: Transparency header truncated "
                   "(read={}, size={})",
                   read, size);
      break;
    }
    ...
    read += 2;
    for (int cnt = 0;
         cnt < transparent && write < static_cast<size_t>(pixels_data_size);
         ++cnt) {
    ...
    if (read + 1 >= size) {
      spdlog::warn("NormalImage::getRGBData: Colored header truncated "
                   "(read={}, size={})",
                   read, size);
      break;
    }
    ...
    if (read + static_cast<size_t>(colored) * bpp > size) {
      spdlog::warn("NormalImage::getRGBData: Read buffer overrun (colored={}, "
                   "bpp={}, read={}, size={})",
                   colored, bpp, read, size);
      break;
    }

    for (int cnt = 0;
         cnt < colored && write < static_cast<size_t>(pixels_data_size);
         ++cnt) {
...
std::unique_ptr<uint8_t[]> NormalImage::getRGBAData() {
  ...
  return GameSprite::Decompress(std::span{dump.get(), size},
                                g_gui.sprite_database.hasTransparency(), id);
}

const AtlasRegion *NormalImage::getAtlasRegion() {
  if (isGLLoaded && atlas_region) {
    // Self-Healing: Check for stale atlas region pointer (e.g. from memory
    // reuse) Force reload if Owner is INVALID or DOES NOT MATCH
    if (atlas_region->debug_sprite_id == AtlasRegion::INVALID_SENTINEL ||
        (atlas_region->debug_sprite_id != 0 &&
         atlas_region->debug_sprite_id != id)) {
      spdlog::warn("STALE ATLAS REGION DETECTED: NormalImage {} held region "
                   "owned by {}. Force reloading.",
                   id, atlas_region->debug_sprite_id);
      isGLLoaded = false;
      atlas_region = nullptr;
    } else {
      visit();
