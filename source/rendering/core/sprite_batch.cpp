...
SpriteBatch::SpriteBatch() { pending_sprites_.reserve(MAX_SPRITES_PER_BATCH); }
...
bool SpriteBatch::initialize() {
  ...
  glVertexArrayVertexBuffer(vao_->GetID(), 0, shared_geometry_.getQuadVBO(), 0,
                            4 * sizeof(float));
  glVertexArrayElementBuffer(vao_->GetID(), shared_geometry_.getQuadEBO());
  ...
  glVertexArrayAttribFormat(vao_->GetID(), 1, 2, GL_FLOAT, GL_FALSE,
                            2 * sizeof(float));
  ...
  glVertexArrayVertexBuffer(vao_->GetID(), 1, ring_buffer_.getBufferId(), 0,
  ...
  glVertexArrayAttribFormat(vao_->GetID(), 2, 4, GL_FLOAT, GL_FALSE,
                            offsetof(SpriteInstance, x));
  glVertexArrayAttribBinding(vao_->GetID(), 2, 1);

  // Loc 3: uv (vec4) - instance
  glEnableVertexArrayAttrib(vao_->GetID(), 3);
  glVertexArrayAttribFormat(vao_->GetID(), 3, 4, GL_FLOAT, GL_FALSE,
                            offsetof(SpriteInstance, u_min));
  glVertexArrayAttribBinding(vao_->GetID(), 3, 1);

  // Loc 4: tint (vec4) - instance
  glEnableVertexArrayAttrib(vao_->GetID(), 4);
  glVertexArrayAttribFormat(vao_->GetID(), 4, 4, GL_FLOAT, GL_FALSE,
                            offsetof(SpriteInstance, r));
  glVertexArrayAttribBinding(vao_->GetID(), 4, 1);

  // Loc 5: layer (float) - instance
  glEnableVertexArrayAttrib(vao_->GetID(), 5);
  glVertexArrayAttribFormat(vao_->GetID(), 5, 1, GL_FLOAT, GL_FALSE,
                            offsetof(SpriteInstance, atlas_layer));
  ...
  if (mdi_renderer_.initialize()) {
    ...
    spdlog::warn(
        "SpriteBatch: MDI renderer failed to initialize, using fallback path");
  }
...
void SpriteBatch::begin(const glm::mat4 &projection,
  ...
  // We use emplace to construct the Scoped objects in-place, which saves the
  // previous state
...
void SpriteBatch::draw(float x, float y, float w, float h,
  ...
  if (pending_sprites_.size() >= MAX_SPRITES_PER_BATCH &&
      current_atlas_manager_) {
...
void SpriteBatch::drawRect(float x, float y, float w, float h,
...
void SpriteBatch::drawRectLines(float x, float y, float w, float h,
                                const glm::vec4 &color,
                                const AtlasManager &atlas_manager) {
  // Top
...
void SpriteBatch::flush(const AtlasManager &atlas_manager) {
  ...
  if (use_mdi_) {
    ...
    // If we fill the entire ring buffer (3 sections), we must flush
    // intermediate draws to avoid overwriting the beginning of the buffer
    // before it is executed.

    mdi_renderer_.clear();

    // DSA handles attribute binding, so we don't need to touch
    // glVertexAttribPointer here. baseInstance in the Indirect command handles
    ...
    while (processed < total) {
      ...
      // (If we just flushed above, the fence was placed, and waitAndMap will
      // block correctly if GPU is slow)
      void *ptr = ring_buffer_.waitAndMap(batch_size);
      if (!ptr) {
        spdlog::error("SpriteBatch: RingBuffer mapping failed (MDI path). "
                      "Aborting batch.");
        break;
      }

      // Copy instance data
      memcpy(ptr, pending_sprites_.data() + processed,
      ...
      GLuint baseInstance =
          static_cast<GLuint>(offset / sizeof(SpriteInstance));

      // Add command
      mdi_renderer_.addDrawCommand(6, static_cast<GLuint>(batch_size), 0, 0,
                                   baseInstance);
    ...
    while (processed < total) {
      ...
      void *ptr = ring_buffer_.waitAndMap(batch_size);
      if (!ptr) {
        spdlog::error("SpriteBatch: RingBuffer mapping failed (Fallback path). "
                      "Aborting batch.");
        break;
      }
      ...
      glVertexArrayVertexBuffer(vao_->GetID(), 1, ring_buffer_.getBufferId(),
                                offset, sizeof(SpriteInstance));
