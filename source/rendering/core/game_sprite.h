...
public:
  ...
  virtual void DrawTo(wxDC *dc, SpriteSize sz, int start_x, int start_y,
...
public:
  ...
  void DrawTo(wxDC *dc, SpriteSize sz, int start_x, int start_y, int width = -1,
              int height = -1) override;
  virtual void unloadDC() override;
  wxSize GetSize() const override { return wxSize(32, 32); }
...
public:
  ...
  size_t getIndex(int width, int height, int layer, int pattern_x,
                  int pattern_y, int pattern_z, int frame) const;

  // Phase 2: Get atlas region for texture array rendering
  const AtlasRegion *getAtlasRegion(int _x, int _y, int _layer, int _subtype,
                                    int _pattern_x, int _pattern_y,
                                    int _pattern_z, int _frame);
  const AtlasRegion *getAtlasRegion(int _x, int _y, int _dir, int _addon,
                                    int _pattern_z, const Outfit &_outfit,
                                    int _frame);

  void DrawTo(wxDC *dc, SpriteSize sz, int start_x, int start_y, int width = -1,
              int height = -1) override;
  virtual void DrawTo(wxDC *dc, SpriteSize sz, const Outfit &outfit,
                      int start_x, int start_y, int width = -1,
                      int height = -1);

  void unloadDC() override;

  wxSize GetSize() const override { return wxSize(width * 32, height * 32); }
  ...
  Decompress(std::span<const uint8_t> dump, bool use_alpha, int id = 0);

  static void ColorizeTemplatePixels(uint8_t *dest, const uint8_t *mask,
                                     size_t pixelCount, int lookHead,
                                     int lookBody, int lookLegs, int lookFeet,
                                     bool destHasAlpha);
...
public:
  ...
  std::vector<std::unique_ptr<TemplateImage>>
      instanced_templates; // Templates that use this sprite
  struct CachedDC {
  ...
  struct RenderKey {
    ...
    int lookMount, lookAddon, lookMountHead, lookMountBody, lookMountLegs,
        lookMountFeet;

    bool operator==(const RenderKey &rk) const {
      return size == rk.size && colorHash == rk.colorHash &&
             mountColorHash == rk.mountColorHash && lookMount == rk.lookMount &&
             lookAddon == rk.lookAddon && lookMountHead == rk.lookMountHead &&
             lookMountBody == rk.lookMountBody &&
             lookMountLegs == rk.lookMountLegs &&
             lookMountFeet == rk.lookMountFeet;
    }
  };
  ...
  struct RenderKeyHash {
    size_t operator()(const RenderKey &k) const noexcept {
      ...
      size_t h = std::hash<uint64_t>{}((uint64_t(k.colorHash) << 32) |
                                       k.mountColorHash);
      h ^= std::hash<uint64_t>{}((uint64_t(k.lookMount) << 32) | k.lookAddon) +
           0x9e3779b9 + (h << 6) + (h >> 2);
      h ^= std::hash<uint64_t>{}((uint64_t(k.lookMountHead) << 32) |
                                 k.lookMountBody) +
           0x9e3779b9 + (h << 6) + (h >> 2);
      return h;
    }
  };
  std::unordered_map<RenderKey, std::unique_ptr<CachedDC>, RenderKeyHash>
      colored_dc;

  bool is_resident =
      false; // Tracks if this GameSprite is in resident_game_sprites
  ...
  void updateSimpleStatus() {
    is_simple = (numsprites == 1 && frames == 1 && layers == 1 && width == 1 &&
                 height == 1 && !spriteList.empty());
  }

protected:
  // Cache for default state (0,0,0,0) to avoid lookups/virtual calls for simple
  // sprites
