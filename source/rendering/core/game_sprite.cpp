...
GameSprite::GameSprite()
    : id(0), height(0), width(0), layers(0), pattern_x(0), pattern_y(0),
      ...
      drawoffset_x(0), drawoffset_y(0), minimap_color(0), is_simple(false) {
...
void GameSprite::ColorizeTemplatePixels(uint8_t *dest, const uint8_t *mask,
                                        size_t pixelCount, int lookHead,
                                        int lookBody, int lookLegs,
                                        int lookFeet, bool destHasAlpha) {
...
uint32_t GameSprite::getSpriteId(int frameIndex, int pattern_x,
                                 int pattern_y) const {
  auto idx =
      getIndex(width, height, 0, pattern_x, pattern_y, 0,
               frameIndex); // Assuming layer, pattern_z are 0 for this context
  if (idx >= 0 && static_cast<size_t>(idx) < spriteList.size() &&
      spriteList[idx]->isNormalImage()) {
...
size_t GameSprite::getIndex(int width, int height, int layer, int pattern_x,
                            int pattern_y, int pattern_z, int frame) const {
  if (is_simple) {
    return 0;
  }
  if (this->frames == 0) {
  ...
  idx = idx * static_cast<size_t>(this->pattern_z) +
        static_cast<size_t>(pattern_z);
  idx = idx * static_cast<size_t>(this->pattern_y) +
        static_cast<size_t>(pattern_y);
  idx = idx * static_cast<size_t>(this->pattern_x) +
        static_cast<size_t>(pattern_x);
...
const AtlasRegion *GameSprite::getAtlasRegion(int _x, int _y, int _layer,
                                              int _count, int _pattern_x,
                                              int _pattern_y, int _pattern_z,
                                              int _frame) {
  if (numsprites == 0) {
    return nullptr;
  }
  ...
  if (_count == -1 && numsprites == 1 && frames == 1 && layers == 1 &&
      width == 1 && height == 1) {
    // Also check default params
    if (_x == 0 && _y == 0 && _layer == 0 && _frame == 0 && _pattern_x == 0 &&
        _pattern_y == 0 && _pattern_z == 0) {
      // Check cache
      ...
      // Optimization: Check lightweight fields BEFORE calling heavy
      // getAtlasRegion()
      ...
      // Cache miss or staleness suspected: Use the getter to ensure
      // self-healing check runs
  ...
  if (_count >= 0 && height <= 1 && width <= 1) {
    ...
    v = ((((((_frame)*pattern_y + _pattern_y) * pattern_x + _pattern_x) *
               layers +
           _layer) *
              height +
          _y) *
...
TemplateImage *GameSprite::getTemplateImage(int sprite_index,
                                            const Outfit &outfit) {
  // While this is linear lookup, it is very rare for the list to contain more
  // than 4-8 entries, so it's faster than a hashmap anyways.
  auto it = std::ranges::find_if(
      instanced_templates, [sprite_index, &outfit](const auto &img) {
        if (img->sprite_index != sprite_index) {
          return false;
        }
        uint32_t lookHash = img->lookHead << 24 | img->lookBody << 16 |
...
const AtlasRegion *GameSprite::getAtlasRegion(int _x, int _y, int _dir,
                                              int _addon, int _pattern_z,
                                              const Outfit &_outfit,
                                              int _frame) {
  if (numsprites == 0) {
    return nullptr;
  }
...
wxMemoryDC *GameSprite::getDC(SpriteSize size) {
  ...
  if (!dc[size]) {
    ...
    g_gui.texture_gc.addSpriteToCleanup(this);
  }
  return dc[size].get();
}
...
void GameSprite::DrawTo(wxDC *dc, SpriteSize sz, int start_x, int start_y,
                        int width, int height) {
  const int sprite_dim =
      (sz == SPRITE_SIZE_64x64) ? 64 : (sz == SPRITE_SIZE_32x32 ? 32 : 16);
  ...
  wxDC *sdc = getDC(sz);
  if (sdc) {
    dc->StretchBlit(start_x, start_y, width, height, sdc, 0, 0, src_width,
                    src_height, wxCOPY, true);
  } else {
...
void GameSprite::DrawTo(wxDC *dc, SpriteSize sz, const Outfit &outfit,
...
bool ProcessTransparencyRun(std::span<const uint8_t> dump, size_t &read,
                            std::span<uint8_t> data, size_t &write,
  ...
  if (write + (transparent * RGBA_COMPONENTS) > data.size()) {
    spdlog::warn("Sprite {}: Transparency run overrun (transparent={}, "
                 "write={}, max={})",
                 ctx.id, transparent, write, data.size());
...
bool ProcessColoredRun(std::span<const uint8_t> dump, size_t &read,
                       std::span<uint8_t> data, size_t &write,
                       DecompressionContext ctx) {
  if (read + 1 >= dump.size()) {
    return false;
  ...
  if (write + (colored * RGBA_COMPONENTS) > data.size()) {
    spdlog::warn(
        "Sprite {}: Colored run overrun (colored={}, write={}, max={})", ctx.id,
        colored, write, data.size());
  ...
  if (read + (colored * ctx.bpp) > dump.size()) {
    spdlog::warn(
        "Sprite {}: Read buffer overrun (colored={}, bpp={}, read={}, size={})",
        ctx.id, colored, ctx.bpp, read, dump.size());
...
std::unique_ptr<uint8_t[]> GameSprite::Decompress(std::span<const uint8_t> dump,
                                                  bool use_alpha, int id) {
  ...
  DecompressionContext ctx{.id = id,
                           .bpp = bpp,
                           .use_alpha = use_alpha,
  ...
  // Debug logging for diagnostic - verify if we are decoding pure transparency
  // or pure blackness
  if (!non_zero_alpha_found && id > 100) {
    static int empty_log_count = 0;
    if (empty_log_count++ < 10) {
      spdlog::info("Sprite {}: Decoded fully transparent sprite. bpp used: {}, "
                   "dump size: {}",
                   id, bpp, dump.size());
    }
    ...
    if (black_log_count++ < 10) {
      spdlog::warn(
          "Sprite {}: Decoded PURE BLACK sprite (Alpha > 0, RGB = 0). bpp "
          "used: {}, dump size: {}. Check hasTransparency() config!",
          id, bpp, dump.size());
    }
  }
