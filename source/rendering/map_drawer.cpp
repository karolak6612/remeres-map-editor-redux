...
MapDrawer::MapDrawer(MapCanvas *canvas)
  ...
  tile_renderer = std::make_unique<TileRenderer>(
      item_drawer.get(), sprite_drawer.get(), creature_drawer.get(),
      creature_name_drawer.get(), floor_drawer.get(), marker_drawer.get(),
      tooltip_drawer.get(), &editor);
...
void MapDrawer::DrawPostProcess(const ViewState &view,
  ...
  ShaderProgram *shader =
      PostProcessManager::Instance().GetEffect(options.screen_shader_name);
  if (!shader) {
...
void MapDrawer::UpdateFBO(const ViewState &view,
                          const DrawingOptions &options) {
  // Determine FBO size.
  ...
  if (fbo_width != target_w || fbo_height != target_h || !scale_fbo) {
    ...
    if (fbo_width < 1 || fbo_height < 1) {
      // This should be impossible due to std::max, but good for invariant
      // documentation
      spdlog::error("MapDrawer: FBO dimension is zero ({}, {})!", fbo_width,
                    fbo_height);
    }
    fbo_resized = true;
  }

  // Update filtering parameters when scaling is enabled and either the FBO was
  // resized or the AA mode changed (scale_texture && (fbo_resized ||
  // options.anti_aliasing != m_lastAaMode))
...
void MapDrawer::Release() {
  // tooltip_drawer->clear(); // Moved to ClearTooltips(), called explicitly
  // after UI draw
}
...
void MapDrawer::Draw() {
  ...
  if (drag_shadow_drawer) {
    drag_shadow_drawer->draw(ctx, this, item_drawer.get(), sprite_drawer.get(),
                             creature_drawer.get());
  }

  live_cursor_drawer->draw(ctx, editor);

  brush_overlay_drawer->draw(ctx, this, item_drawer.get(), sprite_drawer.get(),
...
void MapDrawer::DrawMap() {
  ...
  for (int map_z = view.start_z; map_z >= view.superend_z; map_z--) {
    ...
    preview_drawer->draw(ctx, floor_params, canvas, map_z, editor,
                         item_drawer.get(), sprite_drawer.get(),
                         creature_drawer.get(), options.current_house_id);
...
DrawContext MapDrawer::MakeDrawContext() {
  return DrawContext{*sprite_batch, *primitive_renderer, view, options,
                     light_buffer};
}
