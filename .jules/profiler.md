...
## 2024-05-22 - Fix Excessive Coordinate Calculation
Finding: Screen coordinates were recalculated per tile in `IsTileVisible` inside the hot render loop, duplicating work done by the map iterator.
Impact: Coordinate calculation moved to incremental loop in callers (`MapLayerDrawer`, `IngamePreviewRenderer`), bypassing redundant projection logic.
Learning: Incremental coordinate calculation in tile loops is faster than per-tile projection. Tooltip generation must remain comprehensive for label visibility, despite performance cost.
